...
| INF-17 | SMS broker HTTP client | `crates/focusa-api/src/routes/sms.rs` | `CLIENT` caches one stateless HTTP connection pool | a process-shared transport could be mistaken for message, credential, connector, or scope authority | keep only the stateless client process-shared; every request retains typed scope, renewable credential reference, recipient, and receipt checks | P3 | client reuse retains no token, message, recipient, customer, or project state |
| INF-18 | Backup status verification cache | `crates/focusa-core/src/runtime/backup.rs` | `VERIFICATION_CACHE` retains at most 64 metadata fingerprints for already cryptographically verified generations | an unbounded or authoritative cache could consume memory or let eviction/change substitute for deep verification | keep the status-read optimization bounded and non-authoritative; explicit verification and restore always rehash artifacts, while durable backups, manifests, and receipts remain untouched | P3 | cache remains bounded; eviction cannot change durable state; deep verification catches tampering regardless of cache state |
| INF-19 | Screenshot-evidence-share settings write lock | `crates/focusa-api/src/routes/screenshot_settings.rs` | `WRITE_LOCK` serializes one atomic settings file write at a time | a route-level lock could be mistaken for settings authority or reorder durable records | Spec 185 settings authority lives in the daemon reducer and its signed projection; the route only serializes its own atomic file I/O and every decision re-reads the reducer-owned record | P3 | the lock serializes I/O only; settings authorization always resolves from the daemon-owned authoritative record (Spec 104 singleton inventory gate plus screenshot settings route tests) |
| INF-20 | Temporal keystore test env lock | `crates/focusa-core/src/temporal_integrity.rs` | `ENV_LOCK` serializes process env mutation inside the temporal keystore regression tests only | unserialized env mutation could flake tests or leak test env into production state | cfg-test-only infra lock; production keystore resolution never reads this static and custody decisions always come from the keyring/file result | P3 | the lock serializes test-only env setup; the Spec 104 singleton inventory gate plus temporal_integrity regression tests cover it |
