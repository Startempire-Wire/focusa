...
- `workpoint_id` — optional explicit Workpoint; omit to use active Workpoint.
- `project_root` — explicit project root for canonical evidence linkage.
- `session_id` / `continuity_id` — optional identity axes; continuity is part of the authority boundary.
...
When attached, the tool links a bounded evidence ref to the active Workpoint. It may also:
