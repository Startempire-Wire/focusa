...
- Scope mismatch: verify ProjectIdentity before trusting context. If `status=rejected_current_ask_scope_conflict` or `action_authority_for_current_ask=false`, hard-stop; the trajectory packet is stale/advisory and must not provide an executable next step across the project mismatch.
