...
Writer/readback consistency (#621): goal-definition admission uses the same verified project anchor, exact nonempty continuity, canonical-record and corroborating session-provenance checks as the active reader. An otherwise valid candidate that cannot satisfy those checks returns `trajectory_scope_unverified`, with `canonical`, `persisted` and `mutates_canonical_state` false, before goal or HLT/Ladder commits. Verify the project/session binding and retry with exact continuity; this does not retroactively authorize historical records or weaken reader checks. Source regression evidence and installed-consumer verification remain separate. Checkpoints of an existing scoped trajectory retain that trajectory's identity, including when the checkpoint request has a different idempotency key; otherwise checkpoint history cannot be selected by goal readback. The isolated HTTP regression in `tests/trajectory_scope_http_e2e.py` requires an explicit daemon binary and its expected SHA-256, preserves fixture evidence, and checks restart readback without touching the installed daemon.

The HTTP trajectory view derives `hlt_version` from the existing exact-project, exact-continuity, exact-trajectory ledger revision only when HLT/MLG/STG agree. `ledger_coherence` distinguishes matched, mismatching, absent and unreadable revision evidence; absent proof leaves the version null. This read-only comparison does not create authority, repair historical records or replace installed-consumer proof.
...
| `frame_unavailable` | Pi has no active Focus Frame/session frame key; model-facing fallback headline is `Attentive and awaiting operator direction`. | Do not claim Focus State write; save scratch fallback and recovery details. | Re-establish/ensure Pi frame, then retry durable write if still relevant. |
...
10a. Pi must interruptively prompt for vital project information when passive flags are insufficient. Default-on surfaces are project_root, project_verify, Workpoint, and Trajectory. Every other tool surface is opt-in through `vitalInfoPromptSurfaces`. Behavior is controlled by `vitalInfoPromptMode` (`prompt`/`warn_only`/`off`).
...
- Pi session start/resume and `before_agent_start` block or scope-limit durable Focusa binding on unconfirmed project_root until the operator confirms a safe project root through the default-on vital-info prompt, or explicitly downgrades the prompt mode.
