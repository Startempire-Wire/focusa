# Spec 116 — Focusa Mission Deck, Guided Onboarding, Recall, and PWA Surface

## 0. Status

**Status:** proposed / GTM-ready specification  
**Spec number:** 116  
**Owner:** Verious Smith  
**Created:** 2026-07-05  
**Scope:** Focusa TUI evolution, first-run onboarding, repeatable product education, guided walkthroughs, visual Mission Deck language, Focusa Recall integration, PWA/browser Mission Deck, optional terminal-in-browser bridge, release/GTM proof gates.

**Product surface name:** Focusa Mission Deck  
**Short form:** Mission Deck / Deck  
**Existing binary:** `focusa-tui` remains supported  
...
This spec extends existing Focusa architecture. It does not redefine core authority surfaces.

| Existing surface | This spec uses it for |
| --- | --- |
| `README.md` current runtime | Local-first daemon/API/CLI/TUI/Pi/menubar product shape |
| Spec 27 — TUI | Existing ratatui TUI principles: real-time, introspective, non-invasive, calm, composable, first-class capability client |
| Spec 100 — Context Cognition | Advisory context packet model, selected context, contradiction flags, stale warnings, route guidance |
| Spec 107 — Spec-first lifecycle | Required lifecycle: Idea → New Spec → bead/task decomposition → implementation → tests/proofs → closure |
| Spec 111 — Agent Context Bootstrap & Delivery | Mission/Workpoint/Trajectory/Evidence delivery packets for cold agent sessions |
| Spec 112 — Install Binary Architecture | Release/install path for `focusa`, `focusa-daemon`, and `focusa-tui`; first-install walkthrough; license/eval mode |
| Spec 114 — Public Benchmark Flywheel | Redacted public proof snapshots and claim discipline |
| Spec 115 — Cloud Control Plane | Local-first SaaS boundary, pairing, device trust, PWA/remote access, proof receipts, node registration |
| `docs/current/AUTHORITY_MODEL.md` | Operator steering, ProjectIdentity, Continuity ID, Workpoint, Evidence, Context Authority role boundaries |
| `docs/current/WORKPOINT_SESSION_SCOPE_GUARD.md` | `project_root + continuity_id` authority boundary and action-authority suppression |
...
Focusa already has a real TUI and a deep runtime. The current TUI is useful for operators who already understand Focusa, but it is not yet the best first encounter for a launch buyer.
...
They do not yet understand what to do first.
They do not feel the “mission continuity” value within five minutes.
```

The product already has the substance. The missing layer is a guided, repeatable product education system that converts the runtime into an unforgettable operator experience.

This spec turns the existing TUI into Focusa Mission Deck: a terminal-native and later browser/PWA mission surface that teaches and governs the core product loop:
...
Focusa Mission Deck should make the product explain itself through live state, visual walkthroughs, and safe next actions.
...
Adjacent products validate developer appetite for terminal-native, browser-accessible, cross-agent memory surfaces. Focusa’s differentiation is that memory is not enough. Focusa Recall and Mission Deck must show:
...
- a bypass around Spec 107 lifecycle gates;
- a bypass around Spec 112 install, checksum, license, update, or recovery rules;
...
A repeatable, state-aware process education flow. Walkthroughs are not one-time tutorials. They can be repeated for onboarding, recovery, release readiness, and agent handoff.
...
The existing TUI has these high-value foundations:

- `crates/focusa-tui` exists as a Rust crate.
- Binary name: `focusa-tui`.
- Dependencies include `ratatui`, `crossterm`, `tokio`, `reqwest`, `serde`, and `serde_json`.
- It polls the Focusa API at `FOCUSA_API_URL` or `http://127.0.0.1:8787`.
- It has tabs for Focus State, Focus Stack, Gate, Events, Metrics, Lineage, WorkLoop, Autonomy, Constitution, Telemetry, RFM, Proposals, Skills, UXP, Training, References, Cache, Contribution, and Intuition.
- It already fetches several Focusa endpoints such as state dump, work-loop status, closure evidence, autonomy, constitution, telemetry, proposals, skills, training status, and project identity.
...
- no launch-demo acceptance path proving a new buyer can understand Focusa in five minutes.

---

## 8. Experience principles

Mission Deck must follow these rules.
...
- a verified project root;
- a continuity id;
- a Workpoint or Workpoint candidate;
- at least one Evidence Ref or an explicit proof gap;
- a rendered next safe action;
- a visible explanation of how an agent can resume.
...
```text
Internal: no canonical Workpoint for verified project_root + continuity_id
Beginner: Focusa does not yet know what project this mission belongs to. Bind this folder first.
```

```text
Internal: evidence refs missing
Beginner: The agent says work is done, but Focusa has no proof yet. Attach a test, file, screenshot, or command output.
```

```text
Internal: action_authority_for_current_ask=false
Beginner: This saved mission belongs to a different scope than your current request. Review or rebind before changing files.
```

```text
Internal: daemon unreachable
Beginner: The Focusa background service is not running. Start it, then reopen Mission Deck.
```
...
Walkthroughs are exposed through:
...
Use existing Focusa device pairing concepts from Spec 53/115:
...
or, if launch speed requires reuse of existing Svelte/Tauri UI code:
...
Spec 116 is launch-relevant. It must not be marked done because the idea is written or because a mock exists.
...
Mission Deck includes a Recall tab or Recall command surface that clearly labels results as advisory and shows scope/proof/freshness.
...
Spec 112 must remain respected:
...
Spec 107 applies. No bead or release claim may state Mission Deck/PWA/TUI onboarding is complete unless the required surface has actual evidence.
...
- Add this spec.
- Create parent bead `focusa-116-arch`.
- Create child beads listed in §23.
- Add static guard ensuring Spec 116 exists before implementation.
...
- Add Agent Handoff and No Proof, No Done walkthroughs.
...
- Add memory status labels: active/stale/superseded/contradicted/noise/quarantined.
...
```text
focusa-116-arch — EPIC: Implement Spec 116 Mission Deck onboarding, Recall, and PWA surface
```

Recommended children:

```text
focusa-116-spec-static-guard
focusa-116-tui-title-and-home
focusa-116-beginner-mode-state-machine
focusa-116-help-overlay
focusa-116-next-safe-action
focusa-116-walkthrough-schema
focusa-116-first-mission-walkthrough
focusa-116-agent-handoff-walkthrough
focusa-116-no-proof-no-done-walkthrough
focusa-116-mission-ladder-panel
focusa-116-proof-meter-and-scope-badge
focusa-116-recall-tab
focusa-116-recall-card-schema
focusa-116-workpoint-candidate-promotion
focusa-116-deck-cli-alias
focusa-116-deck-api-routes
focusa-116-pwa-static-shell
focusa-116-pwa-pairing
focusa-116-pwa-read-only-gate
focusa-116-terminal-bridge-readonly-design
focusa-116-release-install-postcard
focusa-116-gtm-five-minute-proof
focusa-116-public-docs-sync
```
...
```text
tests/spec116_mission_deck_static_test.sh
tests/spec116_walkthrough_schema_static_test.sh
tests/spec116_tui_beginner_mode_static_test.sh
tests/spec116_recall_card_authority_static_test.sh
tests/spec116_pwa_safe_surface_static_test.sh
tests/spec116_no_raw_terminal_default_static_test.sh
tests/spec116_deck_cli_alias_static_test.sh
```
...
Spec 116 implementation may be called complete only when:

1. Spec 116 exists in `docs/`.
2. Bead decomposition exists before implementation completion claims.
3. Native Mission Deck home exists in the TUI.
4. Beginner Mode renders at least disconnected, unbound, no-Workpoint, no-evidence, and resumable states.
...
Spec 116 makes the launch experience easier to understand, easier to repeat, and easier to prove.

The Mission Deck is not a cosmetic dashboard. It is the guided operator surface that turns Focusa from “powerful infrastructure” into “I understand why I need this.”
