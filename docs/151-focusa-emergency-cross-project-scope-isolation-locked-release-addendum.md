...
Current proof: core `733/733`; API `431/431`. CLI parity now includes explicit dry-run/status/apply/rollback commands with file-based selections (`114/114` CLI tests), and Pi parity includes a searchable guarded tool whose apply/rollback actions require `confirm=true` (`32/32` Pi test files). An isolated default-stack daemon proved project A proposal count `1` versus project B count `0`, migration read isolation, and identical ownership after daemon restart/replay; evidence: `docs/evidence/spec151/ESI-018-runtime-restart-isolation.txt`. The PRE special full-snapshot submission path was replaced by canonical `ProposalSubmitted` reducer events after it produced a reproducible stack overflow.

Final P0 disposition: explicit read/bash foreign-project and rotated-continuity cases pass in the dedicated cross-project suite; raw ECS storage and rendered augmentation are independently scope-gated. CLI and Pi own confirmed migration actions. Canvas, TUI, menubar, browser, and silent-session surfaces expose no ontology scope-migration mutation route and remain fail-closed/read-only for this operation. The candidate is `0.9.137-dev`; installing it over the Mac's managed `0.9.141` extension would be a downgrade, so final Mac update/activation proof remains with the existing final release decision tasks rather than blocking this candidate-isolation P0.
