...
Every task in the completion DAG, Desktop callgraph, executable callgraph, and transition graph references `CARDINAL-135-SVELTE-001`. A task can close only when its cited Spec 135 behavior is available through the Svelte Mission Canvas destination, or when the task explicitly implements a shared Core/generated-runtime dependency used by that destination. Screenshots, fixtures, terminal output, or scaffolds alone cannot establish closure.
