<?php
// Spec 172 UIAI Operator Lifetime v1 issuance projector (addendum sections 6.3, 7.2,
// 7.3, 8, 11, 16.1, 16.3, 17, and 21; atom focusa-vbcqu.20.15.14). Consumes exactly one
// issued canonical EDD Software Licensing key for one verified complete eligible UIAI
// order and projects `uiai_operator_lifetime_v1` from canonical EDD truth:
//
...
//     limit, node, seat, hosted right, or commercial right
//     (CLIENT_COMMERCIAL_FIELDS_FORBIDDEN).
//   - Only `uiai_operator_lifetime_v1` (product `uiai_engine`) projects here. Any other
//     canonical License Type (for example focusa) fails closed with
...
//   - The projection freezes the local family set, seat/node limits, price version,
//     hosted-resource exclusions, and a strictly monotonic per-account sequence. The
//     family digest is a SHA-256 over the canonical frozen family record (Spec 172
//     section 7.2 families), the price version is derived only from the server-owned
//     offer price and contract version, and the hosted-resource exclusion digest comes
//     from the frozen UIAI hosted-resource exclusion registry (unlimited hosted compute,
//     paid proxies, third-party API consumption, paid model usage, managed hosting,
//     resale, redistribution, and product embedding are all excluded unless separately
//     granted). The sequence advances on the authority account in the same transaction
//     as the journal row (guard update; ENTITLEMENT_SEQUENCE_ROLLBACK_DENIED on
//     regression).
...
// schema migration), docs/contracts/spec172-uiai-hosted-resource-exclusion-registry.v1.php,
// and the server-owned dedicated downloads contract
// (docs/contracts/spec172-edd-operator-v1-downloads.v1.php) to be loaded first.
declare(strict_types=1);

final class UiaiSpec172UiaiOperatorProjector
{
    public const SCHEMA = 'focusa.spec172.license_type_projection.v1';
    public const RESULT_SCHEMA = 'focusa.spec172.uiai_operator_lifetime_projection.v1';
    ...
    /**
     * Frozen UIAI Operator v1 local/product family set (Spec 172 section 7.2): browser
     * observation, browser action, local persistence, diagnostics, proof packets,
     * batch/responsive workflows, and supported integrations. The first three match the
     * Spec 152F registered UIAI family ids (uiai_public_observation, uiai_browser_action,
     * uiai_persistence); the remaining four are the Spec 172 section 7.2 families using
     * the same uiai_ prefix convention. New families never enter silently: the digest is
     * frozen over this exact ordered list.
     */
    ...
    private const CLIENT_CONTROLLED_FIELDS = [
        ...
        'hosted_resources', 'hosted_resource_exclusions', 'hosted_resource_exclusion_digest',
        'metered_limits', 'metered_capacity',
    ];
    ...
    /**
     * Project `uiai_operator_lifetime_v1` for exactly one issued canonical EDD license.
     * Required input:
     ...
     * limit, node, seat, hosted right, or commercial right. Returns a public-safe
     * projection decision; replays return the identical decision; duplicate projection
     * calls for the same issued request return the same projection with existing=true and
     * zero creations.
     */
    public function project(array $input): array
    {
        ...
        $digest = $this->requestDigest([
            'operation' => 'uiai_operator_lifetime_projection',
     ...
     * License Type ref, price id, amount, and an enabled mapping. Only the UIAI Operator
     * Lifetime v1 License Type projects here; any other canonical License Type (for
     * example focusa) fails closed with LICENSE_TYPE_NOT_INCLUDED.
     */
