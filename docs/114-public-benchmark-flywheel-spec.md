...
## 1.4 Existing Focusa Site Infrastructure Pattern (Observed)

Observed 2026-06-26 from UIAI browser, HTTP headers, WHM `domainuserdata`, and `as-user focusadev` read-only inspection.

| Site | cPanel user | Document root | Public stack | Notes |
|------|-------------|---------------|--------------|-------|
| `focusa.dev` | `focusadev` | `/home/focusadev/public_html` | WordPress on LiteSpeed/cPanel | Primary marketing site; WP REST exposed via `wp-json` |
| `arena.focusa.dev` | `focusadev` | `/home/focusadev/arena.focusa.dev` | WordPress + custom `focusa-arena-theme` | Already speaks public-safe receipts; has service discovery links (`openapi.json`, `llms.txt`, agent-card, MCP card) |
| `forge.focusa.dev` | `focusadev` | `/home/focusadev/forge.focusa.dev` | WordPress child theme (`forge-focusa-child`) | Cohort/commerce surface with Gravity Forms/Stripe |
| `engine.focusa.dev` | `focusadev` | `/home/focusadev/engine.focusa.dev` | WordPress child theme (`uiai-engine-child`) | UIAI Engine marketing surface |
| `perpetua.focusa.dev` | `focusadev` | `/home/focusadev/perpetua/public_html` | SvelteKit static frontend + Go API | Exception: `.htaccess` proxies `/api/*` and `/share/{id}/og-image` to `127.0.0.1:8090`; API requires `X-Agent-Key` |

Perpetua runtime evidence:

```text
process: /home/focusadev/tmp/server --port 8090 --pool-dir /home/focusadev/perpetua/.perpetua/logs
.htaccess: RewriteRule ^api/(.*) http://127.0.0.1:8090/api/$1 [P,L]
.htaccess: RewriteRule ^share/([^/]+)/og-image$ http://127.0.0.1:8090/share/$1/og-image [P,L]
/api/* unauthenticated response: {"code":"AGENT_KEY_INVALID","error":"unauthorized"}
```

Deployment implication:

1. `bench.focusa.dev` SHOULD start as the same cPanel/Cloudflare public-site pattern as the existing simple WP sites if it only needs static/public-safe benchmark content.
2. If `bench.focusa.dev` needs dynamic replay search, signed run lookup, OG image generation, or private-to-public snapshot services, use the **Perpetua hybrid pattern**: static frontend + local Go API behind cPanel/LiteSpeed proxy + key-gated `/api/*`.
3. `bench.focusa.dev` MUST NOT expose the internal Focusa daemon or `/v1/evals/*` directly to the public internet. Public APIs serve generated/redacted artifacts only.
4. Public benchmark snapshots can be generated by the daemon/runner and deployed into the public docroot as immutable JSON, then rendered by WordPress or static frontend.
5. Existing WordPress surfaces are the fastest path for marketing pages; Perpetua-style Go API is the pattern for authenticated agent/proof services.
6. `bench.focusa.dev` deploys must run as cPanel user `focusadev` (or immediately restore ownership with `fix-user-perms focusadev`) to avoid root-owned files in the user account.
