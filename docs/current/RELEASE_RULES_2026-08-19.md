...
| **Verification** | `scripts/verify-version-surfaces.py` tail in `create-dev-release-tag.sh` | Agent `gh release view` eyes | Script verifies `isLatest true` + asset count before exiting 0. |
