...
| Metacognition evaluation produced an outcome but lacked durable readback/promotion memory. | Persisted evaluations as first-class records, added `/v1/metacognition/evaluations/recent` and `focusa metacognition recent-evaluations`, and successful evaluations promote bounded retrieval captures. |
| Ontology memory pipeline proposed promotion artifacts but did not create durable follow-up signals. | Memory-pipeline now persists gated runtime artifacts and writes prediction follow-up records linked to the artifact/evidence refs. |
| Work-loop health/status existed but did not expose hot-path dispatch readiness. | `/v1/work-loop/health` now includes dispatch readiness, boundary reason, pause flags, and transport degradation while deep diagnostics remain opt-in. |
| Public licensing metadata conflicted between README and Cargo metadata. | Root BSL/source-available license, commercial terms, trademark/contribution/support files, commercial/CLA templates, and package `license-file` metadata are now present. |
