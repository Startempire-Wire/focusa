...
Current contract registry: **62 Focusa tools** across project identity, trajectory, Workpoint, evidence, traversal/reflexes, Focus State, work-loop, diagnostics, lineage, prediction, and metacognition.

Machine-readable registry: [`focusa-tool-choreography.json`](focusa-tool-choreography.json), also embedded at `GET /v1/ontology/tool-choreography`. It contains 188 weighted per-tool edges and exact `per_tool_next_tools` for all 62 tools, including `focusa_reflex_primitives`, `focusa_project_card_outcome`, and `focusa_session_transfer`. The live API also exposes `runtime_weight_adjustments` from evaluated predictions that cite `tool_edge:from->to`, so route ordering can improve from measured outcomes without overriding operator steering or safety gates.
