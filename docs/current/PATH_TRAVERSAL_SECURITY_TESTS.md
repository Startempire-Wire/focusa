...
| Focus/Workpoint/Trajectory | `project_root`, target refs | Cross-project state bleed | Continue project_root+continuity_id authority gates; add route-level malicious `../` payload tests. |
...
Focusa now has concrete path traversal regression tests for the cleanup surface changed during this remediation. Broader API path-route tests remain tracked as follow-up dynamic/security-scope work before any broad network exposure.
