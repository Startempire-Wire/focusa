...
| P2 | API base regression coverage missing | Sync/AddPeer now use shared `fetchJson`/`postJson`, but no focused regression guard verifies all panels honor `focusa_api_url`. | `src/lib/api.ts`, `SyncPanel.svelte`, `AddPeerModal.svelte` | Add a menubar regression check for shared API client usage and no hardcoded daemon URL outside defaults. |
