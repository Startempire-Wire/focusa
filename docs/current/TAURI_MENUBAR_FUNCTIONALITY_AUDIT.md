...
| Gate | Result | Notes |
|---|---:|---|
| `npm ci` | FAIL | `@sveltejs/vite-plugin-svelte@4.0.4` peers Vite `^5`, while package resolves Vite `6.4.1`. |
...
| `bun run check` | PASS after build | Fails before build if `.svelte-kit/tsconfig.json` is absent; passes after build. |
