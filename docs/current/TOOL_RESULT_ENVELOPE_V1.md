...
Agents should not parse prose to decide whether a Focusa tool succeeded. They should inspect structured fields such as status, failure class, retry posture, recovery/misuse hints, canonical/degraded flags, evidence refs, side effects, next-tool hints, and bounded reflex suggestions.
...
- `reflex_suggestions` — optional Spec97 Reflex Primitive ids for recurring recovery patterns; advisory only and never authority.
- `error` — structured error details when applicable.
- `raw` — compact compatibility copy of the underlying response; large payloads are omitted/capped.

## Agent usage rule

Use `status`, `failure_class`, `canonical`, `degraded`, `retry`, `recovery_hint`, `misuse_hint`, `next_tools`, and optional `reflex_suggestions` for recovery decisions. Treat `canonical=false`, `degraded=true`, `pending`, or `blocked` as a recovery state, not as a final success.
...
4. If `reflex_suggestions` is present, treat each id as bounded advisory metadata and inspect `focusa_reflex_primitives` only when it clarifies the next safe step.
5. Use `next_tools` as the safe route; do not stop at the error unless the operator asks.
...
Instance, token, visual workflow, memory mutation, session resume, sync receive/transfer implementation, focus mutation/toggle, Workpoint, Trajectory, and Traverse routes follow the same contract for missing inputs, invalid enum values, missing tokens, malformed visual evidence content, malformed sync timestamps, reducer/persistence failures, daemon dispatch failures, scope mismatches, and resource/degraded traversal states.
