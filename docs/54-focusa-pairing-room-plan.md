...
- **Tunnel bundling (multi-transport).** When `focusa pair` cannot find any verified phone-reachable transport, it tries a neutral bundle in order: (1) `cloudflared` quick tunnel, (2) Tailscale Funnel, (3) `bore.pub`, (4) `localhost.run`, (5) `ssh -R`, (6) operator-supplied `/etc/focusa/public-url`. The chosen URL is verified by the resolver before being written to `/etc/focusa/public-url`. The shell installer offers `--bundle-tunnel` for the same flow at install time.
