...
```
Intuition Engine        (subconscious: detects patterns, anomalies, repetition)
      ↓
  Focus Gate            (conscious filter: surfaces candidates, never auto-acts)
      ↓
 Focus Stack            (nested attention: exactly one active frame)
      ↓
 Focus State            (state of mind: intent, decisions, constraints, artifacts)
      ↓
Expression Engine       (voice: deterministic prompt assembly within token budget)
      ↓
  Model Invocation      (harness passes through to provider)
```
