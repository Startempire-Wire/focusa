...
For the launch-ready install → post-install → quickstart path, see the [Release Install Postcard](docs/RELEASE_INSTALL_POSTCARD.md). For a buyer/evaluator demo, run the [GTM Five-Minute Proof](docs/GTM_FIVE_MINUTE_PROOF.md).
...
- **Real GUI (preview).** The Tauri menubar app is in active testing — Svelte/web build and pairing flows are proven locally, while native macOS `.app`, Keychain, restart, screenshots/logs, and OS lifecycle proof remain tracked testing work. Not the flagship surface for v0.9.74-dev; **the daemon / CLI / TUI / Operator Preview is**.
...
Failures playbook and audit trail:
- `docs/failures-playbook.md` — human-readable index
- `release-proof/audit/audit.jsonl` — append-only events
- `release-proof/audit/categories.md` — failure category fix + guard list

**Every CI, runtime, release, or deploy failure must be appended to `release-proof/audit/audit.jsonl` with category, root cause, fix, guard, and test.** No silent fixes.
...
- `docs/NEWBIE_ONBOARDING_WALKTHROUGH_QA.md` — full newbie onboarding and walkthrough experience QA, audit checklist, and source-backed claim matrix.
- `docs/PUBLIC_GITHUB_SWEEP.md` — public GitHub repo sweep: allowed/avoid list, findings table, and verification steps.
