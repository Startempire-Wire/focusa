...
curl() {
  command curl \
    ...
    -H "x-scope-continuity-id: work-loop-policy-test" \
    "$@"
}
http_json() { curl -sS "$@"; }

source "$ROOT_DIR/tests/fixtures/admitted-project-scope.sh"
focusa_test_scope_create "$BASE_URL" work-loop-policy-test
...
CHECKPOINT_RESP=$(http_json -X POST "${BASE_URL}/v1/workpoint/checkpoint" \
  ...
  -d "{\"project_root\":\"${ROOT_DIR}\",\"continuity_id\":\"work-loop-policy-test\",\"work_item_id\":\"${WORK_ITEM_ID}\",\"mission\":\"verify work-loop policy consumption\",\"action_intent\":{\"action_type\":\"spec79_policy_consumption\",\"lifecycle_stage\":\"verify_outcome\",\"status\":\"ready\"},\"next_slice\":\"verify consumed continuation inputs\",\"canonical\":true}")
...
for _ in $(seq 1 40); do
  RESUME=$(http_json -X POST "${BASE_URL}/v1/workpoint/resume" -H 'Content-Type: application/json' \
    -d "{\"project_root\":\"${ROOT_DIR}\",\"continuity_id\":\"work-loop-policy-test\",\"mode\":\"compact_prompt\"}")
