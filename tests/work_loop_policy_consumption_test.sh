...
CHECKPOINT_RESP=$(http_json -X POST "${BASE_URL}/v1/workpoint/checkpoint" \
  ...
  -d "{\"project_root\":\"${SCOPE_ROOT}\",\"continuity_id\":\"work-loop-continuation-test\",\"work_item_id\":\"${WORK_ITEM_ID}\",\"mission\":\"verify work-loop policy consumption\",\"current_action\":\"spec79_policy_consumption\",\"next_slice\":\"verify consumed continuation inputs\",\"canonical\":true}")
...
for _ in $(seq 1 40); do
  RESUME=$(http_json -X POST "${BASE_URL}/v1/workpoint/resume" -H 'Content-Type: application/json' \
    -d "{\"project_root\":\"${SCOPE_ROOT}\",\"continuity_id\":\"work-loop-continuation-test\",\"mode\":\"compact_prompt\"}")
...
CTX_RESP=$(http_json -X POST "${BASE_URL}/v1/work-loop/context" \
  -H 'Content-Type: application/json' \
  -H "x-focusa-writer-id: ${ACTIVE_WRITER}" \
  ${FENCING_HEADERS[@]+"${FENCING_HEADERS[@]}"} \
