...
require "$API" '/v1/compaction/prepare'
require "$API" '/v1/compaction/verify'
...
require "$API" 'CREATE TABLE IF NOT EXISTS compaction_packets'
require "$API" 'packet_by_id_durable'
require "$API" 'persistence_warning'
...
require "$PI" 'prepareCompactionEpoch(event)'
require "$PI" 'runPostCompactionVerification(event, ctx)'
require "$PI" 'renderCompactionMissionPacket(projection)'
require "$PI" 'deliverAs: "nextTurn"'
require "$PI" 'focusa.compaction_mission_packet.v1'
require "$PI" 'focusa.compaction_memory_verdict.v1'
