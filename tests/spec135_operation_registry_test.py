...
for path, path_item in openapi["paths"].items():
    for method, operation in path_item.items():
        ...
        assert required_extensions <= operation.keys(), (
            operation_id,
            required_extensions - operation.keys(),
        )
        descriptor = operations[operation_id]
        assert (
            operation["x-focusa-core-action"]
            == descriptor["ownership"]["core_action_ref"]
        )
        assert operation["x-focusa-scope-keys"] == descriptor["scope"]["required_keys"]
        assert (
            operation["x-focusa-permissions"]
            == descriptor["control"]["permission_scopes"]
        )
...
print(
    f"Spec 135 Operation Registry: PASS ({len(operations)} operations, {len(binding_ids)} UI bindings)"
)
