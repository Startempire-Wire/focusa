...
CONTRACT = (
    ROOT / "docs/worksheets/focusa-877z.3-focus-frame-beads-validation-contract.yaml"
)
...
def main() -> None:
    ...
    if (
        data.get("status")
        != "canonical_focus_frame_push_requires_real_project_beads_issue"
    ):
    ...
    if (
        '"canonical": false' not in focus
        or '"reason": "beads_issue_not_found"' not in focus
    ):
    ...
    if not before(
        ...
        fail(
            "focus.rs must validate Beads existence before materializing FocusFramePushed"
        )
    if not before(
        push_body,
        "if !beads_issue_exists(&project_root, &beads_issue_id)",
        ...
        fail(
            "focus.rs must validate Beads existence before constructing FocusFramePushed"
        )
    ...
    if "canonical FocusFramePushed rejected" not in daemon:
        fail(
            "daemon Action::PushFrame must reject missing Beads issue before event emission"
        )
    if not before(
        daemon,
        ...
        fail(
            "daemon Action::PushFrame must validate Beads existence before FocusFramePushed event"
        )
    ...
    if not any(
        path.exists() and '"id":"focusa-877z.3"' in path.read_text()
        for path in issue_sources
    ):
        fail(
            "current project Beads JSONL sources must contain focusa-877z.3 as a real issue"
        )

    print(
        "✓ PASS: canonical FocusFramePushed creation validates real project Beads issue IDs"
    )
