...
assert_has apps/menubar/src/lib/api.ts 'postJson|normalizeToolResult|isDegraded|summarizeError|DEFAULT_API_URL' 'shared menubar API client exposes runtime-view helpers'
assert_has apps/menubar/src/routes/+page.svelte '/v1/project/identity|/v1/trajectory/view\?mode=summary|/v1/workpoint/resume|/v1/work-loop/health|/v1/telemetry/memory|/v1/doctor' 'menubar poll includes current Focusa Mission Canvas hot surfaces'
assert_has apps/menubar/src/lib/components/MissionCanvasView.svelte 'PROJECT|TRAJECTORY|POST /v1/workpoint/resume|GET /v1/work-loop/health|GET /v1/telemetry/memory|GET /v1/doctor' 'Mission Canvas panel includes current Focusa surfaces'
assert_has apps/menubar/src/lib/components/MissionCanvasView.svelte 'envelopeLabel|envelopeTone|evidenceCount|class:watch|class:bad|class="chip"' 'Mission Canvas panel retains result-envelope/status contract markers'
assert_has apps/menubar/src/lib/components/RuntimeView.svelte 'mission-canvas-grid|Focusa Mission Canvas runtime summary' 'runtime summary uses Mission Canvas naming'
assert_has apps/menubar/src/lib/components/TrajectoryPeek.svelte 'HLT / High-Level Trajectory|MLG / Mid-Level Goal|STG / Short-Term Goal|Waypoints|Defer to the operator while actively offering HLT-aligned MLGs, STGs, and Waypoints|Next Workpoint|Evidence refs|Checks / risks' 'trajectory peek renders official HLT/MLG/STG/Waypoint surfaces calmly'
...
assert_has apps/menubar/src/routes/+page.svelte "activeTab === 'mission-canvas'|MissionCanvasView|title=\"Mission Canvas\"" 'menubar exposes the Focusa Mission Canvas without generic Cockpit naming'
assert_has apps/menubar/src/lib/components/MissionCanvasView.svelte 'mission-canvas-view|Focusa Mission Canvas' 'Mission Canvas component naming is explicit'
...
assert_has apps/menubar/src/lib/components/MissionCanvasView.svelte 'manual gate|manual_proof_required' 'release proof UI renders manual proof gate'
...
assert_has apps/menubar/src/lib/components/SyncPanel.svelte 'Peer awareness; no automatic ownership changes|Local-first mode is fine|Pull gently|Confirm pull|requestPull|Listening for peers' 'sync panel copy remains calm local-first awareness with pull confirmation'
assert_has apps/menubar/src/lib/components/AddPeerModal.svelte 'confirmAdd|Confirm this peer is trusted|disabled=\{loading \|\| !confirmAdd\}' 'add-peer write is confirmation-gated'
assert_has docs/current/TAURI_MENUBAR_IMPLEMENTATION_GAPS.md 'Implemented in the current menubar slice|Remaining Phase 0 gaps|Next recommended implementation slices|v0\.9\.13-dev|manual_proof_required|sync write confirmations' 'menubar implementation gaps doc reflects current post-audit status'
...
echo "SPEC96 Menubar Mission Canvas foundation static test: PASS"
