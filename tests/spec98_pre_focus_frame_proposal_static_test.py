...
def main() -> None:
    ...
    handler = proposals[
        proposals.find("async fn submit_focus_frame_proposal") : proposals.find(
            "/// POST /v1/proposals — submit",
            proposals.find("async fn submit_focus_frame_proposal"),
        )
    ]
    if (
        "FocusaEvent::FocusFramePushed" in handler
        or "materialize_focus_event" in handler
    ):
        fail(
            "PRE focus-frame proposal handler must not emit/materialize FocusFramePushed"
        )

    for name, text in [
        ("focus.rs", FOCUS.read_text()),
        ("daemon.rs", DAEMON.read_text()),
    ]:
    ...
    if (
        ...
        fail(
            "proof suite static contract does not include PRE focus-frame proposal guard"
        )
    print(
        "✓ PASS: Spec98 PRE focus-frame proposal route is explicit noncanonical schema"
    )
