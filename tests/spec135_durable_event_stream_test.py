...
persistence = (ROOT / "crates/focusa-core/src/runtime/persistence_sqlite.rs").read_text()
persistence_tests = (ROOT / "crates/focusa-core/src/runtime/persistence_sqlite_test.rs").read_text()
...
assert {
    "schema", "event_id", "sequence", "cursor", "timestamp", "event_type",
    "schema_version", "scope", "source_state_revision", "payload_ref", "invalidate",
...
assert any(item["operation_id"] == "focusa.events.stream" for item in registry["operations"])

for marker in (
    "pub fn durable_events_after", "pub fn durable_event_sequence",
    "pub fn latest_durable_event_sequence", "event_hash_chain h",
    "ORDER BY h.chain_index ASC",
):
    assert marker in persistence
assert "durable_sequence_cursor_replays_after_restart_without_duplicates" in persistence_tests
assert "SqlitePersistence::new(&cfg)" in persistence_tests
assert "let reopened = SqlitePersistence::new(&cfg)" in persistence_tests

for marker in (
    '"last-event-id"', "resolve_durable_cursor", "state.events_tx.subscribe()",
    "durable_events_after(cursor, 256)", "record.sequence <= cursor",
    ".id(record.sequence.to_string())", "RecvError::Lagged", "focusa.stream_event.v1",
):
...
print("Spec 135 durable event stream: PASS (restart replay, cursor, Last-Event-ID, de-duplicated live tail)")
