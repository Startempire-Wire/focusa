                    ...
                    if line.startswith("data:"):
                        ...
                        if data:
                            try:
                                ...
                                self.events.append(
                                    {"time": datetime.now().isoformat(), "data": event}
                                )
                            except json.JSONDecodeError:
                                self.events.append(
                                    {"time": datetime.now().isoformat(), "raw": data}
                                )
...
def test_single_client():
    ...
    if client.connected:
        ...
        if client.events:
            print(
                f"  ✓ Sample event type: {client.events[0]['data'].get('type', 'unknown')}"
            )
        return True
    else:
...
def test_reconnection():
    ...
    if client.connected and second_count > 0:
        print("  ✓ Reconnection successful, stream resumed")
        return True
    else:
