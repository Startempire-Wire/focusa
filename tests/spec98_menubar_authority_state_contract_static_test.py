...
def main() -> None:
    ...
    if "tests/spec98_menubar_authority_state_contract_static_test.py" not in SUITE.read_text():
        fail("Spec98 suite does not run menubar authority contract guard")
    if "tests/spec98_menubar_authority_state_contract_static_test.py" not in PROOF_SUITE.read_text():
        fail("proof suite static contract does not include menubar authority contract guard")
