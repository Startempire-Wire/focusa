...
def main() -> None:
    ...
    for term in [
        ...
        "cannot be rendered as captured Focusa evidence",
    ]:
        if term not in doc:
    ...
    if (
        ok_code != 0
        or '"status": "verified"' not in ok_out
        or '"capture_status": "capture_pending"' not in ok_out
    ):
        fail(f"verified packet did not pass: {ok_out}")
    bad_project_code, bad_project_out = run(
        {
            "scope": {
                "project_root": "/home/other/project",
                "continuity_id": EXPECTED_CONT,
                "scope_source": "focusa_verified",
            }
        }
    ...
    if (
        bad_project_code == 0
        or "cross_project_uiai_scope_guard" not in bad_project_out
        or '"failure_class": "scope_mismatch"' not in bad_project_out
    ):
        fail(f"cross-project packet not blocked: {bad_project_out}")
    bad_cont_code, bad_cont_out = run(
        {
            "scope": {
                "project_root": EXPECTED_ROOT,
                "continuity_id": "other-cont",
                "scope_source": "focusa_verified",
            }
        }
    )
    ...
    unverified_code, unverified_out = run(
        {
            "scope": {
                "project_root": EXPECTED_ROOT,
                "continuity_id": EXPECTED_CONT,
                "scope_source": "caller_supplied",
            }
        }
    )
    ...
    if (
        ...
        fail(
            "proof suite static contract does not include UIAI cross-project scope guard"
        )
