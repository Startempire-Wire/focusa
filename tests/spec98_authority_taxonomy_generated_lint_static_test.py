...
def main() -> None:
    ...
    for term in [
        ...
        "affected_surfaces",
        "proof_commands",
        "side_effects_or_render_semantics",
        "--lint-changed",
        "ROUTE_TOOL_PREFIXES",
    ]:
        if term not in text:
    ...
    changed_missing = run(
        ["--lint-changed", "crates/focusa-api/src/routes/new_route.rs"]
    )
    if (
        changed_missing.returncode == 0
        or "missing --worksheet-id" not in changed_missing.stderr
    ):
        fail(
            f"route/tool lint did not fail without worksheet id: {changed_missing.stdout}\n{changed_missing.stderr}"
        )

    changed_valid = run(
        [
            "--lint-changed",
            "crates/focusa-api/src/routes/new_route.rs",
            "--worksheet-id",
            "shared.tool_result_envelope",
        ]
    )
    if (
        changed_valid.returncode != 0
        or "route/tool authority lint ok" not in changed_valid.stdout
    ):
        fail(
            f"route/tool lint did not pass with valid worksheet id: {changed_valid.stdout}\n{changed_valid.stderr}"
        )
    ...
    md = MD.read_text()
    for term in [
        "Focusa Authority Taxonomy",
        "Route/tool additions must cite",
        "Worksheet ID",
        "shared.tool_result_envelope",
    ]:
        if term not in md:
    ...
    if not entries or not any(
        e.get("worksheet_id") == "shared.tool_result_envelope"
        and e.get("authority_class")
        for e in entries
    ):
    ...
    for entry in entries:
        for field in [
            ...
            if entry.get(field) in (None, "", []):
                fail(
                    f"generated registry entry missing {field}: {entry.get('worksheet_id')}"
                )
    ...
    if (
        ...
        fail(
            "proof suite static contract does not include authority taxonomy generated lint guard"
        )
