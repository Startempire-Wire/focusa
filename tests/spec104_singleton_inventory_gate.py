...
MUTABLE_MARKERS = ("Mutex", "RwLock", "OnceLock", "LazyLock", "HashMap", "HashSet", "Vec<", "DashMap")
VALID_CLASSIFICATIONS = {"eliminate", "scope_key", "infra_allowlist", "consumer_migrate"}
...
def discover() -> dict[tuple[str, str], dict[str, object]]:
    ...
    marker_rules = [
        ...
        ("crates/focusa-api/src/routes/ontology.rs", "spec92_predictions.json", "CONSUMER_FILE:spec92_predictions.json"),
        ("crates/focusa-api/src/routes/metacognition.rs", "fn metacog_base_dir", "GLOBAL_DIR:runtime/metacognition"),
        ("apps/pi-extension/src/state.ts", "focusa-project-root.json", "GLOBAL_FILE:focusa-project-root.json"),
        ("apps/focusa-awareness/index.ts", "/data/wirebot/users/verious", "HARDCODED_SCOPE_DEFAULT"),
    ]
    for path_text, needle, symbol in marker_rules:
        path = ROOT / path_text
        if path.exists() and needle in path.read_text(errors="ignore"):
            findings[(path_text, symbol)] = {"path": path_text, "symbol": symbol, "kind": "unscoped_state_marker"}
    return findings


def main() -> int:
    parser = argparse.ArgumentParser()
    parser.add_argument("--closure", action="store_true", help="fail while any remediation remains open")
    ...
    for entry in entries:
        ...
        if entry.get("classification") not in VALID_CLASSIFICATIONS:
            errors.append(f"invalid classification for {key}: {entry.get('classification')}")
        ...
        if entry.get("classification") == "infra_allowlist" and entry.get("authority_bearing") is not False:
            errors.append(f"infra allowlist cannot be authority-bearing: {key}")

    findings = discover()
    unknown = sorted(set(findings) - set(by_key))
    if unknown:
        errors.extend(f"unclassified singleton/non-scoped state: {key}" for key in unknown)

    active_statuses = {"open", "scope_keyed", "infra_allowed", "migrated"}
    stale = sorted(
        key for key, entry in by_key.items()
        if entry.get("status") in active_statuses and key not in findings
    )
    if stale:
        errors.extend(f"inventory says active but source marker is absent; mark eliminated or update detector: {key}" for key in stale)

    spec_text = SPEC.read_text()
    for key, entry in by_key.items():
        if str(entry.get("annex_id")) not in spec_text:
            errors.append(f"inventory annex id absent from Spec 104: {key} -> {entry.get('annex_id')}")

    open_entries = [entry for entry in entries if entry.get("status") == "open"]
    if args.closure and open_entries:
        errors.append(f"Spec 104 closure blocked: {len(open_entries)} inventory entries remain open")

    print(f"Spec104 inventory: findings={len(findings)} classified={len(by_key)} open={len(open_entries)} closure={args.closure}")
    for entry in open_entries:
        print(f"OPEN {entry['annex_id']} {entry['path']}::{entry['symbol']} -> {entry['classification']}")
    if errors:
        for error in errors:
            print(f"FAIL {error}")
        return 1
    print("PASS: every detected singleton/non-scoped state marker is classified; no unknown or stale inventory entries")
    return 0
