...
def main() -> None:
    ...
    if data.get("status") != "focus_frame_writes_require_project_workstream_scope":
        fail("contract status is not focus_frame_writes_require_project_workstream_scope")
    ...
    if "Never adopt the daemon-global active frame" not in update_body:
        fail("update_delta must document daemon-global active frame is not write authority")
    if "focus_update_requires_frame_id_or_project_root_plus_continuity_id" not in update_body:
    ...
    for proof in ["static Focus push requires project_root and continuity_id", "static Focus update forbids daemon-global active frame adoption"]:
