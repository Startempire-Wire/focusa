...
CONTRACT = ROOT / "docs/worksheets/focusa-877z.12-ontology-projection-advisory-contract.yaml"
...
def assert_advisory_payload(body: str, name: str, canonical_key: str = '"canonical"') -> None:
    for needle in ['"advisory_only": true', '"promotion_path"', '"authority": ontology_projection_authority_metadata()', '"canonical_truth_mutation": false']:
    ...
    if canonical_key == '"projection_canonical"' and '"projection_canonical": false' not in body:
        fail(f"{name} must expose projection_canonical=false")


def main() -> None:
    contract = yaml.safe_load(CONTRACT.read_text())
    if contract.get("schema_version") != "focusa.ontology_projection_advisory_contract.v1":
        fail("unexpected .12 contract schema")
    rule = contract.get("normative_rule", "")
    for phrase in ["advisory by default", "not canonical task meaning", "Workpoint", "PRE proposal", "reducer/governance"]:
    ...
    assert_advisory_payload(fn_body(ontology, "adjacency_index_payload"), "adjacency_index_payload")
    assert_advisory_payload(fn_body(ontology, "working_set_payload"), "working_set_payload")
    assert_advisory_payload(fn_body(ontology, "ontology_context_payload"), "ontology_context_payload")
    assert_advisory_payload(fn_body(ontology, "affordances_payload"), "affordances_payload")
    assert_advisory_payload(fn_body(ontology, "retrieval_governor_payload"), "retrieval_governor_payload")

    cache = fn_body(ontology, "ontology_read_index_cache_metadata")
    assert_advisory_payload(cache, "ontology_read_index_cache_metadata", canonical_key='"projection_canonical"')
    if '"canonical_meaning": "cache_entry_freshness_only_not_task_authority"' not in cache:
    ...
    print("✓ PASS: ontology projections expose advisory/canonical flags and promotion path")
