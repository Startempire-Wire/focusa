...
grep -Fq 'bun.exe $tauriCli --version && bun.exe $tauriCli build --target $env:RUST_TARGET' "$APPVEYOR" || fail "AppVeyor must invoke the package-owned Tauri CLI and build menubar for both Windows targets"
