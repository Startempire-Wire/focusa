...
REQUIRED_ZERO_FRICTION = {"worksheet_lint", "docs_render", "route_scaffold", "tool_scaffold", "proof_bundle", "compact_output"}
...
def main() -> None:
    ...
    for profile_id, profile in profiles.items():
        for field in ["intended_use", "default_classification", "automatic_enforcement", "override_allowed"]:
    ...
    override_fields = set((data.get("advanced_override_schema") or {}).get("required_fields") or [])
    missing_override = REQUIRED_OVERRIDE_FIELDS - override_fields
    if missing_override:
        fail(f"missing override audit fields: {sorted(missing_override)}")
    forbidden = set((data.get("advanced_override_schema") or {}).get("forbidden_override_effects") or [])
    for required in ["erase_scope_validation", "hide_canonical_advisory_degraded_labels", "remove_evidence_handle_requirements", "drop_proof_requirements"]:
    ...
    for phrase in ["focusa taxonomy lint", "focusa taxonomy render-docs", "focusa new-route", "focusa new-tool", "focusa proof surface", "traffic_light_summary_plus_expandable_detail"]:
    ...
    for expected in ["tests/spec98_policy_profiles_defaults_static_test.py", "tests/spec98_authority_taxonomy_worksheet_static_test.py", "npm --prefix apps/pi-extension run check"]:
    ...
    if missing_taxonomy_profiles:
        fail(f"authority taxonomy missing profiles: {sorted(missing_taxonomy_profiles)}")
    ...
    for phrase in ["Opinionated defaults", "Named policy profiles", "Advanced overrides", "Zero-friction implementation model"]:
