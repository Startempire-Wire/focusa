...
if rg -n 'WorkLoopStatus::AwaitingHarnessTurn|reprompt_stale_ms' "$WORK_LOOP_FILE" >/dev/null 2>&1; then log_pass "stale awaiting-harness turns can be auto-reprompted"; else log_fail "awaiting-harness stale re-prompt wiring missing"; fi
...
if rg -n 'maybe_select_global_ready_work_item|re-selected work after unassigned turn state' "$WORK_LOOP_FILE" >/dev/null 2>&1; then log_pass "unassigned loop state can re-select from global ready queue"; else log_fail "unassigned loop state missing global re-selection wiring"; fi
