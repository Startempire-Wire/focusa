...
def main() -> None:
    ...
    doc = DOC.read_text()
    for term in [
        "capture=proposal_only",
        "capture=pending_focusa_tool",
        "capture=focusa_captured",
        "capture=workpoint_linked",
        "capture=rejected",
            ...
            json.dump(
                {
                    ...
                    if source == "focusa_verified"
                    else "present",
                    "evidence_ref": "uiai:test",
                },
                tmp,
            )
            path = tmp.name
        result = subprocess.run(
            [str(SCRIPT), path, "--json"], cwd=ROOT, capture_output=True, text=True
        )
        if (
            result.returncode != 0
            or needle not in result.stdout
            or f'"canonical": {str(canonical).lower()}' not in result.stdout
        ):
            fail(
                f"render case failed for {capture}/{source}: {result.stdout}\n{result.stderr}"
            )
    ...
    if (
        ...
        fail(
            "proof suite static contract does not include UIAI packet capture-status renderer guard"
        )
