...
rg -n 'capture_learning_signal_scoped\(' crates/focusa-api/src/routes/predictions.rs >/dev/null \
  || fail "prediction outcomes do not feed scope-safe metacognition capture"
rg -n 'append_prediction_record_scoped\(' crates/focusa-api/src/routes/metacognition.rs >/dev/null \
  || fail "metacognition evaluations do not create scope-safe follow-up predictions"
