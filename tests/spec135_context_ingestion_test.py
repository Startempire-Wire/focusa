...
proof_contract = json.loads(
    (BUNDLE / "spec135-c1-context-ingestion-proof.json").read_text()
)
...
scenario = json.loads(
    (BUNDLE / "uiai-eval.c1-context-ingestion.scenario.json").read_text()
)
...
for operation_id, method, path in (
    ...
    assert next(
        ...
        if binding["action_id"] == operation_id
    )
...
assert (
    openapi["paths"]["/v1/context/sources/ingest"]["post"]["operationId"]
    == "focusa.context.source.ingest"
)
assert (
    openapi["paths"]["/v1/context/adapters/docling/health"]["get"]["operationId"]
    == "focusa.context.adapter.docling.health"
)
...
for marker in (
    "Context source revision mismatch",
...
for marker in (
    ...
    "FOCUSA_DOCLING_BASE_URL",
    "Action::EmitEvent",
    "FocusaEvent::ContextSourceIngested",
...
for marker in (
    'operationId = "focusa.context.source.ingest"',
    'healthOperationId = "focusa.context.adapter.docling.health"',
    'client.POST("/v1/context/sources/ingest"',
    'client.GET("/v1/context/adapters/docling/health"',
    "FocusaSourceConnectorCard",
    "FocusaProgressStepper",
    "FocusaRecoveryCard",
    "content_base64: bytesToBase64(minimalPdf",
    "Evidence",
    "receipts=",
):
...
assert proof_contract["critical_path"] == [
    "SPEC135-F12",
    "SPEC135-C1",
    "SPEC135-C2",
    "SPEC135-C3",
...
print(
    "Spec 135 C1 Context ingestion: PASS (Markdown/code/PDF, Docling health, incremental reducer, generated clients/UI)"
)
