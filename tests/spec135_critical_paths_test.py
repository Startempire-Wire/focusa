...
edges = {(edge["from"], edge["to"]) for edge in dag["edges"] if edge["kind"] == "blocks"}
nodes = {node["requirement_id"] for node in dag["nodes"]}

assert [item["slice_id"] for item in slices] == alpha_order
assert len(slices) == 8
required_traversal = {
    "requirement_id", "greater_primitive", "schema", "reducer_event", "typed_api",
    "generated_typescript_client", "generated_go_client", "operation_registry_action_binding",
    "a2ui_lit_renderer", "trusted_focusa_svelte_element", "real_integration",
    "focused_runtime_tests", "uiai_eval_when_browser_facing", "evidence", "receipt", "closure",
}
...
for index, item in enumerate(slices):
    ...
    assert set(item["merge_gates"]) == actual_gates, (slice_id, item["merge_gates"], actual_gates)
    ...
    for path in paths:
        ...
        for pair in zip(path, path[1:]):
            assert pair in edges, f"{slice_id}: missing blocking edge {pair[0]} -> {pair[1]}"
    if index:
        assert [alpha_order[index - 1], slice_id] in paths

spec = (ROOT / "docs/135d-complete-implementation-order-framework-reuse-performance-and-no-deferral-spec.md").read_text()
...
print("Spec 135 critical paths: PASS (8 Alpha slices, all merge gates and feeder edges explicit)")
