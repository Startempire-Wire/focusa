...
def main() -> None:
    ...
    require(turns, "pi_uiai_agent_card → uiai_health → uiai_browser_open/read", "turns.ts")
    ...
    require(agents, "Generic `web_search` / `fetch_content` are fallbacks only", "/root/AGENTS.md")

    skill = VISION_SKILL.read_text()
    require(skill, "UIAI-first web/browser/research workflow", "vision skill")
    require(skill, "For any URL, website, browser, documentation, or web-research task, use UIAI first", "vision skill")
