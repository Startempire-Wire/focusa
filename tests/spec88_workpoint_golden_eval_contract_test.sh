...
assert_rg 'workpointDraftOptions|A\) Current task checkpoint|B\) Trajectory gap follow-up|C\) Verify first|D\) Skip for now|E\) Custom edit' "${ROOT_DIR}/apps/pi-extension/src/session.ts" "Pi Workpoint prompt offers mobile-friendly inferred A/B/C/D options before typing fallback"
