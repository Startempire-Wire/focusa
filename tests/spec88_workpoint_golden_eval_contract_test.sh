...
assert_rg 'triggerTurn: true|auto-resume turn submitted|lastCompactResumeKey' "${ROOT_DIR}/apps/pi-extension/src/compaction.ts" "Pi compaction submits exactly one post-compact auto-resume turn"
...
assert_rg 'idempotency_key|idempotent_replay' "${ROOT_DIR}/crates/focusa-api/src/routes/workpoint.rs" "Workpoint checkpoint implements idempotency-key replay semantics"
