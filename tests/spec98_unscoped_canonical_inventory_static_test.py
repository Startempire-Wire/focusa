...
PATTERNS = {
    ...
    "reducer_dispatch": re.compile(r"\.send\(\s*(?:focusa_core::types::)?Action::|Action::[A-Z]"),
    "singleton_active_pointer": re.compile(r"active_frame_id|focus_stack\.active_id|active_workpoint|current_task|active_writer|lastTrajectoryClarity|lastWorkpoint"),
    "pi_focusa_mutating_call": re.compile(r"focusaFetch\(\"/(?:focus|ascc|workpoint|trajectory|work-loop|evidence|visual-workflow|telemetry|project|metacog|predict)"),
}
...
def main() -> None:
    ...
    for entry in entries:
        ...
        if {"project_root", "continuity_id"} - scope_fields:
            fail(f"{entry.get('path')} missing required project_root+continuity_id scope fields")
        ...
        for field in ["status", "canonical", "advisory", "degraded", "stale", "scope_status", "failure_class", "next_tools"]:
    ...
    print(f"✓ PASS: unscoped canonical/action-affecting inventory covers {len(entries)} files")
