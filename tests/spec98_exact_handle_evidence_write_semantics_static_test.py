...
def main() -> None:
    ...
    if "tests/spec98_exact_handle_evidence_write_semantics_static_test.py" not in SUITE.read_text():
        fail("Spec98 suite does not run exact-handle evidence guard")
    if "tests/spec98_exact_handle_evidence_write_semantics_static_test.py" not in PROOF_SUITE.read_text():
