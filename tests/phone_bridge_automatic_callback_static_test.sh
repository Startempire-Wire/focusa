...
assert_has "$MAIN" 'BridgeRuntimeState' 'Tauri uses managed bridge callback state'
assert_has "$MAIN" 'Mutex<HashMap<BridgeAttachmentKey' 'Tauri bridge completion state is thread-safe'
assert_has "$MAIN" 'completions_by_attachment' 'Tauri stores callback completions by typed attachment'
