...
def main() -> None:
    ...
    if enum_start == -1 or const_start == -1:
        fail(
            "types.rs must define ProxyPipelineStage and PROXY_PIPELINE_STAGE_CONTRACT"
        )
    enum_body = types[enum_start:const_start]
    const_body = types[
        const_start : types.find("/// Separate systems participating", const_start)
    ]
    for stage in STAGES:
    ...
    if deterministic_true != ["DeterministicExpressionRender"]:
        fail(
            f"only DeterministicExpressionRender may set deterministic_render=true, got {deterministic_true}"
        )
    for stage, flag in [
    ...
    if "forward_request(" in openai_planner or "sanitize_for_compat" in openai_planner:
        fail(
            "Retrieval/enrichment planner must not perform provider IO or compatibility sanitation"
        )
    ...
    if (
        "build_operator_first_slice" in anthropic_sanitize
        or "inject_system_prompt" in anthropic_sanitize
    ):
        fail(
            "Provider compatibility shim must not perform retrieval planning or prompt injection"
        )
    ...
    for phrase in [
        ...
        if phrase not in expr_header:
            fail(
                f"Expression Engine header missing deterministic boundary phrase {phrase}"
            )
    for forbidden in [
        "forward_request",
        "sanitize_for_compat",
    ...
    print(
        "✓ PASS: proxy pipeline stages are explicit and deterministic expression render is isolated"
    )
