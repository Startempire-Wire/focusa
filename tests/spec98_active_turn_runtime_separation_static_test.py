...
def main() -> None:
    ...
    if data.get("status") != "active_turn_runtime_only_action_routed_and_diagnostics_handle_bounded":
    ...
    for term in ["active_turn_diagnostics_handle", "runtime-diagnostics:active-turn", "bounded_runtime_metadata_only", "raw_input_chars", '"raw_user_input": Value::Null', '"assembled_prompt": Value::Null', '"authority_class": "runtime_correlation"']:
        if term not in session:
            fail(f"session/status prompt_stats missing bounded diagnostics handle term: {term}")
    remaining = "\n".join(data.get("remaining_migration") or [])
    if "runtime cache" not in remaining or "diagnostics handle" in remaining:
        fail("contract must retain only runtime-cache follow-up after diagnostics-handle migration")
    ...
    print("✓ PASS: ActiveTurn snapshot persistence is skipped, API buffer updates are action-routed, and status diagnostics are handle-bounded")
