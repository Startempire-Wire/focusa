...
class RootedSelectionBoundary(unittest.TestCase):
    def test_no_global_fallback_remains(self):
        ...
        rooted_core = DAEMON.split("async fn next_ready_packet_for_parent", 1)[1].split(
            "async fn tranche_has_remaining_ready_work", 1
        )[0]
        self.assertNotIn("next_ready_packet_global", rooted_core)
        self.assertIn("Ok(None)", rooted_core)
