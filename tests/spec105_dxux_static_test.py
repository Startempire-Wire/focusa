...
def main() -> None:
    ...
    api = (
        read("crates/focusa-api/src/routes/dxux.rs")
    ...
    cli = (
        ...
        + read("crates/focusa-cli/src/commands/dxux.rs")
    ...
    for rel in [
        "docs/focusa-tools/tools/focusa_dxux_report.md",
        "docs/focusa-tools/tools/focusa_dxux_requirement.md",
        "docs/focusa-tools/tools/focusa_dxux_explain.md",
        "docs/focusa-tools/tools/focusa_dxux_digest.md",
    ]:
        if not (ROOT / rel).exists():
            fail(f"missing doc {rel}")

    print(
        "✓ PASS: Spec105 DX/UX real surfaces wired across core/api/cli/pi-contracts/choreography/docs"
    )
