...
for row in requirements:
    ...
    assert row["implementation_tasks"] and row["implementation_tasks"][0].startswith(
        "focusa-mc-"
    )
...
assert (
    next(r for r in requirements if r["requirement_id"] == "SPEC135-F0")[
        "closure_status"
    ]
    == "verified"
)
...
assert (
    framework["adoption_order"][-1] == "custom only after failing conformance fixture"
)

assert proof["schema"] == "focusa.spec135.proof_matrix.v1"
assert {r["requirement_id"] for r in proof["requirements"]} == ids
assert all(
    r["tests"] and r["evidence_requirements"] and r["receipt_requirements"]
    for r in proof["requirements"]
)
assert "permanent onboarding-to-exact-resume traversal" in proof["global_closure_gates"]

print(
    f"Spec 135 machine-readable delivery graph: PASS ({len(ids)} requirements, {len(dag['edges'])} edges)"
)
