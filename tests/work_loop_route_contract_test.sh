...
CHECKPOINT=$(curl -sS -X POST "${BASE_URL}/v1/workpoint/checkpoint" -H 'Content-Type: application/json' \
  -d "{\"project_root\":\"${SCOPE_ROOT}\",\"continuity_id\":\"work-loop-continuation-test\",\"work_item_id\":\"spec79-route-contract\",\"mission\":\"verify work-loop route contracts\",\"current_action\":\"route_contract\",\"next_slice\":\"verify typed route envelopes\",\"canonical\":true}")
