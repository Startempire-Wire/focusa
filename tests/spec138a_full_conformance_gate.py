...
def test_combined_source_coverage_has_exact_live_hashes_and_no_omissions() -> None:
    ...
    assert coverage["source_atom_count"] == 2738
    assert coverage["normative_requirement_count"] == 542
...
def test_complete_ledger_is_542_of_542_with_no_deferred_or_unknown_rows() -> None:
    ...
    assert len(rows) == 542
    assert sum(row["source_path"] == PARENT for row in rows) == 273
    assert sum(row["source_path"] == ADDENDUM for row in rows) == 269
    assert len({row["requirement_id"] for row in rows}) == 542
    assert len({row["source_atom_ref"] for row in rows}) == 542
    for row in rows:
...
def test_runtime_proof_map_is_exact_one_to_one_and_fully_proven() -> None:
    ...
    assert proof["row_count"] == proof["verified_row_count"] == 542
...
def test_all_nineteen_proof_families_and_exact_sha_receipt_are_verified() -> None:
    ...
    assert receipt["verified_requirement_count"] == receipt["normative_requirement_count"] == 542
