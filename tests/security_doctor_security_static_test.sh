...
for marker in \
  "DoctorCommand::Security" \
  "security_posture_payload" \
...
for marker in \
  ...
  "commands::doctor::run(cli.json, args)"; do
...
for marker in \
  ...
  "deployment/API security report" \
  "not a license-plan or entitlement command"; do
