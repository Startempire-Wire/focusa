...
request_schema = json.loads((BUNDLE / "json-schema/focusa.context_retrieve.request.v1.json").read_text())
response_schema = json.loads((BUNDLE / "json-schema/focusa.context_retrieve_response.v1.json").read_text())
...
scenario = json.loads((BUNDLE / "uiai-eval.c2-context-retrieval.scenario.json").read_text())
...
for marker in ["/v1/context/retrieve", "spawn_blocking", "canonical_sources", "receipt:context-retrieval"]:
...
assert any(binding["action_id"] == "focusa.context.retrieve" for binding in bindings["bindings"])
assert "/v1/context/retrieve" in openapi["paths"]
assert openapi["paths"]["/v1/context/retrieve"]["post"]["operationId"] == "focusa.context.retrieve"
...
assert result_props["contradictions"]["items"]["properties"]["status"]["const"] == "candidate"
...
assert proof["runtime_proof"]["vector_absence_behavior"] == "deterministic lexical fallback"
assert "test:focusa-core:context-vector-fastembed" in proof["evidence_refs"]

print(f"Spec 135 C2 Context retrieval: PASS ({len(operations)} operations; FTS5 + sqlite-vec + optional fastembed; cited/scoped/bounded)")
