...
def main() -> None:
    ...
    event_struct = types[
        event_struct_start : types.find("// ─── Workers", event_struct_start)
    ]
    for field in [
        "machine_id",
        "instance_id",
    ...
    columns = set(
        required.get("event_store_schema", {}).get("required_future_columns") or []
    )
    for col in [
        "project_root_key",
        "workstream_key",
    ...
    print(
        "✓ PASS: CRDT/event-store wiring foundation and explicit production gap are mapped"
    )
