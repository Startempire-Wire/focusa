...
SCOPE = {"project_root": "/example/focusa", "continuity_id": "focusa-cont-c2", "attachment_id": "attachment:c2"}
...
def retrieve(base: str, scope=SCOPE):
    status, body = request(base, "POST", "/v1/context/retrieve", {
        **scope,
...
def main():
    ...
    try:
        ...
        first = ingest(base, "c2-positive", "Release Policy A", "The release policy requires signed artifacts for every deployment.", current_version(base))
        second = ingest(base, "c2-negative", "Release Policy B", "The release policy does not require signed artifacts for every deployment.", first["state_version"])
        assert second["state_version"] > first["state_version"]

        result = retrieve(base)
        schema = json.loads((ROOT / "docs/contracts/spec135/generated-contract-v1/json-schema/focusa.context_retrieve_response.v1.json").read_text())
        ...
        assert payload["mode_requested"] == "hybrid" and payload["mode_used"] == "lexical"
        ...
        assert all(hit["citation"]["source_revision"] == "git:c2" for hit in payload["hits"])
        assert all(hit["citation"]["line_start"] >= 1 for hit in payload["hits"])
        first_ids = [hit["chunk_id"] for hit in payload["hits"]]

        _, isolated = request(base, "POST", "/v1/context/retrieve", {
        ...
        assert resumed["indexed_source_count"] == 2 and len(resumed["contradictions"]) == 1
        print("Spec 135 C2 Context retrieval E2E: PASS (FTS5, sqlite-vec projection, citations, contradictions, scope, fallback, restart)")
    finally:
