...
for attempt in 1 2 3; do
  ...
  if jq -e '.schema=="focusa.update_plan.v1" and .latest.trust.release_resolved==true and .latest.trust.checksums_resolved==true and .latest.trust.signature_verified==true and .latest.trust.manifest_signature_verified==true and .latest.trust.provenance_verified==true and .latest.trust.deploy_proof_verified==true and .latest.trust.key_revoked==false and (.latest.trust.trusted_key_id|type=="string")' <<<"$plan" >/dev/null; then
    break
  fi
  [[ "$attempt" == 3 ]] || sleep 2
done
jq -e '.schema=="focusa.update_plan.v1" and .latest.trust.release_resolved==true and .latest.trust.checksums_resolved==true and .latest.trust.signature_verified==true and .latest.trust.manifest_resolved==true and .latest.trust.manifest_signature_verified==true and .latest.trust.provenance_verified==true and .latest.trust.deploy_proof_verified==true and .latest.trust.key_revoked==false and (.latest.trust.trusted_key_fingerprint|test("^[0-9a-f]{64}$")) and (.latest.trust.blockers|length)==0 and (.safety.staging.verify_before_promote | index("asset_sha256")) and (.safety.staging.verify_before_promote | index("release_manifest_signature")) and (.safety.no_half_written_executable_rule | test("never write directly"))' <<<"$plan" >/dev/null || fail "plan missing live signed-manifest/deploy/provenance/no-half-written proof after bounded retries"
