...
def main() -> None:
    ...
    if not ledger.exists():
        ...
        pre_count_at_start = sum(
            1 for line in ledger.read_text().splitlines() if line.strip()
        )
    ...
    for line in ledger.read_text().splitlines():
        ...
        try:
            ...
            continue
        if (
            entry.get("schema") == "focusa.project_session_transfer.v1"
            and entry.get("continuity_id") == continuity_id
        ):
    ...
    if last.get("next_action") != next_slice:
        fail(
            f"ledger next_action mismatch: {last.get('next_action')!r} != {next_slice!r}"
        )
    ...
    if transfer.get("mission") != mission:
        fail(
            f"continue.transfer.mission mismatch: {transfer.get('mission')!r} != {mission!r}"
        )
    if transfer.get("next_action") != next_slice:
        fail(
            f"continue.transfer.next_action mismatch: {transfer.get('next_action')!r} != {next_slice!r}"
        )
    ...
    if "command" not in operator_handoff or "first_tool" not in operator_handoff:
        fail(
            f"continue.transfer.operator_handoff missing command/first_tool: {operator_handoff.keys()}"
        )
    ...
    if cp_resp.get("status") not in ("accepted", "completed"):
        fail(
            f"workpoint_checkpoint rejected: status={cp_resp.get('status')}, failure_class={cp_resp.get('failure_class')}"
        )
    ...
    if "save_packet: checkpoint?.body" in wrapper_src:
        fail(
            "Pi wrapper still has the old 'save_packet: checkpoint?.body' (gap not fixed)"
        )

    print(
        f"✓ PASS: save point function (session-transfer + workpoint primitive) evaluated; "
