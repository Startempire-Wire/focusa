...
for marker in ['pull-request-release-gate','Require clean PR queue and tag inclusion','git merge-base --is-ancestor','needs: [rust-check, final-release-gap-gate, open-issue-release-gate, pull-request-release-gate]','PREV_TAG=','RANGE="${PREV_TAG}..${TAG}"','### Breaking changes','### Features added','### Fixes shipped','### Merged pull requests','### Issues resolved','### Contributors','### Known issues','### Full changelog','SHA256SUMS','signature/provenance','focusa update rollback --dry-run=false --yes','### Downloads']:
