...
if rg -n 'otaEnabled|otaProfile|dev_auto_all|/update/policy' "$CMD_FILE" >/dev/null 2>&1; then log_pass "Pi settings panel exposes easy OTA policy controls"; else log_fail "Pi settings panel missing OTA policy controls"; fi
...
if rg -n 'name: "focusa_work_loop_status"' "$TOOLS_FILE" >/dev/null 2>&1 && rg -n 'replay=not_checked_hot_path|cold_paths_checked: false' "$TOOLS_FILE" >/dev/null 2>&1; then log_pass "focusa_work_loop_status tool keeps replay cold path out of hot summary"; else log_fail "focusa_work_loop_status tool missing hot/cold replay boundary"; fi
