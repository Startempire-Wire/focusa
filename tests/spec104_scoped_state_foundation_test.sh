...
jq -e '.properties.schema.const == "focusa.scoped_result.v1" and ."$defs".scopeRef.properties.scope_kind.enum == ["project","host"] and (.required | index("human_readable")) and .properties.human_readable.type == "string"' \
...
rg -q 'pub human_readable: String' "$ROOT/crates/focusa-core/src/scoped_state.rs"
rg -q 'export interface ScopedResultEnvelope' "$ROOT/apps/pi-extension/src/scoped-state.ts"
rg -Fq 'human_readable?: string' "$ROOT/apps/pi-extension/src/scoped-state.ts"
