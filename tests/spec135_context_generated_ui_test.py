...
scenario = json.loads((BUNDLE / "uiai-eval.alpha0-context-commit.scenario.json").read_text())
result = json.loads((BUNDLE / "uiai-eval.alpha0-context-commit.result.json").read_text())
...
binding = next(item for item in bindings["bindings"] if item["action_id"] == operation["operation_id"])
assert binding["scope"]["required_keys"] == ["project_root", "continuity_id", "attachment_id"]
assert binding["control"]["idempotency_required"] and binding["control"]["receipt_required"]
...
assert post["x-focusa-idempotency"] and post["x-focusa-concurrency"] and post["x-focusa-receipt"]
for schema_id in (
    "focusa.context_source_commit.request.v1", "focusa.context_source_commit_result.v1",
    "focusa.context_source_list.request.v1", "focusa.context_source_list.v1",
):
...
for marker in (
    "pub struct ContextSourceRecord", "pub struct ContextSourceEvidence",
    "pub struct ContextSourceReceipt", "ContextSourceCommitted",
    "pub context_sources: Vec<ContextSourceRecord>",
):
    assert marker in core_types
for marker in (
    "Context receipt version mismatch", "duplicate Context source commit",
    "context_source_commit_is_canonical_scoped_and_idempotency_guarded",
):
    assert marker in reducer
for marker in (
    "Action::EmitEvent", "FocusaEvent::ContextSourceCommitted",
    "expected_state_version", "idempotency_key", "evidence_ref", "receipt_ref",
    "ToolResultV1::success", "ToolResultV1::failure",
):
    assert marker in route
for marker in (
    'from "@focusa/spec135-client"', "ui-action-bindings.fixture.json",
    'operationId = "focusa.context.source.commit"',
    'allowedActionNames: new Set([binding.action_id])',
    'client.POST("/v1/context/sources/commit"', "FocusaEvidenceSummary",
):
...
print("Spec 135 F12 generated Context: PASS (binding/client/API/reducer/event/restart/Evidence/Receipt/UIAI)")
