...
CONTRACT = ROOT / "docs/worksheets/focusa-877z.21-workpoint-trajectory-active-scope.yaml"
...
def main() -> None:
    ...
    if data.get("schema_version") != "focusa.workpoint_trajectory_active_scope_contract.v1":
    ...
    if "active_workpoint(state)" in wp_body:
        fail("active_workpoint_for_scope must not fall back to daemon-global active_workpoint(state)")
    if "record.project_root.as_deref().map(str::trim) == Some(clean_project.as_str())" not in wp_body:
        fail("active_workpoint_for_scope must exact-match project_root")
    if "record.continuity_id.as_deref().map(str::trim) == Some(clean_continuity.as_str())" not in wp_body:
    ...
    if forbidden_scoped_fallback in wp:
        fail("scoped Workpoint resolution must not fall back to daemon-global active_workpoint")
    ...
    if "unwrap_or(true)" in tr_body:
        fail("active_persisted_trajectory must not use unwrap_or(true) wildcard scope matching")
    if "record.project_root.as_deref() == Some(expected_project_root.as_str())" not in tr_body:
        fail("active_persisted_trajectory must exact-match project_root")
    if "record.continuity_id.as_deref() == Some(expected_continuity_id.as_str())" not in tr_body:
        fail("active_persisted_trajectory must exact-match continuity_id")

    proofs = set(data.get("proof_requirements") or [])
    for proof in ["static selector fail-closed test", "two-project active Workpoint isolation test", "trajectory prior-project fallback advisory provenance test"]:
        if proof not in proofs:
            fail(f"contract missing proof requirement: {proof}")
    print("✓ PASS: Workpoint/Trajectory active selectors fail closed without project/workstream scope")
