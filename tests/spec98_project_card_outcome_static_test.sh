...
rg -n 'ProjectCardOutcomeRequest|/v1/project/card/outcome|project_card_algorithm_outcomes\.jsonl|append_project_card_algorithm_outcome|project_card_run_exists|update_weights_from_algorithm_outcome|projected_project_card_weights|TAIL_BYTES|project_card_outcome_stats|outcome_learning|outcome_bias|shortest_path_to_success|eliminated_candidates|prior_session_context|recent_decisions|focus_goal_signals' "$API" >/dev/null \
  || fail "project card outcome API/persistence/prior-context/outcome ranking/shortest-path optimization is missing"
