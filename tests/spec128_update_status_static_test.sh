...
API_AUTHORITY="$ROOT/crates/focusa-api/src/routes/update_authority.rs"
...
if rg -q 'latest version placeholder until the release manifest resolver is wired|apply remains disabled until Spec128 apply gates are implemented|once Spec128 planning is implemented|no mutation until all gates are wired|future apply|unless future gates are wired|Still blocked until implementation gates pass|apply_executor_not_enabled_in_spec128_07_scaffold|rollback_executor_not_enabled_in_spec128_08_scaffold' "$SRC" "$API" "$API_AUTHORITY"; then
  fail "completed OTA surface still advertises or accepts obsolete scaffold behavior"
fi
...
rg -q 'Apply\(UpdateApplyArgs\)|focusa.update_apply.v1|build_apply_envelope|execute_verified_apply' "$SRC" || fail "missing CLI guarded transactional apply"
...
rg -q 'execute_verified_rollback|update-history.jsonl|update_apply_blocked_total|trusted_dev_force_latest|focusa.update_admin_state.v1|mutation_requires_dry_run_false_and_yes|updates_paused_by_admin' "$SRC" || fail "rollback/history/admin control safety rules missing"
...
[[ -f "$API_AUTHORITY" ]] || fail "missing canonical update authority API adapter"
rg -q '/v1/update/plan|CliUpdateAuthority::installed' "$API" || fail "missing live signed-release plan API route"
rg -q 'focusa.update_plan.v1|canonical_update_authority_failed' "$API_AUTHORITY" || fail "plan adapter must validate canonical updater output and fail closed"
rg -q '/v1/update/apply|AuthorityApplyRequest' "$API" || fail "missing transactional update apply API route"
rg -q 'explicit_yes_allow_apply_and_dry_run_false_required|--allow-apply|focusa.update_apply.v1' "$API_AUTHORITY" || fail "API apply must enforce explicit consent and delegate to canonical authority"
rg -q 'focusa.update_rollback.v1|explicit_yes_and_dry_run_false_required|--dry-run=false' "$API_AUTHORITY" || fail "API rollback must enforce consent and restore through canonical authority"
