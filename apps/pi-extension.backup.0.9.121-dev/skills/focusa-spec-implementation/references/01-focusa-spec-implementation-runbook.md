# Focusa Spec Implementation Runbook
...
```text
focusa_call_stack_design -> focusa_trajectory_assess
focusa_trajectory_assess -> focusa_workpoint_checkpoint
focusa_workpoint_checkpoint -> focusa_call_stack_verify
focusa_call_stack_verify -> focusa_evidence_capture
```

## Minimal path

1. Call `focusa_call_stack_design` with only required bounded inputs.
2. Call `focusa_trajectory_assess` with only required bounded inputs.
3. Call `focusa_workpoint_checkpoint` with only required bounded inputs.
4. Call `focusa_call_stack_verify` with only required bounded inputs.
...
- Done: Implementation matches the typed call stack, acceptance criteria, task closure, and linked proof.
