...
```text
focusa_browser_workflow_plan -> focusa_browser_capabilities_intake
focusa_browser_capabilities_intake -> focusa_browser_diagnostics_intake
focusa_browser_diagnostics_intake -> focusa_evidence_capture
```

## Minimal path

1. Call `focusa_browser_workflow_plan` with only required bounded inputs.
2. Call `focusa_browser_capabilities_intake` with only required bounded inputs.
3. Call `focusa_browser_diagnostics_intake` with only required bounded inputs.
...
- Done: Browser result is proven in the bound session/origin, diagnostics are ingested, evidence is linked, and unused sessions are closed.
