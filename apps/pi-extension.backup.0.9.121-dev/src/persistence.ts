...
export function stableSemanticValue(value: any, key = ""): any {
  ...
  if (key === "projectSwitchLedger" && Array.isArray(value)) {
    return value
      ...
      .sort((a: any, b: any) =>
        `${a.project_root}\0${a.project_alias}`.localeCompare(`${b.project_root}\0${b.project_alias}`)
      );
  }
  ...
  for (const childKey of Object.keys(value).sort()) {
    if (volatile.has(childKey)) continue;
...
export function semanticPersistenceDigest(value: any): string {
...
function sidecarKey(projectRoot: string, sessionId: string): string {
  return createHash("sha256").update(`${projectRoot}\0${sessionId}`).digest("hex");
}

function validateSidecarKey(key: string): void {
  if (!/^[a-f0-9]{64}$/.test(key)) throw new Error("invalid persistence sidecar key");
}

function sidecarPathForRevision(key: string, revision: number, semanticDigest: string): string {
...
function sidecarCandidates(key: string): string[] {
  validateSidecarKey(key);
  const prefix = `${key}.r`;
  try {
...
export function writeRecoverySidecar(
  ...
  if (!sessionId) throw new Error("session id required for persistence sidecar");
...
export function loadPersistedRecoveryState(
  anchor: any,
  fallbackProjectRoot = ""
): Record<string, any> | null {
  if (!anchor || typeof anchor !== "object") return null;
  if (
    anchor.schema !== COMPACTION_PERSISTENCE_ANCHOR_SCHEMA &&
  ...
  if (!projectRoot || !sessionId || key !== sidecarKey(projectRoot, sessionId)) return null;

  for (const candidate of sidecarCandidates(key)) {
    try {
      const parsed = JSON.parse(readFileSync(candidate, "utf8"));
      if (parsed?.schema !== COMPACTION_PERSISTENCE_SIDECAR_SCHEMA) continue;
      const recoveryState = parsed.recoveryState;
      if (!recoveryState || typeof recoveryState !== "object") continue;
      if (semanticPersistenceDigest(recoveryState) !== parsed.semanticDigest) continue;
      if (String(recoveryState.sessionId || "") !== sessionId) continue;
