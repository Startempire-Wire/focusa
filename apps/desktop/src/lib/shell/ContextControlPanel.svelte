...
<style>
  .context-panel{position:absolute;top:52px;left:calc(100% - 6px);z-index:45;width:min(390px,calc(100vw - var(--sidebar-width,248px) - 24px));max-height:calc(100vh - 110px);display:grid;gap:var(--space-3);overflow-y:auto;padding:var(--space-3);border:1px solid var(--color-border-strong);border-radius:var(--radius-card);color:var(--color-text);background:var(--color-elevated);box-shadow:var(--shadow-popover);transform-origin:top left}
  ...
  @media(max-width:820px){.context-panel{position:fixed;top:70px;left:76px;width:min(390px,calc(100vw - 92px));max-height:calc(100vh - 116px)}}
</style>
