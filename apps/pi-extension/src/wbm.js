...
export async function fetchWbmContext() {
    ...
    const parts = ["[WIREBOT MODE ACTIVE]"];
    ...
    try {
        ...
        if (me.time_context)
            parts.push(`Time: ${me.time_context}`);
        if (me.phase)
            parts.push(`Phase: ${me.phase}`);
    }
    catch {
    ...
    try {
        const headers = sbToken ? { Authorization: `Bearer ${sbToken}` } : {};
        const score = await fetch(`${sbUrl}/v1/score`, { headers, signal: AbortSignal.timeout(2000) }).then((r) => r.json());
        if (score.drift !== undefined)
            parts.push(`Drift: ${score.drift}`);
        if (score.season)
            parts.push(`Season: ${score.season}`);
    }
    catch { }
    ...
    if (allFrames.length) {
        const active = stack?.active_frame_id
            ? allFrames.find((fr) => fr.id === stack.active_frame_id)
    ...
    try {
        ...
        const pillars = soul
            ...
            .slice(0, 5);
        if (pillars.length)
            parts.push(`Pillars: ${pillars.map((p) => p.replace(/^-\s*/, "")).join(" > ")}`);
        // §29: Banned patterns from SOUL
        const banned = soul
            .split("\n")
            .filter((l) => /banned|never|forbidden/i.test(l))
            .slice(0, 3);
        if (banned.length)
            parts.push(`Banned: ${banned.map((b) => b.replace(/^-\s*/, "").trim()).join(", ")}`);
    }
    catch {
...
export async function catalogueFromMessages(messages) {
    if (getAttachmentRuntime().wbmNoCatalogue)
        return 0;
    ...
    for (const msg of messages) {
        ...
        if (text.length > 50)
            allChunks.push(text.slice(0, 2000));
    }
    if (!allChunks.length)
        return 0;
    ...
    if (getAttachmentRuntime().focusaAvailable) {
        for (const window of windows) {
            try {
                ...
                if (r?.items?.length) {
                    extracted.push(...r.items
                        .map((i) => ({ type: i.type || "fact", text: i.text || "" }))
                        .filter((i) => i.text));
                }
            }
            catch {
    ...
    for (const item of extracted) {
        const ok = await wbExec(["memory", "queue", "--source", "pi_session", "--type", item.type, item.text], `${sbUrl}/v1/memory/queue`, { memory_text: item.text, source_type: "pi_session", confidence: 0.85, type: item.type });
        if (ok) {
            count++;
            if (item.type === "decision")
                getAttachmentRuntime().cataloguedDecisions.push(item.text);
            else
                getAttachmentRuntime().cataloguedFacts.push(item.text);
        }
    }
    return count;
}
// ── Register /wbm command with 7 subcommands ─────────────────────────────────
export function registerWbm(pi) {
        ...
        handler: async (args, ctx) => {
            ...
            switch (sub) {
                ...
                case "off":
                    ...
                    break;
                case "status":
                    ctx.ui.notify(`WBM: ${getAttachmentRuntime().wbmEnabled ? (getAttachmentRuntime().wbmDeep ? "DEEP" : "ON") : "OFF"}` +
                        `${getAttachmentRuntime().wbmNoCatalogue ? " (no-catalogue)" : ""}\n` +
                        `Catalogued: ${getAttachmentRuntime().cataloguedDecisions.length} decisions, ${getAttachmentRuntime().cataloguedFacts.length} facts`, "info");
                    break;
                case "deep":
                    getAttachmentRuntime().wbmEnabled = true;
                    ...
                    break;
                case "flush": {
                    const total = getAttachmentRuntime().cataloguedDecisions.length + getAttachmentRuntime().cataloguedFacts.length;
                    for (const d of getAttachmentRuntime().cataloguedDecisions) {
                        await wbExec(["memory", "queue", "--source", "pi_session", "--type", "decision", d], `${sbUrl}/v1/memory/queue`, { memory_text: d, source_type: "pi_session", confidence: 0.85, type: "decision" });
                    }
                    for (const f of getAttachmentRuntime().cataloguedFacts) {
                        await wbExec(["memory", "queue", "--source", "pi_session", "--type", "fact", f], `${sbUrl}/v1/memory/queue`, { memory_text: f, source_type: "pi_session", confidence: 0.85, type: "fact" });
                    }
                    getAttachmentRuntime().cataloguedDecisions = [];
                    getAttachmentRuntime().cataloguedFacts = [];
                ...
                case "decisions":
                    ...
                    ctx.ui.notify(getAttachmentRuntime()
                        .cataloguedDecisions.map((d, i) => `${i + 1}. ${d}`)
                        .join("\n"), "info");
                    break;
                case "ships": {
                    const r = await wbExec(["score", "ships", "--format", "json", "--limit", "5"], `${sbUrl}/v1/ships?limit=5`);
                    if (r?.ships?.length) {
                        ctx.ui.notify(r.ships.map((s) => `🚀 ${s.summary || s.commit?.slice(0, 8)}`).join("\n"), "info");
                    }
                    else {
