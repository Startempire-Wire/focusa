...
export type LifecycleAdvisoryInput = {
  ...
  advisoryKind:
    | "unbound_project"
    | "project_identity_bootstrap"
    | "project_scope_recovery"
    | "startup_receptionist";
  title: string;
  content: string;
  reason: string;
  projectRoot: string;
...
const scheduledReceptionistTurns = new Set<string>();

type ReceptionOperatorContext = {
...
function receptionistGreeting(operator?: ReceptionOperatorContext): string {
...
export function queueStartupReceptionistTurn(
  pi: LifecycleAdvisoryPi,
  ctx: any,
  input: LifecycleAdvisoryInput
): LifecycleAdvisoryStatus {
  ...
  if (scheduledReceptionistTurns.has(input.advisoryKey)) return "queued";
  ...
  const startWhenIdle = async () => {
    ...
    try {
      ...
      ctx?.ui?.setWidget?.(
        "focusa-vital",
        [`${resolvedGreeting} — I’m checking recent projects and preparing a few clear options…`],
