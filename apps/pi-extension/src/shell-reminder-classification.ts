export type ShellReminderClassification = {
  classification: "actual_focusa_bypass" | "shell_used";
  confidence: "high" | "uncertain";
  equivalent_tool: string | null;
  reason: string;
};
...
function equivalentForCommand(command: string): string | null {
  const routeMappings: Array<[RegExp, string]> = [
...
export function classifyShellReminderInteraction(
  toolName: unknown,
  args: unknown
): ShellReminderClassification {
