...
export type ProjectBindingState =
  "DISCOVER" | "RECONCILE" | "VERIFY" | "BOUND" | "RECOVERING" | "QUARANTINED";

export type ProjectBindingCapabilityTier = "scoped" | "recovery_read_plan" | "unbound_read_only";

export interface ProjectBindingCandidateV1 {
  project_root: string;
  active_worktree_root?: string;
  canonical_parent_root?: string;
  score: number;
  sources: string[];
  markers: string[];
  relationship?: string;
...
export interface ProjectBindingDecisionV1 {
  schema: "focusa.project_binding_decision.v1";
  decision_id: string;
  state: ProjectBindingState;
  selected_project_root?: string;
  selected_worktree_root?: string;
  canonical_parent_root?: string;
  continuity_id: string;
  candidates: ProjectBindingCandidateV1[];
  ...
  permitted_capability_tier: ProjectBindingCapabilityTier;
  recovery_packet_ref?: string;
...
export interface ReconcileProjectBindingInput {
  selectedProjectRoot?: string;
  selectedWorktreeRoot?: string;
  canonicalParentRoot?: string;
  continuityId?: string;
  candidates?: Array<Record<string, unknown>>;
  ambiguous?: boolean;
  selectedRootSafe?: boolean;
  verificationCanonical?: boolean;
  ...
  recoveryPacketRef?: string;
  operatorDecisionRef?: string;
  previousDecision?: ProjectBindingDecisionV1 | null;
  effectiveAt?: string;
}
...
function normalizedCandidates(values: Array<Record<string, unknown>>): ProjectBindingCandidateV1[] {
  return values
    .map((candidate) => ({
...
function confidenceFor(
  candidates: ProjectBindingCandidateV1[],
  state: ProjectBindingState
): ProjectBindingDecisionV1["confidence"] {
  if (state === "QUARANTINED" || !candidates.length) return "none";
  const score = candidates[0]?.score || 0;
  return score >= 900 ? "high" : score >= 700 ? "medium" : "low";
}

export function reconcileProjectBindingDecision(
  input: ReconcileProjectBindingInput
): ProjectBindingDecisionV1 {
  ...
  const capability: ProjectBindingCapabilityTier =
    state === "BOUND" ? "scoped" : state === "QUARANTINED" ? "unbound_read_only" : "recovery_read_plan";
  ...
  return {
    ...
    supersedes_decision_id:
      input.previousDecision && input.previousDecision.decision_id !== decisionId
        ? input.previousDecision.decision_id
        : undefined,
  };
}

export function canReuseFreshVerifiedBindingOffline(
  decision: ProjectBindingDecisionV1 | null | undefined,
  input: {
    selectedProjectRoot?: string;
    repoFingerprint?: string;
    projectFingerprint?: string;
  ...
  if (!decision || decision.state !== "BOUND") return false;
  const selectedRoot = normalizedRoot(input.selectedProjectRoot);
  if (!selectedRoot || selectedRoot !== normalizedRoot(decision.selected_project_root)) return false;
  const observedAt = Date.parse(decision.effective_at);
  const now = input.nowMs ?? Date.now();
  const maxAge = input.maxAgeMs ?? 15 * 60 * 1000;
  if (!Number.isFinite(observedAt) || now - observedAt < 0 || now - observedAt > maxAge) return false;
  const repoMatches =
    !!decision.repo_fingerprint &&
    !!input.repoFingerprint &&
    decision.repo_fingerprint === input.repoFingerprint;
  const projectMatches =
    !!decision.project_fingerprint &&
    !!input.projectFingerprint &&
    decision.project_fingerprint === input.projectFingerprint;
  return repoMatches || projectMatches;
}

export function projectBindingAllowsDurableWrites(
  decision: ProjectBindingDecisionV1 | null | undefined
): boolean {
  return !decision || decision.state === "BOUND";
}

export function shouldEmitProjectScopeRecoveryPacket(
  previous: ProjectBindingDecisionV1 | null | undefined,
  next: ProjectBindingDecisionV1
): boolean {
  if (next.state !== "RECOVERING" && next.state !== "QUARANTINED" && next.state !== "VERIFY") return false;
