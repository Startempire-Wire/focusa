...
export type FocusaOperationClass = "read" | "value_mutation" | "recovery" | "internal_maintenance";
...
function operationPolicyForContract(contract: FocusaToolContract): FocusaOperationPolicy {
  ...
  const operation_class: FocusaOperationClass = recovery ? "recovery" : read ? "read" : "value_mutation";
  const capability_family: FocusaCapabilityFamily = recovery
    ...
    : read
      ...
      : lowerName.includes("silent_sessions")
        ? "automation"
        : "base_focusa";
  const commercial_treatment: FocusaCommercialTreatment =
  ...
  return {
    ...
    recovery_allowance: recovery ? "account_recovery" : read ? "read_projection" : "none",
...
const SMS_TOOL_CONTRACTS: FocusaToolContract[] = (
  [
    [
      ...
      "communications.health.read",
      ["CommunicationsConnectorHealth", "EncryptedConnectorCheckpoint"],
      "GET /v1/sms/health",
      "focusa sms health",
      "read",
      "sms:health",
      "read_value_free_health",
      "read",
    ],
    [
      ...
      "communications.enrollment.read",
      ["CommunicationsEnrollment"],
      "GET /v1/sms/enrollment",
      "focusa sms enrollment",
      "read",
      "sms:enrollment",
      "read_value_free_enrollment",
      "read",
    ],
    [
      "focusa_sms_threads",
      ...
      ["CommunicationsThread", "ScopedCapabilityGrant"],
      "GET /v1/sms/threads",
      "focusa sms threads",
      "read",
      "sms:list_threads",
      "authorized_customer_data_read",
      "read",
    ],
    [
      ...
      ["CommunicationsThread", "CommunicationsMessage", "ScopedCapabilityGrant"],
      "GET /v1/sms/threads/{thread}/messages",
      "focusa sms read <thread-handle>",
      "read",
      "sms:read_thread",
      "authorized_customer_data_read",
      "read",
    ...
    [
      ...
      ["CommunicationsMessage", "ScopedCapabilityGrant"],
      "GET /v1/sms/search",
      "focusa sms search <query>",
      "read",
      "sms:search",
      "authorized_customer_data_read",
      "read",
    ...
    [
      ...
      ["CommunicationsDelivery", "ScopedCapabilityGrant"],
      "POST /v1/sms/send",
      "focusa sms send <recipient-handle> --confirm",
      "write",
      "sms:send",
      "confirmed_idempotent_message_delivery",
      "value_mutation",
    ],
    [
      "focusa_sms_otp_challenge",
      "Register SMS OTP Challenge",
      "Register an exact provider and target challenge before OTP delivery.",
      "communications.otp_challenge.register",
      ["OtpChallenge", "ScopedCapabilityGrant"],
      "POST /v1/sms/otp/challenges",
      "focusa sms otp-challenge",
      "write",
      "sms:otp_challenge",
      "bounded_challenge_registration",
      "value_mutation",
    ],
    [
      ...
      "communications.otp.inject",
      ["OtpChallenge", "TargetInjectionReceipt"],
      "POST /v1/sms/otp/inject",
      "focusa sms otp-inject",
      "control",
      "sms:inject_otp",
      "single_use_secret_injection",
      "value_mutation",
    ],
    [
      "focusa_sms_checkpoint",
      "Checkpoint SMS Connector",
      "Create and verify an encrypted atomic connector checkpoint with value-free receipt metadata.",
      "communications.connector.checkpoint",
      ["EncryptedConnectorCheckpoint"],
      "POST /v1/sms/checkpoint",
      "focusa sms checkpoint",
      "control",
      "sms:checkpoint",
      "confirmed_encrypted_checkpoint",
      "value_mutation",
    ],
    [
      "focusa_sms_events",
      ...
      "communications.events.read",
      ["CommunicationsAuditEvent"],
      "GET /v1/sms/events",
      "focusa sms events",
      "read",
      "sms:events",
      "read_value_free_audit_events",
      "read",
    ],
    [
      "focusa_sms_revoke",
      "Revoke SMS Connector",
      "Revoke one customer-owned connector and all associated grants with explicit confirmation.",
      "communications.connector.revoke",
      ["CommunicationsConnector", "ScopedCapabilityGrant"],
      "POST /v1/sms/revoke",
      "focusa sms revoke --confirm",
      "control",
      "sms:revoke",
      "confirmed_connector_and_grant_revocation",
      "recovery",
    ],
  ] as const satisfies readonly SmsToolDescriptor[]
).map(
  ([
    name,
    label,
    ...
    return {
      ...
      spec_path:
        "docs/156-focusa-project-scoped-credential-authority-secret-broker-delegated-autonomy-mfa-totp-and-cross-surface-injection-spec.md",
      result_envelope: "tool_result_v1",
      side_effect_profile: sideEffect,
      parity_status: "full",
      exemptions: [],
...
export const FOCUSA_TOOL_CONTRACTS: FocusaToolContract[] = (
  [
    ...
    },
    {
      ...
      purpose: "validating a CallGraph definition deterministically before any create or dispatch action.",
      family: "callgraph",
...
const TOOL_NEXT_TOOLS: Record<string, string[]> = {
  ...
  focusa_daemon_routing_status: ["focusa_project_identity", "focusa_tool_doctor", "focusa_workpoint_resume"],
...
const FAMILY_DEFAULT_INPUTS: Record<FocusaToolFamily, string[]> = {
  ...
  communications: [
    "separately authorized capability grant",
    "opaque connector/thread/target handles",
    "confirmation and idempotency for mutations",
  ],
  agent_runtime: [
