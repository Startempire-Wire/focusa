...
export function normalizeCacheUsage(rawUsage) {
    ...
    const cacheWriteTokens = Number(usage.cacheCreationInputTokens || usage.cacheWrite || usage.cache_write || 0);
    return {
...
function textContentContains(content, marker) {
    if (typeof content === "string")
        return content.includes(marker);
    return (Array.isArray(content) &&
        content.some((part) => part?.type === "text" && String(part.text || "").includes(marker)));
}
/** Attach volatile Focusa context to the newest user turn, after cacheable history. */
export function attachFocusSliceToNewestUser(messages, slice) {
    ...
    if (textContentContains(current?.content, marker))
        return next;
...
export function buildCachePrefixSnapshot(systemPrompt, messages, dynamicSlice, capturedAt = Date.now(), sessionKey = "no-session") {
...
function preservesHistoricalPrefix(previous, current) {
...
export class CacheSafetyMonitor {
    ...
    captureSystemPrompt(sessionKey, systemPrompt) {
        const state = this.state(sessionKey);
        state.systemPrompt = systemPrompt;
        return hash(systemPrompt);
    }
    captureRequest(sessionKey, messages, dynamicSlice) {
        const state = this.state(sessionKey);
        const snapshot = buildCachePrefixSnapshot(state.systemPrompt, messages, dynamicSlice, Date.now(), sessionKey);
        state.previous = state.current;
        state.current = snapshot;
        return snapshot;
    }
    isDegraded(sessionKey) {
        return this.state(sessionKey).degraded;
    }
    resetForDiscontinuity(sessionKey) {
        const state = this.state(sessionKey);
    ...
    observeUsage(observation) {
        ...
        const idleDurationMs = state.lastObservedAt === undefined ? null : Math.max(0, now - state.lastObservedAt);
        const miss = totalPromptTokens >= LARGE_PROMPT_TOKENS && cacheReadRatio < MISS_READ_RATIO;
        const modelChanged = Boolean(state.lastModel) &&
        ...
        const systemChanged = Boolean(state.previous) && state.previous?.stableSystemPrefixHash !== current.stableSystemPrefixHash;
        const historyShortened = Boolean(state.previous) &&
            current.historyMessageHashes.length < (state.previous?.historyMessageHashes.length || 0);
        const historyShifted = Boolean(state.previous) &&
            !preservesHistoricalPrefix(state.previous?.historyMessageHashes || [], current.historyMessageHashes);
        const cacheReadPlateau = state.lastCacheReadTokens !== undefined &&
            cacheReadTokens > 0 &&
            Math.abs(cacheReadTokens - state.lastCacheReadTokens) <=
                Math.max(256, Math.floor(state.lastCacheReadTokens * 0.05));
        let reason = "cache_hit";
        if (miss) {
            if (modelChanged)
            ...
            else if (historyShortened)
                reason = "compaction_or_branch_reset";
            else if (historyShifted)
                reason = "historical_prefix_shifted";
            else if (cacheReadTokens === 0)
                reason = "provider_cache_unavailable";
            else
                reason = "unknown_provider_miss";
        }
        const qualifyingMiss = miss &&
            !modelChanged &&
            !ttlExpired &&
            (reason === "stable_system_prefix_changed" ||
                reason === "historical_prefix_shifted" ||
        ...
        const structuralPrefixChange = reason === "stable_system_prefix_changed" || reason === "historical_prefix_shifted";
        const transitionedToDegraded = !state.degraded &&
