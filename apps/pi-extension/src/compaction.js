...
import { buildProjectWorkstreamKey, scopedQueryParams, verifiedScopeRefForRoot, } from "./scoped-state.js";
import { getAttachmentRuntime, focusaFetch, buildCompactInstructions, persistState, sanitizeFocusFailures, getScopedWorkpointPacket, isWorkpointPacketScopedToCurrentSession, isProjectRootAuthoritySafe, normalizeWorkpointResumePacketEnvelope, normalizeProjectRoot, refreshTrajectoryClarityLifecycle, stampWorkpointPacketForCurrentPiSession, isExplicitContinuationAsk, isNonTaskStatusLikeText, buildAttentionRecallVerdict, formatAttentionRecallFocusSliceLines, toolOutputVisibleRecapReason, formatToolOutputVisibleRecapLines, formatProjectSwitchLedgerLines, buildCurrentAskScopeVerdict, formatCurrentAskScopeVerdictLines, getTurnCount, getActiveFrameId, getContinuityId, getSessionFrameKey, getSessionCwd, getActiveWorkpointPacket, setActiveWorkpointPacket, getActiveWorkpointSummary, setActiveWorkpointSummary, getLastTrajectoryClarity, setLastTrajectoryClarity, getTotalCompactions, incrementTotalCompactions, } from "./state.js";
...
function normalizeCompactionArtifacts(files) {
    return (Array.isArray(files) ? files : [])
        .map((file) => String(file || "").trim())
        .filter(Boolean)
        .slice(0, 20)
        .map((file) => ({ kind: "file", label: basename(file), path_or_id: file }));
}
function compactLines(values, mapper) {
...
function compactionMemorySample() {
...
function scheduleCompactionMemoryEvaluation() {
    const before = getAttachmentRuntime().compactionMemoryBefore;
    if (!before)
        return;
    setTimeout(() => {
        ...
        getAttachmentRuntime().lastCompactionMemory = {
        ...
        delete getAttachmentRuntime().compactionMemoryBefore;
        persistState();
        if (retainedUnderPressure) {
            console.warn(`[focusa] compaction retained ${Math.round(after.rssBytes / 1024 / 1024)} MiB RSS; checkpoint and start a bounded fresh Pi session before host OOM`);
        }
    }, 5_000);
}
function compactText(value, fallback = "unknown", max = 180) {
...
function currentCompactionScope() {
    ...
    if (!verifiedScopeRefForRoot(projectRoot))
        return null;
    return buildProjectWorkstreamKey(projectRoot, continuityId);
}
async function buildLearningCompactionCard(currentAsk, mission, nextSlice) {
    ...
    try {
        ...
        if (candidates.length) {
            candidates
                .slice(0, 3)
                .forEach((c, i) => lines.push(`- Lesson ${i + 1}: ${compactText(c.content || c.kind || c.capture_id, "signal", 180)}`));
        }
        else {
...
function semanticCurrentAsk() {
    const text = String(getAttachmentRuntime().currentAsk?.text || "").trim();
    if (!text || isExplicitContinuationAsk(text) || isNonTaskStatusLikeText(text))
        return "";
    return text;
}
function renderResumeProjection(packet) {
    return renderCompactionResumeProjection(packet, projectionPressure(packet, getAttachmentRuntime().currentTier));
}
async function buildCompactionMissionPacket(resumeSource) {
    if (!getAttachmentRuntime().focusaAvailable)
        return null;
    const scope = currentCompactionScope();
    if (!scope)
    ...
    try {
        ...
        if (packet?.schema_version !== "focusa.compaction_mission_packet.v1")
            return null;
        getAttachmentRuntime().lastCompactionMissionPacket = packet;
        return packet;
    }
    catch {
        return null;
...
async function buildCompactionFallbackSummary(fs, workpointPacket) {
    ...
    const mission = packetField(packet, "mission") ||
        ask ||
    ...
    const nextSlice = packetField(packet, "next_slice") ||
        getAttachmentRuntime().lastFocusSnapshot.currentFocus ||
        getAttachmentRuntime().lastCompactDecision ||
        ask ||
        getAttachmentRuntime().activeFrameGoal;
    const currentFocus = fs?.current_focus ||
        fs?.current_state ||
        getAttachmentRuntime().lastFocusSnapshot.currentFocus ||
        nextSlice ||
        mission;
    ...
    if (nextSlice)
        nextSteps.unshift(`Continue from Workpoint next_slice: ${nextSlice}`);
    ...
    if (!decisions.length && mission)
        decisions.push(`Continuation anchored to mission: ${mission}`);
    ...
    if (!openQuestions.length)
        openQuestions.push("No open questions recorded by Focusa or Workpoint.");
    if (!blockers.length)
        blockers.push("No open blockers recorded by Focusa or Workpoint.");
    if (!failures.length)
        failures.push("No active failure records in Focusa state.");
    if (!notes.length && rendered)
        notes.push(`Workpoint summary: ${rendered}`);
    ...
    const attentionSection = [
        ...
        ...formatCurrentAskScopeVerdictLines(buildCurrentAskScopeVerdict({
            currentAskText: ask,
            workpointPacket: packet,
            projectRoot: getSessionCwd(),
            continuityId: getContinuityId(),
...
async function refreshWorkpointResumePacket(mode = "compact_prompt") {
    if (!getAttachmentRuntime().focusaAvailable)
        return null;
    const scope = currentCompactionScope();
...
async function checkpointTrajectoryBeforeCompaction(reason = "before_compaction") {
    if (!getAttachmentRuntime().focusaAvailable)
        return null;
    const scope = currentCompactionScope();
    if (!scope)
...
async function refreshTrajectoryResumePacket(reason = "compaction") {
    if (!getAttachmentRuntime().focusaAvailable)
        return null;
    const scope = currentCompactionScope();
    if (!scope)
    ...
    try {
        ...
        setLastTrajectoryClarity({
            ...
            status: String(view?.intelligence_view?.clarity_gate?.status ||
                trajectory.definition_status ||
                packet?.status ||
                "unknown"),
            recommended_action: String(view?.intelligence_view?.clarity_gate?.recommended_action ||
            ...
            long_term_goal: trajectory.long_term_goal ||
            ...
            mid_level_goal: trajectory.mid_level_goal ||
                trajectory.trajectory_ladder?.mlg ||
                getLastTrajectoryClarity()?.mid_level_goal ||
                null,
            short_term_goal: trajectory.short_term_goal ||
...
function formatTrajectoryPacketForPrompt(packet) {
    ...
    const waypoints = Array.isArray(trajectory.waypoints || packet?.waypoints)
        ? (trajectory.waypoints || packet?.waypoints)
            .slice(0, 5)
            .map((item) => compactText(typeof item === "string" ? item : item?.title || item?.desired_state_delta, "", 100))
            .filter(Boolean)
        : [];
    if (hlt === "missing" && mlg === "missing" && stg === "missing" && desired === "missing")
        return "";
...
function recordLocalWorkpointFallback(reason) {
    const fallback = {
        status: "partial",
        canonical: false,
        reason,
        mission: semanticCurrentAsk() ||
            getAttachmentRuntime().activeFrameGoal ||
            getAttachmentRuntime().lastFocusSnapshot.intent ||
            "unknown mission",
...
async function checkpointBeforeCompaction() {
    if (!getAttachmentRuntime().focusaAvailable)
        return null;
    const scope = currentCompactionScope();
    if (!scope)
...
export async function prepareCompactionRollover() {
    const scope = currentCompactionScope();
    if (!scope) {
        return {
            ready: false,
...
export function isFocusaContextContinuityHealthy() {
    const scope = currentCompactionScope();
    if (!scope)
        return false;
    const continuityId = scope.continuity_id;
    ...
    return Boolean(getAttachmentRuntime().focusaAvailable && String(continuityId || "").trim() && noDegradedWorkpoint);
}
export function classifyBloatgaurdPressureAction(pct, cfg, canCompact) {
    if (pct >= cfg.hardPct)
        return "hard";
    if (pct >= cfg.compactPct && canCompact)
        return "auto";
    if (pct >= cfg.warnPct)
        return "warn";
    return "none";
}
export function resetLiveContextPressureAfterCompaction(now = Date.now()) {
...
export function contextPressureWarningCopy(kind, pct, totalCompactions = getTotalCompactions()) {
...
export function contextTierLabel(tier) {
    if (tier === "warn")
        return "monitor";
    if (tier === "auto")
        return "compacting";
...
function setContextStatus(ctx, tier, pct, focusaContinuityReady = isFocusaContextContinuityHealthy()) {
...
function formatResumePacketV2ForPrompt(packet) {
    const v2 = packet?.resume_packet_v2 ||
    ...
    if (!isProjectRootAuthoritySafe(packetProjectRoot))
        return "";
    if (!packetContinuityId || (getContinuityId() && packetContinuityId !== getContinuityId()))
        return "";
    if (v2.canonical === false)
        return "";
    ...
    return [
        ...
        ...(bestNext.length
            ? bestNext
            : ["focusa_workpoint_resume", "focusa_trajectory_view", "focusa_traverse", "focusa_tool_doctor"]).map((tool) => `  - ${tool}`),
        "DO_NOT_USE_BY_DEFAULT:",
        ...(doNotUse.length
            ? doNotUse
            : ["transcript tail as authority", "full lineage tree", "full ontology graph", "deep work-loop status"]).map((item) => `  - ${item}`),
        "RECOVERY_ORDER:",
...
let activeCompactionEpoch = null;
async function withCompactionDeadline(signal, operation, timeoutMs = 2_500) {
...
function compactionEpochKey(event) {
    const entry = event?.compactionEntry || {};
    const ordinal = getTotalCompactions() || entry.details?.totalCompactions || "unknown";
    return String(entry.id || entry.uuid || entry.timestamp || `${getSessionFrameKey() || "session"}:compact:${ordinal}`);
}
async function prepareCompactionEpoch(event) {
    const scope = currentCompactionScope();
    if (!scope || !getAttachmentRuntime().focusaAvailable)
        return null;
    const epochKey = compactionEpochKey(event);
    const result = await withCompactionDeadline(event?.signal, (signal) => focusaFetch("/compaction/prepare", {
        method: "POST",
        signal,
        body: JSON.stringify({
    ...
    if (!result?.epoch_id || !["prepared", "degraded"].includes(String(result.status)))
        return null;
...
async function runPostCompactionVerification(event, ctx) {
    const runtime = getAttachmentRuntime();
    ...
    try {
        ...
        const entry = event?.compactionEntry || {};
        const verification = await withCompactionDeadline(undefined, (signal) => focusaFetch("/compaction/verify", {
            method: "POST",
            signal,
            body: JSON.stringify({
                ...
                native_compaction_result: {
                    ...
                    modified_files: normalizeCompactionArtifacts(entry.details?.modifiedFiles || entry.details?.fileOps || []),
        ...
        if (runtime.compactResumeDeliveryState === "superseded_by_operator")
            return;
        runtime.compactResumePending = true;
        const projection = epoch.prepare.resume_projection;
        const liveScope = currentCompactionScope();
        const resumeText = projection && liveScope && canInjectCompactionMission(projection, liveScope)
        ...
        if (ctx.hasUI) {
            ctx.ui.notify("Focusa verification degraded; Pi compaction and continuation remain available.", "warning");
        }
    }
    finally {
        ...
        if (liveScope?.root_scope?.root_path) {
            await refreshTrajectoryClarityLifecycle("post_compaction", String(liveScope.root_scope.root_path)).catch(() => null);
        }
        persistState();
        updateNorthStarCard(ctx, "post_compaction");
    }
}
export function registerCompaction(pi) {
    // ── session_before_compact (§33.1 ASCC replacement, §33.10 fallback) ───────
    pi.on("session_before_compact", async (event, ctx) => {
        getAttachmentRuntime().compactionMemoryBefore = compactionMemorySample();
        try {
    ...
    pi.on("session_compact", (event, ctx) => {
        ...
        const entry = event.compactionEntry || {};
        const ordinal = getTotalCompactions() || entry.details?.totalCompactions || "unknown";
        const epochKey = String(entry.id || entry.uuid || entry.timestamp || `${getSessionFrameKey() || "session"}:compact:${ordinal}`);
        if (runtime.compactionVerifyPendingKey === epochKey)
            return;
        ...
        setTimeout(() => {
            void runPostCompactionVerification(event, ctx).catch((error) => {
                ...
                if (ctx.hasUI) {
                    ctx.ui.notify("Focusa post-compaction verification degraded; Pi continuation remains available.", "warning");
                }
            });
        }, 0);
    });
...
export async function checkCompactionTier(ctx) {
    ...
    if (pressureAction === "hard") {
        ...
        const requestResult = requestCoordinatedCompaction(ctx, {
            ...
            customInstructions: buildCompactInstructions("HARD COMPACT: preserve canonical Focusa authority and release live Pi context."),
            onComplete: onDone,
        ...
        const requestResult = requestCoordinatedCompaction(ctx, {
            ...
            customInstructions: buildCompactInstructions("Compact live Pi context while preserving canonical Focusa authority."),
            onComplete: onDone,
            onError: (error) => ctx.ui.notify(`Compaction failed: ${error.message}`, "error"),
        });
        if (requestResult === "coordinator_unavailable") {
...
export async function checkMicroCompact() {
    ...
    if (legacyCadence > 0 && !legacyMicroCompactDiagnosticEmitted) {
        ...
        console.warn(`[focusa] microCompactEveryNTurns=${legacyCadence} is retained for compatibility but automatic fixed-cadence compaction is disabled; use pressure policy or the manual micro-compact command.`);
    }
}
