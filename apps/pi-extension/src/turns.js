...
import { getAttachmentRuntime, currentProjectBindingDecision, nativeSessionAllowsNonessentialPersistence, focusaFetch, focusaPost, compatibleWorkLoopStatusState, extractText, getFocusState, getCachedFocusState, getEffectiveFocusSnapshot, estimateTokens, wbExec, storeEcsArtifact, classifyCurrentAsk, deriveQueryScope, isOperatorSteeringInput, selectRelevantItems, selectRelevantRankedItems, shouldIncludeMissionContext, buildSliceSection, selectionRelevanceScore, retentionBucketsFromSelection, formatWorkingSetItems, formatVerifiedDeltaItems, buildCanonicalReferenceAliases, orderSliceSections, rescopePiFrameFromCurrentAsk, stripQuotedFocusaContext, detectForbiddenVisibleOutputLeakClasses, detectScopeFailureSignals, getCachedSemanticMemorySummary, getCachedEcsHandlesSummary, getCachedOntologyContext, getScopedWorkpointPacket, ensureContinuityId, isProjectRootAuthoritySafe, isWorkpointPacketScopedToCurrentSession, refreshTrajectoryClarityLifecycle, refreshOntologyContextLifecycle, stampWorkpointPacketForCurrentPiSession, adoptPiProjectRoot, persistState, projectRootConfirmationRequired, projectRootConfirmationSummary, buildAttentionRecallVerdict, formatAttentionRecallFocusSliceLines, maybeCaptureReportSummaryFromAssistantOutput, recordToolOutputPressure, toolOutputVisibleRecapReason, formatToolOutputVisibleRecapLines, markVisibleRecapEmittedIfPresent, observeProjectThreadHintsFromText, formatProjectSwitchLedgerLines, buildCurrentAskScopeVerdict, formatCurrentAskScopeVerdictLines, getActiveWorkpointPacket, setActiveWorkpointPacket, getActiveWorkpointSummary, setActiveWorkpointSummary, getLastTrajectoryClarity, getLastProjectVerify, getCurrentScopeStore, getLatestReportSummary, setLastStreamLen, resetToolUsageBatch, getToolUsageBatch, pushToToolUsageBatch, setLongSessionSignaled, getLongSessionSignaled, setCurrentTaskTurnStart, getLastStreamLen, pushCompilationError, getCompilationErrors, incrementFileEditCount, getFileEditCounts, getTurnCount, incrementTurnCount, getSessionCwd, getContinuityId, pushRecentTurn, formatRecentTurnsSection, shouldEmitRecentTurnsSlice, markRecentTurnsSliceEmitted, } from "./state.js";
...
import { attachFocusSliceToNewestUser, CacheSafetyMonitor, normalizeCacheUsage, } from "./cache-safe-context.js";
...
async function turnWorkLoopWriterHeaders() {
...
function shouldIncludeTurnInSlice(text, toolCallCount) {
    // Drop status-only turns (test/cont/ack/...) and tool-empty turns when non-task text exists.
    if (isNonTaskStatusLikeText(text || ""))
        return false;
    if (toolCallCount === 0 && (!text || text.trim().length === 0))
        return false;
    return true;
}
function deriveOutcome(text, hasFailureSignal) {
    if (hasFailureSignal)
        return "blocked";
...
function captureRecentTurnSlice(assistantOutput) {
    try {
        const turnCount = getTurnCount();
        const toolCount = getAttachmentRuntime().currentTaskToolCalls ?? 0;
        const text = assistantOutput || "";
        if (!shouldIncludeTurnInSlice(text, toolCount))
            return;
        const slice = {
            turn_id: `pi-turn-${turnCount}`,
            mission_at_turn: (getAttachmentRuntime().activeFrameGoal ||
...
async function fetchRecentTurnsFromDaemon(n) {
    if (!getAttachmentRuntime().focusaAvailable)
        return [];
    try {
        ...
        if (!continuity)
            return [];
        const res = await focusaFetch(`/v1/turns/recent?n=${n}&continuity_id=${encodeURIComponent(continuity)}`);
        const value = res?.value;
        if (!value || Array.isArray(value?.error))
            return [];
...
async function buildRecentTurnsSlice(n = 4) {
    const currentTurn = getTurnCount();
    if (!shouldEmitRecentTurnsSlice(currentTurn))
        return "";
...
function buildCachedRecentTurnsSlice(n = 4) {
    const currentTurn = getTurnCount();
    if (!shouldEmitRecentTurnsSlice(currentTurn))
        return "";
    const localSection = formatRecentTurnsSection(n);
...
const RECALL_INTENT_PATTERNS = [
    ...
    },
    {
        ...
        pattern: /\b(what did we|earlier|last time|previously|where were we|as we discussed|we talked about|you mentioned|you said|i asked|i said|i meant|didn't we)\b/i,
    },
    {
...
function detectRecallIntent(text) {
    ...
    for (const { category, pattern } of RECALL_INTENT_PATTERNS) {
        ...
        if (m)
            return { matched_category: category, matched_phrase: m[0] };
    }
    return null;
}
function isNonTaskStatusLikeTextLocal(text) {
    // Delegate to the canonical helper imported in the wider scope if available; otherwise re-implement narrowly.
    try {
        return require("./state.js").isNonTaskStatusLikeText?.(text) ?? false;
    }
    catch {
...
function isNonTaskStatusLikeText(text) {
    return isNonTaskStatusLikeTextLocal(text);
}
const traceBatch = [];
function vitalPromptSurfaceEnabled(surface) {
    const raw = String(getAttachmentRuntime().cfg?.vitalInfoPromptSurfaces || "project_root,project_verify,workpoint,trajectory");
    return raw
        .split(",")
        .map((part) => part.trim())
        .includes(surface);
}
function hardGateVitalProjectRoot(ctx) {
    if (!getAttachmentRuntime().focusaAvailable || !vitalPromptSurfaceEnabled("project_root"))
        return null;
...
function predictionAuthorityProjectionPath(verified, continuityId) {
    ...
    if (fields.some((field) => !String(scope?.[field] || "").trim()))
        return undefined;
    const query = new URLSearchParams();
    for (const field of fields)
        query.set(field, String(scope[field]));
    query.set("continuity_id", continuity);
    return `/prediction-authority/projection?${query.toString()}`;
}
function queueTraceTelemetry(event) {
    traceBatch.push(event);
}
function flushTraceTelemetryBatch(reason = "turn_end") {
...
async function checkpointDiscontinuity(reason, extra = {}) {
    if (!getAttachmentRuntime().focusaAvailable)
        return;
    ...
    try {
        await focusaFetch("/workpoint/checkpoint", {
            method: "POST",
            body: JSON.stringify({
                mission: getAttachmentRuntime().currentAsk?.text ||
                    getAttachmentRuntime().activeFrameGoal ||
                    getAttachmentRuntime().lastFocusSnapshot.intent ||
                    "Pi discontinuity boundary",
                next_slice: getAttachmentRuntime().lastFocusSnapshot.currentFocus ||
...
function formatWorkpointContextSections() {
    ...
    const driftBoundaries = String(packet.next_slice || "")
        ...
        .filter(Boolean);
    const activeObjects = Array.isArray(packet.active_object_refs) && packet.active_object_refs.length
        ? packet.active_object_refs
        : ["(none)"];
    const verificationHooks = [
        ...(Array.isArray(action.verification_hooks) ? action.verification_hooks : []),
        ...evidence.map((v) => v.result || v.evidence_ref || v.target_ref).filter(Boolean),
    ].slice(0, 8);
    ...
    return [
        ...
        `ACTIVE_OBJECT_SET:\n${activeObjects.map((x) => `  - ${x}`).join("\n")}`,
        `ACTION_INTENT: ${action.action_type || "unknown"}${action.target_ref ? ` -> ${action.target_ref}` : ""}`,
        `VERIFICATION_HOOKS:\n${(verificationHooks.length ? verificationHooks : ["(none)"]).map((x) => `  - ${x}`).join("\n")}`,
        `DRIFT_BOUNDARIES:\n${boundaryItems.map((x) => `  - ${x}`).join("\n")}`,
    ];
}
function boundedTrajectoryText(value, max = 180) {
...
function trajectoryWaypointTitle(value) {
...
function currentVerifiedTrajectoryScope() {
    ...
    if (!store || store.identity.scopeKind !== "project")
        return null;
...
function handleTrajectoryMatchesCurrentScope(handle) {
    ...
    const candidateRoot = String(scope.project_root || trajectory.project_root || handle?.project_root || "")
        .trim()
        .replace(/\/+$/, "");
    const candidateContinuity = String(scope.continuity_id || trajectory.continuity_id || handle?.continuity_id || "").trim();
    if (candidateRoot !== current.projectRoot || candidateContinuity !== current.continuityId)
        return false;
...
function existingProjectDirs(root, dirs) {
    return dirs.filter((dir) => safeExists(root, dir)).slice(0, 8);
}
function safeRead(root, rel, maxBytes = 4096) {
    try {
...
function manifestName(root) {
    ...
    if (cargoName)
        return `cargo=${cargoName}`;
    return "manifest=unknown";
}
function buildProjectArchitectureDigestLine(root) {
    const evidenceRefs = [];
    const mark = (rel, label = rel) => {
        const exists = safeExists(root, rel);
        if (exists)
            evidenceRefs.push(label);
        return exists;
    };
    ...
    keyDirs.forEach((dir) => {
        if (!evidenceRefs.includes(dir))
            evidenceRefs.push(dir);
    });
    const deploy = [
    ...
    const confidence = stacks.length && docs.length && tests.length
        ? "high"
...
function formatTrajectoryFallbackFocusSlice(root, reason) {
...
function trajectoryViewMatchesCurrentScope(view) {
    const current = currentVerifiedTrajectoryScope();
    if (!current || !view || typeof view !== "object")
        return false;
...
function formatTrajectoryFocusSlice(view) {
    if (!trajectoryViewMatchesCurrentScope(view))
        return [];
    const project = view.project_identity || {};
    const trajectory = view.trajectory || {};
    ...
    const beadsPrefix = boundedTrajectoryText(workingSubpath.beads_prefix || project.beads_prefix || projectApi.beads_prefix, 40);
    ...
    const waypoints = Array.isArray(trajectory.waypoints)
        ? trajectory.waypoints
            .slice(0, 5)
            .map((item) => boundedTrajectoryText(trajectoryWaypointTitle(item), 120))
            .filter(Boolean)
    ...
    const evidence = Array.isArray(trajectory.evidence_refs)
        ? trajectory.evidence_refs
            .slice(0, 4)
            .map((item) => boundedTrajectoryText(item.evidence_ref || item.result || item.target_ref, 120))
            .filter(Boolean)
        : [];
    const doNotUse = Array.isArray(intelligence.do_not_use)
        ? intelligence.do_not_use
            .slice(0, 6)
            .map((item) => boundedTrajectoryText(item, 100))
            .filter(Boolean)
        : [];
    const missingFacts = Array.isArray(sufficiency.missing_facts)
        ? sufficiency.missing_facts
            .slice(0, 6)
            .map((item) => boundedTrajectoryText(trajectoryWaypointTitle(item), 80))
            .filter(Boolean)
        : [];
    const candidateBits = [
...
async function getResourceModeFocusSliceLines() {
    if (!getAttachmentRuntime().focusaAvailable)
        return [];
    try {
...
function currentAskLooksLikeWebResearch(text) {
    const value = String(text || "").toLowerCase();
    return (/https?:\/\//.test(value) ||
        /\b(url|website|webpage|browser|browse|research|docs?|article|blog|github\.com)\b/.test(value));
}
function getUiaiFirstFocusSliceLines(askText) {
    if (!currentAskLooksLikeWebResearch(askText))
        return [];
    return [
...
function getCachedTrajectoryFocusSliceLines() {
    const scope = currentVerifiedTrajectoryScope();
    if (!scope || !isProjectRootAuthoritySafe(scope.projectRoot))
        return [];
    return formatTrajectoryFallbackFocusSlice(scope.projectRoot, "prompt_hot_path_cache");
}
async function getTrajectoryFocusSliceLines() {
    const scope = currentVerifiedTrajectoryScope();
    if (!scope || !isProjectRootAuthoritySafe(scope.projectRoot))
        return [];
...
function providerStatusSuggestsContextOverflow(status, headers = {}) {
    ...
    const joined = Object.entries(headers)
        ...
        .join(" ")
        .toLowerCase();
    return (/context[_ -]?length|token|too large|payload|maximum context|input exceeds/.test(joined) ||
        joined.length === 0);
}
function textSuggestsContextOverflow(text) {
...
export function elideOldRehydratableToolHistory(messages, recentMessageWindow = BLOATGAURD_RECENT_TOOL_HISTORY_MESSAGES) {
    const safeWindow = Math.max(0, Math.floor(recentMessageWindow));
    const recentStart = Math.max(0, messages.length - safeWindow);
    return messages.map((message, index) => {
        if (index >= recentStart || message?.role !== "toolResult" || message?.isError)
            return message;
...
function attachCacheSafeFocusSlice(_event, messages, text) {
...
function emitCacheSafetyObservation(observation, ctx) {
    queueTraceTelemetry({
    ...
    if (observation.transitionedToDegraded &&
        getAttachmentRuntime().cfg?.cacheSafePromptLayoutEnabled !== false) {
        ctx.ui?.notify?.("Focusa cache-safe degraded mode: repeated same-model prefix misses; optional context is temporarily suppressed.", "warning");
    }
}
export function registerTurns(pi) {
    ...
    pi.on("before_agent_start", (event, ctx) => {
        ...
        cacheSafetyMonitor.captureSystemPrompt(cacheSessionKey(), event.systemPrompt);
        if (!getAttachmentRuntime().seenFirstBeforeAgentStart) {
            ...
            if (nativeSessionAllowsNonessentialPersistence()) {
                ...
                void buildAwarenessPacket("reload")
                    ...
                    try {
                        pi.sendMessage({ customType: "focusa-utility-card", content: visibleCard, display: true }, { triggerTurn: false });
                    }
                    finally {
    ...
    pi.on("context", (event, ctx) => {
        ...
        const startupReceptionistTurn = contextMessages.slice(-8).some((message) => {
            if (message?.customType === "focusa-startup-receptionist" ||
                message?.custom_type === "focusa-startup-receptionist" ||
                message?.details?.schema === "focusa.pi_startup_receptionist.v1")
                return true;
            try {
                const encoded = JSON.stringify(message);
                return (encoded.includes("focusa-startup-receptionist") ||
        ...
        const [recentTurnsContext, wbmContext] = !cacheSafeLayoutEnabled || cacheSafeDegraded ? ["", ""] : [buildCachedRecentTurnsSlice(4), ""];
        ...
        const maxTokens = (getAttachmentRuntime().currentContextPct || 0) >= 85
            ? 250
        ...
        const artifactLabels = fs.artifacts?.map((a) => `${a.kind}:${a.label}${a.path_or_id ? "@" + a.path_or_id : ""}`) || [];
        ...
        const ontologyContext = getCachedOntologyContext();
        ...
        const ontologyUncertaintyLines = Array.isArray(ontologyPayload?.uncertainty_flags)
            ? ontologyPayload.uncertainty_flags.slice(0, 6).map((item) => String(item))
            : [];
        const trajectoryLines = getCachedTrajectoryFocusSliceLines();
        const resourceModeLines = [];
        ...
        const sectionEntries = [
            ...
            },
            {
                ...
                text: (() => {
                    const temporal = getAttachmentRuntime().temporalPriorityContext;
                    if (!temporal)
            ...
            },
            {
                ...
                text: (() => {
                    const authority = getAttachmentRuntime().predictionAuthorityContext;
                    if (!authority)
                    ...
                    const count = (key) => Object.keys(projection[key] || {}).length;
            ...
            },
            {
                ...
                text: (() => {
                    const integrity = getAttachmentRuntime().instructionIntegrityContext;
                    if (!integrity)
            ...
            buildSliceSection("current_ask_scope_verdict", "CURRENT_ASK_SCOPE_VERDICT", formatCurrentAskScopeVerdictLines(buildCurrentAskScopeVerdict({
                currentAskText: askText,
                workpointPacket: getScopedWorkpointPacket(),
                projectRoot: getSessionCwd(),
                continuityId: getContinuityId(),
            })), true, (values) => values.join("\n"), 0, 4, 100),
            buildSliceSection("project_switch_ledger", "PROJECT_SWITCH_LEDGER", formatProjectSwitchLedgerLines(askText), getAttachmentRuntime().projectSwitchLedger.length > 0, (values) => `PROJECT_SWITCH_LEDGER:\n${values.map((value) => `  - ${value}`).join("\n")}`, 0, 5, 96),
            ...
            buildSliceSection("decayed_context", "DECAYED_CONTEXT", decayedContextItems, (scopeKind === "mission_carryover" || scopeKind === "correction" || scopeKind === "meta") &&
                decayedContextItems.length > 0, (values) => fmt("DECAYED_CONTEXT", values), 0, 21, 6),
        ...
        const cacheSafeExcludedLabels = cacheSafeDegraded
            ? sectionEntries
                ...
                .map((entry) => entry.key)
            : [];
        const scopedEntries = orderSliceSections(sectionEntries).filter((entry) => entry.include && !cacheSafeExcludedLabels.includes(entry.key));
        ...
        const contextReceiptHelpful = receiptExcludedCount > 0 ||
        ...
        const priorMissionReused = scopeKind === "mission_carryover" &&
        ...
        const scopeSourceTurnId = getAttachmentRuntime().queryScope?.sourceTurnId ||
        ...
        const resetReason = scopeKind === "fresh_question" ? "fresh_scope" : scopeKind === "correction" ? "correction_reset" : null;
        ...
        if (getAttachmentRuntime().focusaAvailable) {
            turnWorkLoopWriterHeaders()
                .then((headers) => focusaFetch("/work-loop/context", {
                method: "POST",
                headers,
                body: JSON.stringify({
        ...
        const lifecycleSessionId = String(ctx?.sessionManager?.getSessionId?.() || getAttachmentRuntime().sessionFrameKey || "no-session");
    ...
    pi.on("input", async (event, _ctx) => {
        ...
        if (runtime.focusaAvailable) {
            const verified = getLastProjectVerify();
            ...
            if (projectRoot && continuityId) {
                void focusaFetch(`/temporal/status?project_root=${encodeURIComponent(projectRoot)}&continuity_id=${encodeURIComponent(continuityId)}`).then((temporal) => {
                    runtime.temporalPriorityContext = temporal?.projection || temporal;
                }, () => {
                    runtime.temporalPriorityContext = undefined;
                });
                const predictionAuthorityPath = predictionAuthorityProjectionPath(verified, continuityId);
                if (predictionAuthorityPath)
                    void focusaFetch(predictionAuthorityPath).then((authority) => {
                        runtime.predictionAuthorityContext = authority;
                    }, () => {
                        runtime.predictionAuthorityContext = undefined;
                    });
            }
            void focusaFetch("/agent-runtime/instruction-integrity/status").then((integrity) => {
                runtime.instructionIntegrityContext = integrity;
            }, () => {
                runtime.instructionIntegrityContext = undefined;
            });
        }
        if (cleanedText.trim() &&
            (runtime.compactionVerifyPendingKey ||
                ["pending", "unknown_completion", "deferred_to_next_turn"].includes(runtime.compactResumeDeliveryState))) {
        ...
        const boundAsk = String(activeWorkpoint?.current_ask_binding || "").trim();
        if (activeWorkpoint && newTaskText && boundAsk !== newTaskText) {
            setActiveWorkpointPacket({
                ...
                current_ask_scope: {
                    ...activeWorkpoint.current_ask_scope,
                    action_authority_for_current_ask: false,
                    matches_current_ask_scope: false,
        ...
        const projectRoot = adoptPiProjectRoot(_ctx?.cwd);
        ...
        if (getAttachmentRuntime().focusaAvailable) {
            ...
            if (steeringDetected && rootConfirmed) {
                ...
                refreshOntologyContextLifecycle("operator_steering", getAttachmentRuntime().currentAsk?.text, true).catch(() => null);
            }
        }
        if (getAttachmentRuntime().cfg?.emitMetrics) {
            const common = {
    ...
    pi.on("turn_end", async (event, ctx) => {
        ...
        if (getAttachmentRuntime().focusaAvailable) {
            ...
            if (scopeFailures.length || detectedLeakClasses.length) {
                ...
                refreshOntologyContextLifecycle("failure_or_degradation", getAttachmentRuntime().currentAsk?.text).catch(() => null);
            }
        ...
        const { inputTokens: usageInputTokens, cacheReadTokens: usageCacheReadTokens, cacheWriteTokens: usageCacheWriteTokens, } = normalizeCacheUsage(ev.usage || ev.message?.usage);
        const selectedModel = ev.model?.id || ev.message?.model?.id || ev.message?.model || ev.model || "unknown";
        const selectedProvider = ev.provider?.id || ev.message?.provider?.id || ev.message?.provider || ev.provider || "unknown";
        const cacheObservation = cacheSafetyMonitor.observeUsage({
            ...
            layoutMode: getAttachmentRuntime().cfg?.cacheSafePromptLayoutEnabled === false
                ? "legacy_prepend"
                : "cache_safe_tail",
        });
        if (cacheObservation)
            emitCacheSafetyObservation(cacheObservation, ctx);
        ...
        if (snapshot.decisions.length)
            w.push(`📌 ${snapshot.decisions.length} decisions`);
        if (snapshot.constraints.length)
            w.push(`🔒 ${snapshot.constraints.length} constraints`);
        if (snapshot.failures.length)
            w.push(`⚠️ ${snapshot.failures.length} failures`);
        if (getAttachmentRuntime().wbmEnabled)
        ...
        if (!getAttachmentRuntime().focusaAvailable)
            w.push("⚪ degraded");
        // §10.4: Thesis snippet
        if (liveFocus?.frame?.thread_thesis)
            w.push(`🎯 ${liveFocus.frame.thread_thesis.slice(0, 50)}`);
        // §30: Metacognitive indicator
        if (getAttachmentRuntime().lastMetacogEvent)
            w.push(`✨ ${getAttachmentRuntime().lastMetacogEvent}`);
        ...
        if (!projectBinding || projectBinding.state !== "BOUND") {
            ...
            ctx.ui.setWidget("focusa", (_tui, theme) => ({
                render(width) {
                    return renderWorkRailWidget(workRailWidget, width, {
        ...
        if (getAttachmentRuntime().focusaAvailable && getAttachmentRuntime().activeFrameId) {
            const hasSignificant = getAttachmentRuntime().localDecisions.length > 0 ||
    ...
    pi.on("message_update", async (event, _ctx) => {
        ...
        if (getTurnCount() % 10 !== 0 && fullText.length - getLastStreamLen() < 500)
            return;
    ...
    pi.on("model_select", async (event, ctx) => {
        ...
        const continuityId = getContinuityId();
        try {
            const temporal = projectRoot && continuityId
                ? await focusaFetch(`/temporal/status?project_root=${encodeURIComponent(projectRoot)}&continuity_id=${encodeURIComponent(continuityId)}`)
                : undefined;
            getAttachmentRuntime().temporalPriorityContext = temporal?.projection || temporal;
        }
        catch {
            getAttachmentRuntime().temporalPriorityContext = undefined;
        }
        try {
            const predictionAuthorityPath = predictionAuthorityProjectionPath(verified, continuityId);
            getAttachmentRuntime().predictionAuthorityContext = predictionAuthorityPath
                ? await focusaFetch(predictionAuthorityPath)
                : undefined;
        }
        catch {
            getAttachmentRuntime().predictionAuthorityContext = undefined;
        }
        try {
            getAttachmentRuntime().instructionIntegrityContext = await focusaFetch("/agent-runtime/instruction-integrity/status");
        }
        catch {
            getAttachmentRuntime().instructionIntegrityContext = undefined;
        }
    ...
    pi.on("after_provider_response", async (event, _ctx) => {
        ...
        if (!providerStatusSuggestsContextOverflow(status, headers))
            return;
    ...
    pi.on("tool_result", async (event, _ctx) => {
        ...
        if (getAttachmentRuntime().focusaAvailable) {
            focusaPost("/ontology/tool-result-proposals", {
                ...
                evidence_refs: [],
                workpoint_id: getActiveWorkpointPacket()?.workpoint_id ||
        ...
        if ((content.length > byteThreshold || tokens > tokenThreshold) &&
    ...
    pi.on("tool_call", async (event, _ctx) => {
        pushToToolUsageBatch(event.toolName || event.name || "");
    });
}
