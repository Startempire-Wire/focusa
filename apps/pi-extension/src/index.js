...
import { attachmentRuntimeRegistry, getAttachmentRuntime, getEffectiveFocusSnapshot, getFocusaAvailable, getActiveFrameId, getActiveWorkpointPacket, isProjectRootAuthoritySafe, makeAttachmentKey, runWithAttachmentRuntime, } from "./state.js";
import { PiExtensionSessionBinding, attachmentRoutingHints, verifiedScopeRefForRoot, } from "./scoped-state.js";
...
export default function focusaPiBridge(pi) {
    ...
    const withRuntime = (fn) => runWithAttachmentRuntime(extensionKey, fn);
    const sessionBinding = new PiExtensionSessionBinding();
    return withRuntime(() => {
        const runtimeFor = (ctx, eventOrParams) => {
            const hints = attachmentRoutingHints(eventOrParams);
            const sessionId = String(hints.sessionId ||
            ...
            const continuityId = String(explicitContinuity || getAttachmentRuntime(extensionKey).continuityId || "extension-bootstrap");
        ...
        const prepareRuntime = (key) => {
            ...
            if (!target.cfg && bootstrap.cfg)
                target.cfg = bootstrap.cfg;
            if (!target.pi)
                target.pi = pi;
            attachmentRuntimeRegistry.bindSessionAttachment(key);
            if (isProjectRootAuthoritySafe(key.workstream.root_scope.root_path) &&
        ...
        if (originalOn) {
            pi.on = (eventName, handler) => originalOn(eventName, (event, ctx) => runWithAttachmentRuntime(prepareRuntime(runtimeFor(ctx, event)), () => handler(event, ctx)));
        }
        const originalRegisterCommand = pi.registerCommand?.bind(pi);
        if (originalRegisterCommand) {
            pi.registerCommand = (name, command) => originalRegisterCommand(name, {
                ...command,
                handler: command?.handler
                    ? (args, ctx) => runWithAttachmentRuntime(prepareRuntime(runtimeFor(ctx, { command: name })), () => command.handler(args, ctx))
                    : command?.handler,
            });
        }
        const originalRegisterShortcut = pi.registerShortcut?.bind(pi);
        if (originalRegisterShortcut) {
            pi.registerShortcut = (keys, shortcut) => originalRegisterShortcut(keys, {
                ...shortcut,
                handler: shortcut?.handler
                    ? (ctx) => runWithAttachmentRuntime(prepareRuntime(runtimeFor(ctx, { shortcut: keys })), () => shortcut.handler(ctx))
                    : shortcut?.handler,
            });
        }
        const originalRegisterTool = pi.registerTool?.bind(pi);
        if (originalRegisterTool) {
            pi.registerTool = (tool) => originalRegisterTool({
                ...tool,
                execute: tool?.execute
                    ? (id, params, signal, onUpdate, ctx) => runWithAttachmentRuntime(prepareRuntime(runtimeFor(ctx, params || {})), () => tool.execute(id, params, signal, onUpdate, ctx))
                    : tool?.execute,
            });
        }
        ...
        const ownsCompactionCoordinator = registerAutoCompaction(pi, () => proactiveCompactionPolicy(getAttachmentRuntime().cfg), () => getAttachmentRuntime().cfg);
        if (!ownsCompactionCoordinator)
            return;
        registerTools(pi);
        registerCommands(pi);
        ...
        pi.registerShortcut("ctrl+shift+r", {
            ...
            handler: async (ctx) => {
                ...
                ctx.ui.notify(`Work Rail | ${bead} | ${workpointId} | ${proofStatus} | next:${next}`, packet ? "info" : "warning");
            },
        });
        ...
        const renderFocusaState = (message, _options, theme) => {
            ...
            const parts = ["📎 Focusa State"];
            if (d.frameTitle)
                parts.push(`Title: ${d.frameTitle}`);
            if (d.frameGoal)
                parts.push(`Goal: ${d.frameGoal}`);
            if (d.intent)
                parts.push(`Mission: ${d.intent}`);
            if (d.currentFocus)
                parts.push(`Focus: ${d.currentFocus}`);
            if (d.frameId)
                parts.push(`Frame: ${d.frameId}`);
            if (d.sessionId)
                parts.push(`Session: ${d.sessionId}`);
            if (decisions.length)
                parts.push(`D:${decisions.length}`);
            if (constraints.length)
                parts.push(`C:${constraints.length}`);
            if (failures.length)
                parts.push(`F:${failures.length}`);
            parts.push(`T:${d.turnCount || 0}`);
            if (d.totalCompactions)
                parts.push(`Compactions:${d.totalCompactions}`);
