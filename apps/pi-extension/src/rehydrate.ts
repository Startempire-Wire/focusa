...
export type RehydrateDependencies = {
  getLocal: (kind: string, id: string) => string | null;
  fetchRemote: (path: string, init: { method: "POST" }) => Promise<unknown>;
};
...
export function parseRehydrateArgs(args: string): { handleId: string; maxTokens: number } {
...
function clipLocal(content: string, maxTokens: number): { content: string; truncated: boolean } {
...
export async function rehydrateHandle(
  ...
  const response = (await dependencies.fetchRemote(
    `/ecs/rehydrate/${encodeURIComponent(handleId)}?max_tokens=${maxTokens}`,
    { method: "POST" }
