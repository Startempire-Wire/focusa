...
import { DEFAULT_DAEMON_RESTART_COMMAND } from "./config.js";
import { buildProjectWorkstreamKey, registerVerifiedScopeRef, verifiedScopeRefForRoot, } from "./scoped-state.js";
import { projectBindingAllowsDurableWrites } from "./project-binding.js";
import { resolveCanonicalMarkerProjectRoot, resolveProjectIdentityLookupCwd, } from "./project-identity-working-context.js";
import { COMPACTION_PERSISTENCE_ANCHOR_REF_SCHEMA, COMPACTION_PERSISTENCE_ANCHOR_SCHEMA, NATIVE_ANCHOR_MAX_BYTES, PROJECT_SWITCH_ANCHOR_MAX_BYTES, semanticPersistenceDigest, stableSemanticValue, writeRecoverySidecar, } from "./persistence.js";
export { COMPACTION_PERSISTENCE_ANCHOR_REF_SCHEMA, COMPACTION_PERSISTENCE_ANCHOR_SCHEMA, NATIVE_ANCHOR_MAX_BYTES, PROJECT_SWITCH_ANCHOR_MAX_BYTES, loadPersistedRecoveryState, semanticPersistenceDigest, } from "./persistence.js";
...
function createAttachmentRuntime() {
    return {
        ...
        activeFrameTitle: "",
        activeFrameGoal: "",
        uiCtx: null, // §93: SSE handler needs ctx.ui for high-priority agent alerts
        sessionFrameKey: "",
        sessionCwd: "",
        continuityId: "",
        startupReceptionistActive: false,
        startupReceptionistStartedAt: 0,
        startupReceptionistPreviousThinkingLevel: "",
        modelProvider: "",
        modelId: "",
        providerUsagePercent: null,
        providerRenewalAt: "",
        providerUsageObservedAt: 0,
        wbmEnabled: false,
        wbmDeep: false,
        ...
        compactResumeDeliveryState: "none",
        ...
        pendingLifecycleAdvisories: {},
        sessionProjectClassification: "unknown",
        piSessionProjectRegistry: {},
        projectBindingDecisions: {},
        ...
        spec92HookTelemetry: [],
        spec92TokenTelemetry: [],
        spec92ToolStartTimes: {},
        // FOCUSA_FIX-a52s: shell-tool reminder frequency gate state.
        lastShellReminderAt: 0,
        lastShellReminderTurn: 0,
        ...
        lastNativeSessionPressure: null,
...
function attachmentRuntimeKey(key) {
    return [
...
export class AttachmentRuntimeRegistry {
    runtimes = new Map();
    boundAttachmentsBySession = new Map();
    getOrCreate(key) {
...
const attachmentRuntimeContext = new AsyncLocalStorage();
...
export function currentAttachmentKey() {
    return attachmentRuntimeContext.getStore();
}
export function getAttachmentRuntime(key) {
...
export function runWithAttachmentRuntime(key, fn) {
...
export function classifyTurnOutcome(toolCallCount, hasNonTaskText, hasFailureSignal) {
    if (hasFailureSignal)
        return "blocked";
    if (toolCallCount === 0 && hasNonTaskText)
        return "ack";
    if (toolCallCount === 0)
        return "observed";
    return "tooled";
}
export function pushRecentTurn(slice) {
...
export function clearRecentTurns() {
...
export function shouldEmitRecentTurnsSlice(currentTurnCount) {
    if (currentTurnCount < 1)
        return false;
    if (getAttachmentRuntime().lastRecentTurnsSliceTurn === currentTurnCount)
        return false;
    return true;
}
export function markRecentTurnsSliceEmitted(currentTurnCount) {
...
export function formatRecentTurnsSection(n = RECENT_TURNS_N_DEFAULT) {
 ...
 */
export function splitCacheableSystemPrompt(systemPrompt) {
...
export function resetPiSessionScopedState(reason = "session_boundary") {
...
export function compatibleWorkLoopStatusState(payload) {
...
// ── HTTP helper ──────────────────────────────────────────────────────────────
export async function focusaFetch(path, opts = {}) {
    ...
    const typedScopeHeaders = attachment && isProjectRootAuthoritySafe(root) && continuity && continuity !== "extension-bootstrap"
        ? {
    ...
    for (let attempt = 0; attempt < attempts; attempt++) {
        ...
        try {
            ...
            if (![429, 502, 503, 504].includes(r.status) || attempt === attempts - 1)
                return null;
        }
        catch {
...
export function focusaPost(path, body) {
    focusaFetch(path, { method: "POST", body: JSON.stringify(body) }).catch(() => { });
}
function hasQuotedFocusaPayload(text) {
...
function isContaminatedFrameIdentity(frame) {
...
function isFocusaPayloadWrapperText(text) {
    ...
    if (!normalized)
        return false;
    if (/^(restarted again,?\s*)?(still wrong|wrong|not true|this is|this output|this context|look|see|after restart|same issue|again)[:\s]*$/.test(normalized))
        return true;
    if (/^(why|how|what)[:\s]*$/.test(normalized))
        return true;
    return false;
}
export function sanitizeFocusFailures(items) {
    return (Array.isArray(items) ? items : [])
        .map((item) => String(item || "").trim())
        .filter(Boolean)
        .filter((item) => !/^operator correction:/i.test(item));
}
export function stripQuotedFocusaContext(text) {
    ...
    if (hasQuotedFocusaPayload(raw) && isFocusaPayloadWrapperText(normalized))
        return "";
    return normalized;
}
...
export function detectForbiddenVisibleOutputLeakClasses(text) {
    const normalized = stripQuotedFocusaContext(String(text || "")).trim();
    if (!normalized)
        return [];
    return FORBIDDEN_VISIBLE_OUTPUT_LEAK_CLASSES.filter((entry) => entry.pattern.test(normalized)).map((entry) => entry.class_id);
}
export function isNonTaskStatusLikeText(text) {
    const normalized = String(text || "")
        .replace(/\s+/g, " ")
        .trim();
    if (!normalized)
    ...
    if (/^\//.test(normalized))
        return true;
    if (/^#\s*focusa context\b/i.test(normalized))
        return true;
    if (/^rendered live from focusa-pi-bridge current state\.?/i.test(normalized))
        return true;
    if (/^focusa:\s/i.test(normalized) && /(frame:|title:|goal:|wbm:|turns:|config:)/i.test(normalized))
        return true;
    if (hasQuotedFocusaPayload(normalized))
        return !stripQuotedFocusaContext(normalized);
    return false;
}
export function classifyCurrentAsk(text) {
    const cleaned = stripQuotedFocusaContext(text);
    const lower = cleaned.trim().toLowerCase();
    if (isNonTaskStatusLikeText(text))
        return "meta";
    if (!lower)
        return hasQuotedFocusaPayload(text) ? "meta" : "unknown";
    if (/^(no\b|undo\b|revert\b|wrong\b|that's incorrect\b|not what i asked\b|stop\b|instead\b|ignore previous\b|new task\b|different task\b|go back\b|don't\b)/i.test(lower))
        return "correction";
    if (lower.endsWith("?") ||
        /^(what|why|how|when|where|who|which|can|could|should|is|are|do|does|did)\b/.test(lower))
        return "question";
    if (/^(note|remember|fyi|for context|meta|discussion:)\b/.test(lower))
        return "meta";
    return "instruction";
}
export function isExplicitContinuationAsk(text) {
...
export function isOperatorSteeringInput(text, askKind) {
    const trimmed = stripQuotedFocusaContext(text).trim();
    if (!trimmed)
        return false;
    if (askKind === "question" || askKind === "correction")
        return true;
    if (askKind === "meta")
        return false;
...
export function deriveQueryScope(askKind) {
    return {
...
export function detectScopeFailureSignals(params) {
    ...
    const failures = [];
    const addFailure = (signal) => {
        if (!failures.some((existing) => existing.kind === signal.kind))
    ...
    if ((params.scopeKind === "fresh_question" || params.scopeKind === "correction") &&
        /\b(as we discussed|as noted earlier|continuing from|from the previous task|carry(ing)? over)\b/i.test(outputLower)) {
        addFailure({
...
function boundedAttentionText(value, max = 180) {
...
function workpointValue(packet, key) {
    return String(packet?.[key] || packet?.workpoint?.[key] || "").trim();
}
function latestReportSummaryRefFromFocusState(focusState) {
    if (getLatestReportSummary()?.handle)
        return getLatestReportSummary().handle;
    const candidates = [
        ...arrayField(focusState?.recent_results),
        ...arrayField(focusState?.notes),
        ...arrayField(focusState?.artifacts).map((artifact) => `${artifact?.kind || "artifact"}:${artifact?.label || artifact?.path_or_id || "unknown"}${artifact?.path_or_id ? `@${artifact.path_or_id}` : ""}`),
    ]
        .map((item) => String(item || "").trim())
        .filter(Boolean);
...
function resetToolOutputPressureWindow(now = Date.now()) {
...
export function recordToolOutputPressure(toolName, bytes, tokens) {
    const now = Date.now();
    if (!getAttachmentRuntime().toolOutputPressure.windowStartedAt ||
...
export function toolOutputVisibleRecapReason() {
    if (!getAttachmentRuntime().toolOutputPressure?.recapRequired)
        return "";
    if (getAttachmentRuntime().toolOutputPressure.windowStartedAt &&
...
export function formatToolOutputVisibleRecapLines(reason = toolOutputVisibleRecapReason()) {
    if (!reason)
        return [];
    return [
...
export function markVisibleRecapEmittedIfPresent(assistantOutput) {
...
function cleanResumeVisibleRecapReason(reason) {
    const value = String(reason || "").trim();
    return value ? boundedAttentionText(value, 220) : "";
}
function projectRootFromAbsolutePath(value) {
...
function currentAskDeclaresProjectScope(value) {
    const text = stripQuotedFocusaContext(String(value || ""));
    if (!text.trim())
        return false;
    return (/\b(?:wrong place|not this repo|not this project|different project|remote project|switch project)\b/i.test(text) ||
        /\b(?:switch|change|move|bind|rebind)\s+(?:to\s+)?(?:the\s+)?(?:project|repo|repository|scope|root)\b/i.test(text) ||
...
function isPlausibleProjectAlias(value) {
    const alias = String(value || "")
        .trim()
        .toLowerCase();
    if (!alias || !/[a-z]/.test(alias) || /^\d+(?:\.\d+)+$/.test(alias))
        return false;
    const suffix = alias.includes(".") ? alias.split(".").at(-1) || "" : "";
    return !NON_PROJECT_ARTIFACT_SUFFIXES.has(suffix);
}
function projectAliasesForText(text, root = "") {
    const lower = String(text || "").toLowerCase();
    const aliases = new Set();
    if (/\b(ptm|planmarr|plan-the-marriage|plan the marriage)\b/i.test(lower) ||
        /planmarr|plan-the-marriage/i.test(root))
        aliases.add("PTM");
    if (/\bfocusa\b/i.test(lower) || /\/focusa$/i.test(root))
        aliases.add("Focusa");
    for (const match of lower.matchAll(/(?:https?:\/\/)?([a-z0-9][a-z0-9-]*(?:\.[a-z0-9][a-z0-9-]*)+)/gi)) {
        const host = match[1].replace(/^www\./, "");
        if (!isPlausibleProjectAlias(host))
            continue;
        aliases.add(host);
...
function boundedProjectAction(source, action) {
...
export function observeProjectThreadEvidence(input) {
    ...
    const alias = boundedAttentionText(input.project_alias ||
        projectAliasesForText(input.action || "", projectRoot)[0] ||
        projectRoot.split("/").filter(Boolean).at(-1) ||
        "unknown", 80);
    if (!projectRoot && !alias)
        return null;
    const now = Date.now();
    const keyRoot = projectRoot || `alias:${alias.toLowerCase()}`;
    const existing = getAttachmentRuntime().projectSwitchLedger.find((entry) => (entry.project_root && entry.project_root === projectRoot) ||
    ...
    getAttachmentRuntime().projectSwitchLedger = [
        observation,
        ...getAttachmentRuntime().projectSwitchLedger.filter((entry) => entry !== existing),
    ]
...
export function markObservationAsSupportingWork(alias, primaryScopeRef, whyRelated) {
    const lower = alias.toLowerCase().trim();
    const entry = getAttachmentRuntime().projectSwitchLedger.find((e) => e.project_alias.toLowerCase() === lower || e.project_root.toLowerCase().includes(lower));
    if (!entry)
        return false;
...
function rememberedProjectRootForAlias(alias) {
...
function normalizeProjectHint(value) {
    return String(value || "")
        .trim()
        .replace(/^https?:\/\//i, "")
        .replace(/^www\./i, "")
...
function markerHintValues(marker) {
    const values = [
        marker?.project_id,
    ...
    if (Array.isArray(marker?.aliases))
        values.push(...marker.aliases);
...
function markerMatchesProjectHint(marker, alias) {
...
function searchProjectMarkerForAlias(alias) {
    ...
    while (queue.length && visited < 300) {
        ...
        if (!dir || seen.has(dir) || item.depth > 4)
            continue;
        seen.add(dir);
        visited++;
        ...
        try {
            ...
            if (markerMatchesProjectHint(marker, alias)) {
                ...
                try {
                    ...
                    appendFileSync("/tmp/pi-scratch/alias-resolution.log", `[alias-resolution] directory_detector: resolved ${alias} to ${root} via ${markerPath}\n`);
                }
                catch {
                    /* best effort */
                }
...
export function observeProjectThreadHintsFromText(text, turnId, source, action) {
    ...
    const observations = [];
    if (root) {
        observations.push(observeProjectThreadEvidence({
            project_root: root,
...
function aliasMatchesSavedProject(alias, savedProjectRoot) {
...
function isAliasOnlyProjectRoot(root) {
    return normalizeProjectRoot(root).startsWith("alias:");
}
function projectSwitchLedgerCandidateForAsk(currentAskText, savedProjectRoot) {
    ...
    const scored = getAttachmentRuntime()
        .projectSwitchLedger.filter((entry) => isPlausibleProjectAlias(entry.project_alias))
        .map((entry) => {
        ...
        if (entryRoot && !isAliasOnlyProjectRoot(entryRoot) && lower.includes(entryRoot.toLowerCase()))
            score += 0.8;
        if (/\b(wrong place|not this repo|not this project|different project|remote project|switch project)\b/i.test(lower))
            score += 0.15;
        if (savedRoot && entryRoot === savedRoot)
            score -= 0.6;
    ...
    const best = scored[0];
    if (!best || best.score < PROJECT_SWITCH_LEDGER_MIN_CONFLICT_CONFIDENCE)
        return null;
    const bestRoot = normalizeProjectRoot(best.entry.project_root);
    if (savedRoot && bestRoot === savedRoot)
        return null;
    if (savedRoot &&
...
export function formatProjectSwitchLedgerLines(currentAskText = getAttachmentRuntime().currentAsk?.text || "") {
    const candidate = projectSwitchLedgerCandidateForAsk(currentAskText, getScopedWorkpointPacket()?.project_root || getAttachmentRuntime().sessionCwd || "");
    const entries = (candidate
        ? [
            candidate,
            ...getAttachmentRuntime().projectSwitchLedger.filter((entry) => entry !== candidate),
        ]
        : getAttachmentRuntime().projectSwitchLedger).slice(0, 4);
...
export function emitCurrentAskScopeVerdictTelemetry(verdict, sourceTurnId = getAttachmentRuntime().currentAsk?.sourceTurnId ||
    getAttachmentRuntime().sessionFrameKey ||
    "pi-current-ask-scope") {
    if (!getAttachmentRuntime().focusaAvailable || verdict.status !== "conflict")
        return;
    ...
    if (getAttachmentRuntime().lastCurrentAskScopeTelemetryKey === key)
        return;
...
export function buildCurrentAskScopeVerdict(options = {}) {
    const ask = stripQuotedFocusaContext(options.currentAskText ?? getAttachmentRuntime().currentAsk?.text ?? "");
    const packet = options.workpointPacket || getScopedWorkpointPacket() || {};
    const savedRoot = normalizeProjectRoot(workpointValue(packet, "project_root") || options.projectRoot || getAttachmentRuntime().sessionCwd || "");
    const continuityId = String(options.continuityId ||
        getAttachmentRuntime().continuityId ||
    ...
    const candidateRoot = savedRoot && isAliasOnlyProjectRoot(rawCandidateRoot) && aliasMatchesSavedProject(alias, savedRoot)
        ? ""
        : rawCandidateRoot;
    ...
    const durableProjectWriteAllowed = status === "aligned" &&
...
export function formatCurrentAskScopeVerdictLines(verdict = buildCurrentAskScopeVerdict()) {
    if (verdict.status === "aligned" && verdict.durable_project_write_authority)
        return [];
    return [
...
function currentAskProjectConflictReason(currentAskText, projectRoot, workpointProjectRoot) {
    const ask = stripQuotedFocusaContext(currentAskText || "");
    if (!ask.trim())
        return "";
    const lower = ask.toLowerCase();
    ...
    if (/\b(ptm|planmarr|plan-the-marriage)\b/i.test(lower) &&
...
export function buildAttentionRecallVerdict(options = {}) {
    const packet = options.workpointPacket || getScopedWorkpointPacket() || {};
    const askText = stripQuotedFocusaContext(options.currentAskText ?? getAttachmentRuntime().currentAsk?.text ?? "");
    const projectRoot = normalizeProjectRoot(options.projectRoot || getAttachmentRuntime().sessionCwd || workpointValue(packet, "project_root"));
    const packetProjectRoot = normalizeProjectRoot(workpointValue(packet, "project_root"));
    const continuityId = String(options.continuityId ||
        getAttachmentRuntime().continuityId ||
        workpointValue(packet, "continuity_id") ||
        "").trim();
    const mission = workpointValue(packet, "mission") ||
        getAttachmentRuntime().activeFrameGoal ||
        getAttachmentRuntime().activeFrameTitle ||
        "current Focusa task";
    ...
    const memoryRefreshRecommended = Boolean(visibleRecapReason ||
        conflictReason ||
    ...
    const evidenceRefs = [
        ...
        ...arrayField(packet?.verification_records)
            .slice(0, 3)
            .map((record) => String(record?.evidence_ref || record?.result || "").trim()),
    ]
        .filter(Boolean)
    ...
    return {
        ...
        memory_anchor: {
            ...
            durable_project_write_authority: !conflictReason && isProjectRootAuthoritySafe(projectRoot || packetProjectRoot),
        },
    };
}
export function formatAttentionRecallFocusSliceLines(verdict) {
    const anchor = verdict.memory_anchor;
    if (verdict.status === "attentive" && !verdict.attention_risks.length)
        return [];
    return [
...
function assistantOutputLooksLikeReport(text) {
...
export function nativeSessionAllowsNonessentialPersistence() {
...
export function maybeCaptureReportSummaryFromAssistantOutput(text, turnId) {
    if (!assistantOutputLooksLikeReport(text))
        return null;
...
function scoreRelevance(candidate, askText) {
    const askTokens = tokenizeForRelevance(askText);
    if (!askTokens.length)
        return 0;
    ...
    if (normalizedAsk && candidateText.includes(normalizedAsk))
        score += 8;
    if (/\b(test|failing|error|bug|trace|constraint|decision|scope|question|correction)\b/.test(normalizedAsk) &&
        /\b(test|failing|error|bug|trace|constraint|decision|scope|question|correction)\b/.test(candidateText)) {
        score += 2;
    }
    return score;
}
const GOVERNING_PRIOR_BAND_BOOST = {
...
function freshnessBoost(updatedAt, pinned) {
    if (pinned)
        return 4;
    if (!updatedAt)
        return 0;
...
function normalizeActiveGoverningPriors(priors) {
    const seen = new Set();
    const out = [];
...
function governingPriorContribution(itemPriorKinds, activePriors) {
...
export function selectRelevantRankedItems(items, askText, options) {
    const values = (items || []).filter((item) => Boolean(item?.value && item.value.trim()));
    if (!values.length)
        return { items: [], excluded: [], scores: [] };
...
export function selectRelevantItems(items, askText, options) {
    return selectRelevantRankedItems((items || []).filter((item) => Boolean(item && item.trim())).map((value) => ({ value })), askText, options);
}
export function selectionRelevanceScore(selection) {
    if (!selection.items.length || !selection.scores.length)
        return 0;
...
export function retentionBucketsFromSelection(selection, options) {
...
function inferGoverningPriorKinds(text) {
...
export function formatWorkingSetItems(records) {
    const out = [];
...
export function formatVerifiedDeltaItems(handles) {
    const out = [];
...
export function buildCanonicalReferenceAliases(items) {
    const out = [];
    const seen = new Set();
    const re = /^\[HANDLE:([^:]+):([^\s]+)\s+"([^"]+)"\]$/;
...
export function orderSliceSections(sections) {
...
export function shouldIncludeMissionContext(askText, scopeKind, missionLike) {
    if (scopeKind === "meta")
        return true;
    if (!missionLike.some(Boolean))
        return false;
    if (isExplicitContinuationAsk(askText))
        return true;
    const joinedMission = missionLike.filter(Boolean).join(" \n ").toLowerCase();
    const askTokens = tokenizeForRelevance(askText);
    if (!askTokens.length)
        return scopeKind === "mission_carryover";
    const overlapsMission = askTokens.some((token) => joinedMission.includes(token));
    if (scopeKind === "fresh_question" || scopeKind === "correction")
        return overlapsMission;
    return overlapsMission;
}
export function buildSliceSection(key, label, items, include, formatter, excludedCount, priority, relevanceScore) {
...
export async function kickstartFocusaDaemon(reason = "health_check") {
    if (!getAttachmentRuntime().cfg?.daemonAutoRestart || !getAttachmentRuntime().pi)
        return false;
    if (getAttachmentRuntime().daemonRestartInFlight)
        return getAttachmentRuntime().daemonRestartInFlight;
    const now = Date.now();
    const hourAgo = now - 3_600_000;
    getAttachmentRuntime().daemonRestartAttempts = getAttachmentRuntime().daemonRestartAttempts.filter((t) => t >= hourAgo);
    if (getAttachmentRuntime().daemonRestartAttempts.length >=
        (getAttachmentRuntime().cfg.daemonRestartMaxPerHour || 20))
        return false;
    const last = getAttachmentRuntime().daemonRestartAttempts[getAttachmentRuntime().daemonRestartAttempts.length - 1] ||
        0;
    if (now - last < (getAttachmentRuntime().cfg.daemonRestartCooldownMs || 5_000))
        return false;
    ...
    getAttachmentRuntime().daemonRestartInFlight = (async () => {
        try {
            if (cmd !== DEFAULT_DAEMON_RESTART_COMMAND)
                return false;
...
export async function checkFocusa() {
    const h = await focusaFetch("/health");
    ...
    if (getAttachmentRuntime().focusaAvailable) {
        ...
        getAttachmentRuntime().healthBackoffMs = Math.min(1_000 * Math.pow(2, Math.min(getAttachmentRuntime().healthFailCount - 1, 4)), 15_000);
...
async function loadFocusState() {
    ...
    if (getAttachmentRuntime().activeFrameId)
        scopedQs.set("frame_id", getAttachmentRuntime().activeFrameId);
    if (getAttachmentRuntime().continuityId)
        scopedQs.set("continuity_id", getAttachmentRuntime().continuityId);
    ...
    if (!frame) {
        ...
        frame = getAttachmentRuntime().activeFrameId
            ? frames.find((f) => f.id === getAttachmentRuntime().activeFrameId) || null
            : null;
        if ((!frame || frame.status !== "active" || isContaminatedFrameIdentity(frame)) &&
            getAttachmentRuntime().sessionFrameKey) {
            const scopedActive = [...frames]
                .reverse()
                .find((f) => f.status === "active" &&
                Array.isArray(f.tags) &&
                f.tags.includes(getAttachmentRuntime().sessionFrameKey || "") &&
                !isContaminatedFrameIdentity(f)) || null;
            if (scopedActive) {
                frame = scopedActive;
    ...
    const fs = {
        ...frameState,
        ...(liveAscc || {}),
        current_focus: liveAscc?.current_focus ||
            frameState.current_focus ||
            frameState.current_state ||
            trajectoryShortTermGoal ||
            "",
        current_state: liveAscc?.current_state ||
            frameState.current_state ||
            frameState.current_focus ||
            trajectoryShortTermGoal ||
            "",
...
export function getCachedSemanticMemorySummary() {
    return getAttachmentRuntime().semanticMemoryCache.data || null;
}
export function getCachedEcsHandlesSummary() {
    return getAttachmentRuntime().ecsHandlesCache.data || null;
}
export async function getFocusState() {
    if (!getAttachmentRuntime().activeFrameId && !getAttachmentRuntime().sessionFrameKey)
        return null;
    const cacheKey = `${getAttachmentRuntime().activeFrameId || ""}|${getAttachmentRuntime().sessionFrameKey || ""}`;
    const now = Date.now();
    if (getAttachmentRuntime().focusStateCache.data &&
    ...
    if (getAttachmentRuntime().focusStateCache.inflight &&
        getAttachmentRuntime().focusStateCache.key === cacheKey) {
...
export async function getSemanticMemorySummary() {
    const now = Date.now();
    if (getAttachmentRuntime().semanticMemoryCache.data &&
...
export async function getEcsHandlesSummary() {
    const now = Date.now();
    if (getAttachmentRuntime().ecsHandlesCache.data &&
...
export function trimFrameText(text, max = 80) {
    const normalized = String(text || "")
        .replace(/\s+/g, " ")
        .trim();
    if (!normalized)
...
function derivePiFrameIntent(cwd) {
    ...
    if (ask && askKind !== "meta") {
        const titlePrefix = askKind === "question" ? "Pi Question" : askKind === "correction" ? "Pi Correction" : "Pi Task";
        return {
            projectName,
...
export function ensureContinuityId(cwd) {
    if (getAttachmentRuntime().continuityId)
        return getAttachmentRuntime().continuityId;
    const root = String(cwd || getAttachmentRuntime().sessionCwd || process.cwd())
        .split("/")
        .filter(Boolean)
        .pop() || "root";
...
function projectBeadsIssueJsonlPaths(projectRoot) {
    return [
        join(projectRoot, ".beads", "issues.jsonl"),
        join(projectRoot, ".git", "beads-worktrees", "beads-sync", ".beads", "issues.jsonl"),
    ];
}
export function selectExistingBeadsIssueIdForFocusFrame(projectRoot) {
    ...
    for (const file of projectBeadsIssueJsonlPaths(projectRoot)) {
        ...
        try {
            ...
            const preferred = parsed
                ...
                String(b.updated_at || b.created_at || "").localeCompare(String(a.updated_at || a.created_at || "")))[0] || parsed.find((issue) => typeof issue.id === "string" && issue.id.trim());
            if (preferred?.id)
...
function ensureAutocreatedBeadsIssueForProject(projectRoot) {
    try {
...
export async function ensureFocusaSessionForFrame(projectRoot, continuityId) {
...
export async function createPiFrame(cwd, source = "pi-auto") {
...
export function projectRootAuthorityFailure(value) {
    const root = normalizeProjectRoot(value);
    if (!root)
        return "missing_project_root";
    if (UNSAFE_PROJECT_AUTHORITY_ROOTS.has(root))
        return "unsafe_broad_project_root";
    if (/^\/home\/[^/]+$/.test(root) || /^\/Users\/[^/]+$/.test(root))
        return "unsafe_user_home_project_root";
    // QN Addendum: Check agent runtime patterns
    for (const pattern of AGENT_RUNTIME_PATTERNS) {
        if (pattern.test(root))
            return "agent_runtime_directory";
    }
    return null;
}
export function isProjectRootAuthoritySafe(value) {
...
function confidenceForScore(score) {
    if (score >= 10_000)
        return { confidence: "high", confidenceScore: 0.99 };
    if (score >= 8_000)
        return { confidence: "high", confidenceScore: 0.95 };
    if (score >= 6_000)
        return { confidence: "high", confidenceScore: 0.9 };
    if (score >= 2_000)
        return { confidence: "medium", confidenceScore: 0.75 };
    return { confidence: "low", confidenceScore: 0.25 };
}
function projectRootScoreRequiresConfirmation(confidenceScore) {
    return confidenceScore < 0.9;
}
function findAncestorProjectRootCandidates(start) {
    ...
    while (dir && dir !== "/") {
        if (isProjectRootAuthoritySafe(dir)) {
            ...
            if (score > 0)
                candidates.push({ root: dir, score, depth, markers });
        }
...
function rootCandidatesForOutput(candidates) {
...
export function resolvePiProjectRootCandidate(cwdInput, persistedPacket) {
...
export function resolvePiProjectRoot(cwdInput, persistedPacket) {
    return resolvePiProjectRootCandidate(cwdInput, persistedPacket).projectRoot;
}
export function resolveFocusWriteProjectRoot(liveCwdInput, cachedCwdInput) {
...
export function projectRootConfirmationRequired(projectRoot) {
...
export function projectRootConfirmationSummary(projectRoot) {
    const resolution = getLastProjectRootResolution();
    if (!resolution ||
...
export function adoptPiProjectRoot(cwdInput, persistedPacket) {
...
export function confirmPiProjectRoot(projectRootInput, source = "operator_confirmed_project_root") {
...
export function normalizeWorkpointResumePacketEnvelope(packet) {
...
export async function buildFocusaSessionIdentity(projectRootInput, resumeSource = "manual", overrides = {}) {
    const projectRoot = normalizeProjectRoot(projectRootInput || getAttachmentRuntime().sessionCwd || process.cwd());
    ...
    const persistedBody = persisted.project_identity || persisted;
    ...
    if (safe) {
        ...
        if (markerProjectRoot)
            query.set("project_root", markerProjectRoot);
        if (sessionId)
            query.set("pi_session_id", sessionId);
        const remoteContext = persistedBody.remote_context || {};
        if (remoteContext.remote_host)
            query.set("remote_host", String(remoteContext.remote_host));
        if (remoteContext.remote_user)
            query.set("remote_user", String(remoteContext.remote_user));
        if (remoteContext.remote_port)
            query.set("remote_port", String(remoteContext.remote_port));
        ...
        if (projectIdentity)
            setLastProjectIdentity(projectIdentity);
    }
    const canonicalProjectRoot = normalizeProjectRoot(projectIdentity?.canonical_parent_root || projectIdentity?.project_root || projectRoot);
    const activeWorktreeRoot = normalizeProjectRoot(projectIdentity?.active_worktree_root ||
    ...
    if (sharedBeadsRoot)
        process.env.BEADS_DIR = sharedBeadsRoot;
    else
        delete process.env.BEADS_DIR;
    if (workingSubpathId)
        process.env.FOCUSA_WORKING_SUBPATH_ID = workingSubpathId;
    else
        delete process.env.FOCUSA_WORKING_SUBPATH_ID;
    if (workingBeadsPrefix)
        process.env.FOCUSA_BEADS_PREFIX = workingBeadsPrefix;
    else
        delete process.env.FOCUSA_BEADS_PREFIX;
    const continuityId = String(overrides.continuityId || ensureContinuityId(canonicalProjectRoot || process.cwd()) || "").trim();
...
export async function refreshTrajectoryClarityLifecycle(reason, projectRootInput) {
    if (!getAttachmentRuntime().focusaAvailable)
        return null;
    const projectRoot = normalizeProjectRoot(projectRootInput || getAttachmentRuntime().sessionCwd || process.cwd());
    ...
    if (getAttachmentRuntime().sessionFrameKey)
        query.set("session_id", getAttachmentRuntime().sessionFrameKey);
    ...
    try {
        ...
        const candidateRoot = normalizeProjectRoot(projectIdentity?.project_root || projectIdentityApi?.project_root || "");
        ...
        if (projectIdentity?.status !== "verified" ||
        ...
        if (view?.trajectory?.fallback_prior_project_trajectory === true) {
            ...
            if (receipt?.status !== "verified_same_project_fallback" ||
                !sourceContinuity ||
        ...
        else if (receipt?.status !== "verified_exact" ||
        ...
        const snapshot = {
            ...
            recommended_action: String(clarity.recommended_action ||
            ...
            project_urls: view?.project_identity?.project_urls ||
                view?.project_identity?.project_summary?.urls ||
                view?.project?.project_urls ||
                null,
            deployment: view?.project_identity?.deployment ||
        ...
        if (snapshot.project_identity)
            setLastProjectIdentity(snapshot.project_identity);
...
export function clearScopedWorkpointForUnsafeCwd(reason = "unsafe_cwd_scope_guard") {
    setActiveWorkpointPacket(null);
    setActiveWorkpointSummary("");
    getAttachmentRuntime().continuityId = "";
    getAttachmentRuntime().activeFrameId = null;
...
export function stampWorkpointPacketForCurrentPiSession(packet) {
    if (!packet || typeof packet !== "object")
        return packet;
    return {
        ...packet,
...
export function isWorkpointPacketScopedToCurrentSession(packet) {
    if (!packet || typeof packet !== "object")
        return false;
    ...
    if (!currentProjectRoot || !currentContinuityId || !packetProjectRoot || !packetContinuityId)
        return false;
    if (!isProjectRootAuthoritySafe(currentProjectRoot) || !isProjectRootAuthoritySafe(packetProjectRoot))
        return false;
    if (currentProjectRoot !== packetProjectRoot)
        return false;
    if (currentContinuityId !== packetContinuityId)
        return false;
...
export function getScopedWorkpointPacket() {
    return isWorkpointPacketScopedToCurrentSession(getActiveWorkpointPacket())
        ? getActiveWorkpointPacket()
        : null;
}
const verifiedContinuityBySessionRoot = new Map();
function verifiedContinuityKey(sessionId, projectRoot) {
    return `${String(sessionId || "").trim()}|${normalizeProjectRoot(projectRoot)}`;
}
export function adoptVerifiedContinuityForCurrentSession(projectRoot, continuityId) {
    const root = normalizeProjectRoot(projectRoot);
    const continuity = String(continuityId || "").trim();
    const identityEnvelope = getLastProjectIdentity() || {};
    const identity = identityEnvelope.project_identity || identityEnvelope;
    const decision = currentProjectBindingDecision() || {};
    const verifiedRoot = normalizeProjectRoot(resolveCanonicalMarkerProjectRoot(process.cwd()) ||
        identity.canonical_parent_root ||
        identity.project_root ||
        decision.selected_project_root);
    if (!root || !continuity || !isProjectRootAuthoritySafe(root) || verifiedRoot !== root)
        return false;
    getAttachmentRuntime().continuityId = continuity;
    verifiedContinuityBySessionRoot.set(verifiedContinuityKey(getSessionFrameKey(), root), continuity);
    syncRuntimeFieldsToScopeStore();
    return true;
}
export function adoptPersistedContinuityForSession(data, eventSessionId, cwd) {
...
export function buildCompactInstructions(prefix) {
    const base = getAttachmentRuntime().cfg?.compactInstructions ||
        "Preserve intent, decisions, constraints, next_steps, failures.";
    const workpoint = getScopedWorkpointPacket() || {};
    const mission = String(workpoint?.mission ||
    ...
    const projectRoot = String(workpoint?.project_root ||
    ...
    if (mission)
        parts.push(`Fallback Mission:\n- ${mission}`);
    if (nextSlice)
        parts.push(`Fallback Next Step:\n- ${nextSlice}`);
    if (projectRoot)
        parts.push(`Fallback Scope:\n- project_root:${projectRoot}`);
    if (getAttachmentRuntime().localDecisions.length)
        parts.push(`Decisions:\n${getAttachmentRuntime()
            .localDecisions.map((d) => `- ${d}`)
            .join("\n")}`);
    if (getAttachmentRuntime().localConstraints.length)
        parts.push(`Constraints:\n${getAttachmentRuntime()
            .localConstraints.map((c) => `- ${c}`)
            .join("\n")}`);
    if (getAttachmentRuntime().localFailures.length)
        parts.push(`Failures:\n${getAttachmentRuntime()
            .localFailures.map((f) => `- ${f}`)
            .join("\n")}`);
    return parts.join("\n");
}
// ── wb CLI with HTTP fallback (§38.2) ────────────────────────────────────────
export async function wbExec(args, fallbackUrl, fallbackBody) {
...
export function isGenericPiFrameForCwd(cwd, title, goal) {
    const projectName = cwd.split("/").filter(Boolean).pop() || "root";
    return (title || "") === `Pi: ${projectName}` && (goal || "") === `Work on ${projectName}`;
}
export function adoptWorkpointScopeForFrameRecovery(packet, source, expectedScope) {
    if (!packet || typeof packet !== "object")
        return null;
    ...
    if (packet.canonical === false ||
        ...
        packet.status === "rejected_scope_mismatch")
        return null;
    const explicitScopeMatch = expectedScope?.allowSessionTransfer === true &&
    ...
    if (currentContinuityId && currentContinuityId !== packetContinuityId && !explicitScopeMatch)
        return null;
    ...
    if (currentSessionKey &&
        ...
        !explicitScopeMatch)
        return null;
    if (currentSessionKey && !packetPiSessionKey && !packetSessionId && !explicitScopeMatch)
        return null;
...
export async function ensurePiFrame(cwd, sessionId, source = "pi-auto") {
    ...
    const requestedScopeUsable = requestedResolution.safe === true &&
    ...
    if (getAttachmentRuntime().activeFramePromise)
    ...
    getAttachmentRuntime().activeFramePromise = (async () => {
        ...
        if (!(await ensureFocusaSessionForFrame(resolvedCwd, continuityId)))
            return null;
...
export async function rescopePiFrameFromCurrentAsk(cwd, source = "pi-ask-rescope") {
    ...
    const askKind = getAttachmentRuntime().currentAsk?.kind || "unknown";
    if (!ask || askKind === "meta" || isNonTaskStatusLikeText(ask))
        return getAttachmentRuntime().activeFrameId;
    const activeGoal = trimFrameText(stripQuotedFocusaContext(getAttachmentRuntime().activeFrameGoal || ""), 100).toLowerCase();
    ...
    const genericFrame = isGenericPiFrameForCwd(resolvedCwd, getAttachmentRuntime().activeFrameTitle, getAttachmentRuntime().activeFrameGoal);
...
export function getEffectiveFocusSnapshot(fs) {
    return {
        decisions: fs?.decisions ||
...
function trimPersistText(input) {
    const normalized = String(input || "")
        .replace(/\s+/g, " ")
        .trim();
    if (normalized.length <= MAX_PERSIST_TEXT_CHARS)
        return normalized;
    return `${normalized.slice(0, MAX_PERSIST_TEXT_CHARS - 1)}…`;
}
function tailBounded(items, maxItems = MAX_PERSIST_LIST_ITEMS) {
    return (items || [])
        .map((item) => trimPersistText(String(item || "")))
        .filter(Boolean)
        .slice(-maxItems);
}
function pruneEcsRegistry(now = Date.now()) {
    const flat = [];
    for (const [kind, bucket] of Object.entries(getAttachmentRuntime().ecsRegistry || {})) {
        ...
        if (!Object.keys(bucket || {}).length)
            delete getAttachmentRuntime().ecsRegistry[kind];
    }
...
function boundedObject(value, maxBytes, fallback) {
    if (value == null)
        return value;
    try {
...
function compactWorkpointPacket(packet) {
    const workpoint = packet?.resume_packet?.workpoint || packet?.workpoint || packet || {};
    return {
...
function compactTrajectoryClarity(trajectory) {
    if (!trajectory)
        return null;
    return {
...
function boundedVitalInfoPrompted(value) {
    return Object.fromEntries(Object.entries(value || {})
        .sort((a, b) => Number(b[1] || 0) - Number(a[1] || 0))
        .slice(0, 40));
}
function buildPersistedRecoveryState() {
    const workpoint = getScopedWorkpointPacket();
    return {
        sessionId: getAttachmentRuntime().sessionFrameKey,
        continuityId: getAttachmentRuntime().continuityId,
        projectRoot: normalizeProjectRoot(getLastProjectRootResolution()?.projectRoot || getAttachmentRuntime().sessionCwd || process.cwd()),
        ...
        authoritativeFailures: tailBounded(sanitizeFocusFailures(getAttachmentRuntime().lastFocusSnapshot.failures), 20),
        ...
        lastTrajectoryClarity: boundedObject(getLastTrajectoryClarity(), 48 * 1024, compactTrajectoryClarity(getLastTrajectoryClarity())),
        ...
        toolOutputPressure: getAttachmentRuntime().toolOutputPressure?.recapRequired
            ...
            : null,
        projectSwitchLedger: getAttachmentRuntime().projectSwitchLedger.slice(0, PROJECT_SWITCH_LEDGER_MAX_OBSERVATIONS),
        vitalInfoPrompted: boundedVitalInfoPrompted(getAttachmentRuntime().vitalInfoPrompted),
        pendingLifecycleAdvisories: Object.fromEntries(Object.entries(getAttachmentRuntime().pendingLifecycleAdvisories).slice(-8)),
        sessionProjectClassification: getAttachmentRuntime().sessionProjectClassification,
        piSessionProjectRegistry: Object.fromEntries(Object.entries(getAttachmentRuntime().piSessionProjectRegistry).slice(-64)),
        projectBindingDecisions: Object.fromEntries(Object.entries(getAttachmentRuntime().projectBindingDecisions).slice(-16)),
...
function appendBoundedNativeEntry(customType, payload, hardCap) {
...
function projectSwitchSemanticPayload() {
    return {
        observations: stableSemanticValue(getAttachmentRuntime().projectSwitchLedger.slice(0, 6), "projectSwitchLedger"),
    };
}
function persistProjectSwitchLedgerAnchor() {
    if (!getAttachmentRuntime().pi)
        return;
    const semantic = projectSwitchSemanticPayload();
    const digest = semanticPersistenceDigest(semantic);
    if (digest === getAttachmentRuntime().lastProjectSwitchPersistHash)
        return;
    const payload = {
    ...
    while (Array.isArray(payload.observations) &&
    ...
    if (Buffer.byteLength(JSON.stringify(payload), "utf8") > PROJECT_SWITCH_ANCHOR_MAX_BYTES)
        return;
...
export function persistState() {
    ...
    if (!getAttachmentRuntime().pendingPersistAnchor || !getAttachmentRuntime().lastPersistSidecarKey)
        return;
    const now = Date.now();
    if (getAttachmentRuntime().lastPersistAt > 0 &&
    ...
    if (!appendBoundedNativeEntry("focusa-state", anchor, NATIVE_ANCHOR_MAX_BYTES))
        return;
    if (getAttachmentRuntime().wbmEnabled) {
        appendBoundedNativeEntry("focusa-wbm-state", {
...
export function storeEcsArtifact(kind, content) {
    const id = `local-${Date.now()}-${++_handleCounter}`;
    if (!getAttachmentRuntime().ecsRegistry[kind])
        getAttachmentRuntime().ecsRegistry[kind] = {};
...
export function getEcsArtifact(kind, id) {
    pruneEcsRegistry();
 ...
 */
export function syncRuntimeFieldsToScopeStore() {
    const store = getCurrentScopeStore();
    if (!store)
        return;
    store.sessionCwd = getAttachmentRuntime().sessionCwd;
 ...
 */
export function syncScopeStoreFieldsToRuntime() {
    const store = getCurrentScopeStore();
    if (!store)
        return;
    ...
    if (store.activeFrameId !== null)
        getAttachmentRuntime().activeFrameId = store.activeFrameId;
 ...
 */
export function setTurnCount(v) {
    const store = getCurrentScopeStore();
    if (store)
        store.turnCount = v;
}
 ...
 */
export function getSessionFrameKey() {
    const store = getCurrentScopeStore();
 ...
 */
export function getFocusaAvailable() {
    const store = getCurrentScopeStore();
 ...
 */
export function getLastProjectRootResolution() {
    const store = getCurrentScopeStore();
 ...
 */
export function setLastProjectRootResolution(resolution) {
    if (!resolution?.projectRoot)
        return;
    const store = getCurrentScopeStore();
    if (!store)
        return;
    if (normalizeProjectRoot(resolution.projectRoot) !== store.identity.rootPath)
        return;
 ...
 */
export function getLastProjectIdentity() {
    const store = getCurrentScopeStore();
 ...
 */
export function setLastProjectIdentity(identity) {
    ...
    const store = getCurrentScopeStore();
    if (store)
        store.lastProjectIdentity = identity;
}
/**
 ...
 */
export function setActiveWorkpointPacket(packet) {
    const store = getCurrentScopeStore();
    if (store)
        store.activeWorkpointPacket = packet;
}
/**
 * PI-03: Get active workpoint summary from the typed scope store only.
 */
export function getActiveWorkpointSummary() {
    const store = getCurrentScopeStore();
 ...
 */
export function setActiveWorkpointSummary(summary) {
    const store = getCurrentScopeStore();
    if (store)
        store.activeWorkpointSummary = summary;
}
/** PI-04: Get lastTrajectoryClarity from the typed scope store only. */
export function getLastTrajectoryClarity() {
    const store = getCurrentScopeStore();
...
function trajectorySnapshotMatchesStore(snapshot, store) {
    ...
    if (snapshot?.fallback_prior_project_trajectory === true) {
        ...
        return Boolean(receipt?.status === "verified_same_project_fallback" &&
            sourceContinuity &&
            sourceContinuity !== store.identity.continuityId &&
            String(receipt?.continuity_id || "").trim() === sourceContinuity);
    }
    return (receipt?.status === "verified_exact" &&
...
export function setLastTrajectoryClarity(snapshot) {
    const store = getCurrentScopeStore();
    if (!store)
        return;
...
function ontologyContextMatchesStore(packet, store) {
    ...
    return Boolean(packet?.status === "ok" &&
        packet?.stale !== true &&
...
export function setCachedOntologyContext(packet) {
    const store = getCurrentScopeStore();
    if (!store)
        return;
    store.lastOntologyContext = packet && ontologyContextMatchesStore(packet, store) ? packet : null;
}
export async function refreshOntologyContextLifecycle(reason, currentAsk, operatorSteeringDetected = false) {
    const store = getCurrentScopeStore();
    if (!store || !getAttachmentRuntime().focusaAvailable)
        return null;
...
export function setLatestReportSummary(summary) {
    const store = getCurrentScopeStore();
    if (store)
        store.latestReportSummary = summary;
}
/** PI-07: Reset turn-scoped runtime state on the active store. */
export function resetTurnRuntimeState() {
    const store = getCurrentScopeStore();
    if (!store)
        return;
