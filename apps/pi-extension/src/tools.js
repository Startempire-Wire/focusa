...
import { getAttachmentRuntime, checkFocusa, focusaFetch, focusaPost, compatibleWorkLoopStatusState, ensurePiFrame, getFocusState, ensureContinuityId, isProjectRootAuthoritySafe, projectRootAuthorityFailure, buildFocusaSessionIdentity, normalizeProjectRoot, resolvePiProjectRoot, resolveFocusWriteProjectRoot, confirmPiProjectRoot, projectRootConfirmationRequired, projectRootConfirmationSummary, refreshTrajectoryClarityLifecycle, stampWorkpointPacketForCurrentPiSession, normalizeWorkpointResumePacketEnvelope, adoptWorkpointScopeForFrameRecovery, adoptVerifiedContinuityForCurrentSession, persistState, estimateTokens, storeEcsArtifact, getTurnCount, getActiveFrameId, getContinuityId, getSessionFrameKey, getSessionCwd, getCurrentScopeStore, getLastProjectRootResolution, getLastProjectIdentity, setLastProjectIdentity, getActiveWorkpointPacket, setActiveWorkpointPacket, getActiveWorkpointSummary, setActiveWorkpointSummary, getLastTrajectoryClarity, setLastTrajectoryClarity, getLastProjectVerify, setLastProjectVerify, currentProjectBindingDecision, setCurrentProjectBindingDecision, getToolUsageBatch, getCurrentTaskTurnStart, currentAttachmentKey, } from "./state.js";
import { FOCUSA_TOOL_CONTRACTS, buildFocusaToolAffordanceCatalog, focusaToolContractSummary, } from "./tool-contracts.js";
import { buildProjectWorkstreamKey, renderScopedResultHuman, scopedQueryParams, isWorkstreamKey, sameWorkstream, } from "./scoped-state.js";
...
import { projectEntitlementDecision } from "./entitlement-policy-adapter.js";
function modelVisibleDiscoveryPayload(label, payload, maxChars = 12_000) {
    return renderDiscoveryPayload(label, payload, storeEcsArtifact, maxChars);
}
const SCRATCHPAD_DIR = "/tmp/pi-scratch";
function scratchDir(turn) {
...
function projectRootPermissionPosture(projectRoot) {
...
function appendScratchpadLine(note, tag) {
...
function emitWriteTelemetry(event, body) {
...
function mirrorFailedFocusWrite(kind, reason, payload, meta) {
...
function validateDecision(decision) {
    ...
    if (decision.length > 160) {
        return {
            valid: false,
            reason: "Too verbose — distill to ONE crystallized sentence (max 160 chars). Use scratchpad for elaboration.",
        };
    }
    if (TASK_PATTERNS.test(decision)) {
        return {
            valid: false,
            reason: "Sounds like a task list — decisions capture ARCHITECTURAL CHOICES, not implementation plans. Write task in scratchpad. Distill the decision.",
        };
    }
    if (DEBUG_PATTERNS.test(decision)) {
        return {
            valid: false,
            reason: "Sounds like debugging metadata — decisions are stable choices, not investigation notes. Move to scratchpad.",
        };
    }
    if (SELF_REF_PATTERNS.test(decision)) {
        return {
            valid: false,
            reason: "Sounds like stream-of-consciousness — decisions should be objective architectural statements. Distill from scratchpad notes.",
        };
    }
    if (MULTI_SENTENCE.test(decision)) {
        return {
            valid: false,
            reason: "Multiple sentences — decisions should be ONE crystallized sentence. Per §AsccSections (<=160 chars).",
        };
    }
    return { valid: true };
}
function validateConstraint(constraint, source) {
    ...
    if (!operatorDirective && TASK_PATTERNS.test(constraint)) {
        return {
            valid: false,
            reason: "Sounds like a self-imposed task — constraints are DISCOVERED REQUIREMENTS from environment/architecture. Not 'I will do X'.",
        };
    }
    if (!operatorDirective && /\b(will|should|must|need to|going to)\b/i.test(constraint)) {
        return {
            valid: false,
            reason: "Sounds like self-imposed obligation — constraints are discovered requirements from environment, not agent commitments. Use scratchpad.",
        };
    }
    return { valid: true };
}
function validateFailure(failure) {
    ...
    if (!/(\.|:)/.test(failure)) {
        return {
            valid: false,
            reason: "Vague — failures should be SPECIFIC: what failed AND why (or what you suspect). 'It didn't work' = scratchpad.",
        };
    }
    if (SELF_REF_PATTERNS.test(failure) &&
        !/^(Build|Test|Deploy|API|Request|Query|Compil|Cargo)/i.test(failure)) {
        return {
            valid: false,
            reason: "Sounds like investigation process — failures should be: SPECIFIC COMPONENT failed, with DIAGNOSIS. Move investigation notes to scratchpad.",
        };
    }
    return { valid: true };
}
...
function validateSlot(value, maxChars) {
    ...
    if (/\b(implement | add | create | update | remove | fix all | check | verify | next:|signal:)/.test(lower))
        return false;
    if (/\b(i think|i tried|i'm working|i was|in this session|while i was|my fs\.|my fix|let me|i need to|i will|i'll need)/.test(lower))
        return false;
    if (/\b(status:|next action:|blocker:)/.test(lower))
        return false;
    if (/(\*\*|\u2705|\u274C|- \[ \]|---|```)/.test(value))
        return false;
    if (lower.includes("now") && lower.includes("need to"))
        return false;
    if (lower.includes("continue") && value.length > 80)
        return false;
    return true;
}
function validateNamedSlot(value, maxChars, kind) {
    const trimmed = String(value || "").trim();
    if (!trimmed)
        return { valid: false, reason: `${kind.replace("_", " ")} cannot be empty.` };
...
const RECENT_COGNITIVE_WRITE_KEYS = [];
function cognitiveWriteKey(delta, explicitKey) {
    if (explicitKey)
        return explicitKey;
    return JSON.stringify(delta).toLowerCase().replace(/\s+/g, " ").slice(0, 500);
}
function duplicateCandidateForWrite(key) {
    const duplicate = RECENT_COGNITIVE_WRITE_KEYS.includes(key);
    RECENT_COGNITIVE_WRITE_KEYS.push(key);
    while (RECENT_COGNITIVE_WRITE_KEYS.length > 80)
        RECENT_COGNITIVE_WRITE_KEYS.shift();
    return duplicate;
}
function reflexSuggestionsForFailure(failureClass, status, nextTools) {
    ...
    if (status === "degraded" || status === "offline")
        suggestions.add("degrade_with_recovery");
    if (nextTools.includes("focusa_workpoint_resume"))
        suggestions.add("resume_from_canonical_workpoint");
    if (nextTools.includes("focusa_project_identity") || nextTools.includes("focusa_project_verify"))
        suggestions.add("bind_project_root");
    if (nextTools.includes("focusa_traverse"))
        suggestions.add("prefer_summary_hot_path");
    return Array.from(suggestions).slice(0, 4);
}
function inferFailureClass(status, summary, message, canonical, degraded) {
    const text = `${summary} ${message || ""}`.toLowerCase();
    if (text.includes("no active pi frame") ||
        text.includes("no active frame") ||
        text.includes("frame recovery"))
        return "frame_unavailable";
    if (text.includes("payload_equal=false") ||
    ...
    if (text.includes("schema_invalid") ||
    ...
    if (text.includes("project_root mismatch") ||
        text.includes("scope mismatch") ||
        text.includes("cross-project"))
        return "scope_mismatch";
    if (text.includes("approval required") || text.includes("requires approved"))
...
function recoveryHintForFailure(failureClass, status, tool) {
    switch (failureClass) {
        case "scope_conflict":
            return {
                recovery_hint: "Treat the saved packet as canonical only for its saved scope; verify the current-ask project, then checkpoint/resume in the correct project before file/API action.",
                misuse_hint: "Usually caused by operator project correction, alias/path mismatch, or project-switch ledger evidence that predates API-level scope_mismatch.",
                next_tools: [
        ...
        case "scope_mismatch":
            return {
                recovery_hint: "Use focusa_project_identity/verify with explicit project_root, then checkpoint/resume in the same continuity; do not retry stale packets unchanged.",
                misuse_hint: "Usually caused by broad cwd, cross-project packet reuse, or tool call before project binding.",
                next_tools: [
                    "focusa_project_identity",
                    "focusa_project_verify",
                    "focusa_workpoint_checkpoint",
        ...
        case "frame_unavailable":
            return {
                recovery_hint: "Stay attentive to operator direction, continue from repo/operator context, then create/resume a scoped Workpoint before durable Focus State writes.",
                misuse_hint: "Focus State note tools were used without an active Pi frame; this is recoverable, not a dead end.",
                next_tools: [
                    "focusa_project_identity",
                    "focusa_workpoint_checkpoint",
                    "focusa_workpoint_resume",
        ...
        case "validation_rejected":
            return {
                recovery_hint: "Rewrite the durable slot as one compact declarative sentence, or put verbose/debug/task content in focusa_scratch.",
                misuse_hint: "Durable Focus State slots reject task lists, verbose reasoning, and non-declarative wording.",
        ...
        case "not_found":
            return {
                recovery_hint: "Use the relevant recent/list/read tool or create the missing record before retrying the mutation.",
                misuse_hint: "Likely stale id, missing record, wrong project scope, or evaluating/linking before the source object exists.",
                next_tools: [
                    "focusa_project_identity",
                    "focusa_workpoint_resume",
        ...
        case "read_model_lag":
            return {
                recovery_hint: "Wait briefly, then read/resume the current packet once with the same idempotency scope; avoid duplicate writes.",
        ...
        case "hot_path_timeout":
            return {
                recovery_hint: "Retry the bounded hot route once, then run focusa_tool_doctor/resource_mode; avoid cold/full payload reads.",
        ...
        case "cold_path_timeout":
            return {
                recovery_hint: "Switch to summary/traverse slices or explicit rehydrate refs; schedule cold diagnostics separately.",
        ...
        case "daemon_unavailable":
            return {
                recovery_hint: "Run focusa_tool_doctor; if daemon is down or overloaded, continue from operator/repo context and retry after health is ok.",
                misuse_hint: "Tool failure is infrastructure/reachability, not a reason to stop all useful repo work.",
                next_tools: ["focusa_tool_doctor", "focusa_resource_mode"],
            };
        case "writer_conflict":
            return {
                recovery_hint: "Use writer-status/preflight and avoid mutating work-loop ownership without explicit operator approval.",
        ...
        case "approval_required":
            return {
                recovery_hint: "Do not infer approval; use preflight/read-only path or wait for explicit approved=true/force=true where required.",
                misuse_hint: "A mutating/destructive/background-session action was attempted without required approval fields.",
                next_tools: ["focusa_tool_doctor"],
            };
        case "process_control_failed":
            return {
                recovery_hint: "Read the canonical session/run projection and daemon capabilities, then retry only with the exact current target and durable approval.",
                misuse_hint: "Likely stale run generation, missing approval, runner disconnect, or process-control race.",
                next_tools: ["focusa_silent_sessions", "focusa_tool_doctor"],
            };
        case "unknown_ambiguous_completion":
            return {
                recovery_hint: "Check side effects or canonical read state first, then retry only if no duplicate/cross-scope mutation risk exists.",
        ...
        default:
            if (status === "blocked" || status === "error")
                return {
                    recovery_hint: "Read failure_class, retry.posture, and next_tools; prefer project_identity → trajectory_view → workpoint_resume/checkpoint before retrying.",
...
function focusaToolResult(params) {
    ...
    const failureClass = params.failure_class ??
...
function compactText(value, fallback = "unknown", max = 140) {
    const text = String(value ?? "")
        .replace(/\s+/g, " ")
        .trim();
    return (text || fallback).slice(0, max);
}
function compactApiEcho(value) {
    if (!value || typeof value !== "object")
        return value;
    const input = value;
    const keys = [
    ...
    if (Array.isArray(input.next_tools))
        out.next_tools = input.next_tools.slice(0, 4);
...
function toolTrajectoryProjectRoot(details) {
    ...
    for (const candidateRoot of candidates) {
        ...
        if (root && isProjectRootAuthoritySafe(root))
            return root;
    }
    return "";
}
function normalizeToolTrajectoryLadder(source, sourceLabel) {
    ...
    const waypoints = waypointValues
        .map((item) => typeof item === "string" ? item : firstNonEmptyText(item?.title, item?.desired_state_delta))
        .filter(Boolean);
    if (!hlt && !mlg && !stg && !activeGap && !waypoints.length)
        return null;
    return {
...
function cachedToolTrajectoryLadder(details, refreshed) {
...
function formatToolTrajectoryLadderSummary(ladder) {
    const parts = [
...
async function ensureToolTrajectoryClarity(tool, details) {
    const projectRoot = toolTrajectoryProjectRoot(details);
    if (!projectRoot)
        return objectValue(getLastTrajectoryClarity());
    const cached = objectValue(getLastTrajectoryClarity());
    ...
    const cacheFresh = Boolean(cachedLadder &&
        cachedRoot === projectRoot &&
        cachedAt > 0 &&
        Date.now() - cachedAt < FOCUSA_TOOL_TRAJECTORY_TTL_MS);
    if (cacheFresh)
        return cached;
    if (cachedToolTrajectoryLadder(details, null) && cachedRoot === projectRoot)
        return cached;
    try {
...
function focusaToolDetails(details, result, trajectoryClarity) {
...
function blockedToolResponse(tool, family, summary, failureClass, raw, nextTools) {
...
function formatPiToolTemplate(tpl) {
    ...
    if (Array.isArray(tpl.fields) && tpl.fields.length) {
        const fieldLine = tpl.fields
            ...
            .join(" ");
        if (fieldLine)
            lines.push(`fields: ${fieldLine}`);
    }
    if (tpl.failureClass)
        lines.push(`class: ${tpl.failureClass}`);
    if (tpl.note)
        lines.push(`note: ${tpl.note}`.slice(0, 160));
...
function piToolText(tpl) {
    return formatPiToolTemplate(tpl);
}
function humanSummary(prefix, ids, fields = [], max = 240) {
    const idPart = ids
        .filter((i) => i.value !== null && i.value !== undefined && i.value !== "")
        .map((i) => `${i.label}=${i.value}`)
        .join(" ");
...
function collectVisibleToolIds(value, out, depth = 0) {
    if (!value || depth > 2 || out.size >= 10)
        return;
    if (Array.isArray(value)) {
        for (const item of value.slice(0, 6))
            collectVisibleToolIds(item, out, depth + 1);
        return;
    }
...
function ensureVisibleToolTemplate(_tool, result, details, toolResult) {
    if (!Array.isArray(result?.content))
        return result;
    const textIndex = result.content.findIndex((entry) => entry?.type === "text");
    if (textIndex < 0)
        return result;
    ...
    if (ids.size && !/^ids:/m.test(originalText)) {
        appended.push(`ids: ${Array.from(ids.entries())
            .slice(0, 6)
            .map(([k, v]) => `${k}=${v}`)
            .join(" ")}`);
    }
    if (!/^(fields|status):/m.test(originalText)) {
        ...
        if (toolResult.canonical)
            fields.push("canonical=true");
        if (toolResult.degraded)
            fields.push("degraded=true");
    ...
    if (!appended.length)
        return result;
    const content = result.content.map((entry, index) => index === textIndex ? { ...entry, text: `${originalText}\n${appended.join("\n")}` } : entry);
    return { ...result, content, details: { ...details, visible_tool_template_v1: true } };
}
function timeoutPreservedText(surface, noun = "fallback", timeoutMs = null, recoveryHint = null) {
    const parts = [`${surface} preserved cached advisory ${noun}`, "cause=timeout"];
    if (timeoutMs)
        parts.push(`timeout_ms=${timeoutMs}`);
    parts.push("next=resource_mode/doctor/retry");
    if (recoveryHint)
        parts.push(`recovery_hint=${recoveryHint}`);
...
function resolveActiveWorkpointContext() {
    ...
    const evidenceRefs = verificationRecords
        .map((record) => String(record?.evidence_ref || record?.result || ""))
        .filter(Boolean)
        .slice(0, 8);
    return {
...
function ontologyCandidateDeltaRefs(tool, result, status) {
    const details = (result?.details || {});
    const refs = new Set();
    const add = (kind, value) => {
        const text = String(value || "").trim();
        if (text)
            refs.add(`${kind}:${text}`.slice(0, 220));
    };
    add("tool", tool);
    add("status", status);
    for (const key of ["target_ref", "targetRef", "file", "path", "endpoint", "workpoint_id"])
        add("target", details[key]);
    for (const ref of Array.isArray(details.evidence_refs) ? details.evidence_refs : [])
        add("evidence", ref);
...
function inferToolResult(tool, result) {
    const details = (result?.details || {});
    if (details.tool_result_v1)
        return details.tool_result_v1;
    const text = String(result?.content?.[0]?.text || details.summary || "");
    ...
    const mappedExplicitStatus = ["accepted", "completed", "no_op"].includes(explicitStatus)
        ? explicitStatus
                    ...
                    : ["blocked", "not_found"].includes(explicitStatus)
                        ...
                        : explicitStatus === "degraded"
                            ...
                            : explicitStatus === "error"
                                ? "error"
                                : null;
    const ok = mappedExplicitStatus !== null
        ? ["accepted", "completed", "no_op"].includes(mappedExplicitStatus)
        : details.ok === true ||
            details.valid === true ||
            (!/^❌|blocked|.* unavailable/.test(text) && details.ok !== false && details.valid !== false);
    const validationRejected = mappedExplicitStatus === null && (details.valid === false || /validation_rejected|rejected/.test(text));
    ...
    const degraded = mappedExplicitStatus === null && (details.canonical === false || /degraded|NON-CANONICAL/.test(text));
    const status = mappedExplicitStatus ||
        (validationRejected
            ? "validation_rejected"
            : offline
                ? "offline"
                : blocked
                    ...
                    : degraded
                        ...
                        : ok
                            ? "completed"
                            : "error");
    const readOnly = family === "lineage_intelligence" ||
    ...
    const detailsFailureClass = typeof details.failure_class === "string"
        ...
        : typeof details.response?.failure_class === "string"
            ? details.response.failure_class
            : undefined;
    ...
    const daemonResponse = details.response && typeof details.response === "object"
        ? details.response
        : null;
    const entitlementBlocked = detailsFailureClass === "entitlement_blocked" || daemonResponse?.failure_class === "entitlement_blocked";
    const entitlementCode = daemonResponse && daemonResponse.error && typeof daemonResponse.error === "object"
        ? String(daemonResponse.error.code || "ENTITLEMENT_BLOCKED")
        : "ENTITLEMENT_BLOCKED";
    const activeWorkpoint = resolveActiveWorkpointContext();
    const resultWorkpointId = String(details.response?.workpoint_id ||
...
function defaultFocusaPromptSnippet(name, description) {
...
function paramsWithAutoIdempotency(toolName, params, id) {
    if (toolName !== "focusa_workpoint_checkpoint" ||
        !params ||
...
function shouldAutoRetryWorkpoint(toolName, result, toolResult) {
    if (!["focusa_workpoint_checkpoint", "focusa_workpoint_resume"].includes(toolName))
        return false;
...
function annotateAutoRetry(result, attempts) {
    const details = { ...(result?.details || {}) };
    details.auto_retry = { attempts, policy: "bounded_workpoint_pending_retry" };
    return { ...result, details };
}
function capToolText(text, max = 700) {
    const normalized = String(text ?? "")
        .replace(/\s+\n/g, "\n")
        .trim();
    return normalized.length <= max ? normalized : `${normalized.slice(0, Math.max(0, max - 1))}…`;
}
function formatOperatorDateTime(ms) {
...
function currentTaskTimingAndTokens() {
    const now = Date.now();
    const elapsedMs = Math.max(0, now - (getAttachmentRuntime().currentTaskStartTime || getAttachmentRuntime().sessionStartTime || now));
    const providerTotal = getAttachmentRuntime().currentTaskProviderInputTokens +
        getAttachmentRuntime().currentTaskProviderOutputTokens;
    const estimatedTotal = getAttachmentRuntime().currentTaskInputTokenEstimate +
    ...
    return {
        task_timing: {
            started_at: new Date(getAttachmentRuntime().currentTaskStartTime || getAttachmentRuntime().sessionStartTime || now).toISOString(),
            started_at_operator: formatOperatorDateTime(getAttachmentRuntime().currentTaskStartTime || getAttachmentRuntime().sessionStartTime || now),
...
function capToolOutputText(result) {
    if (!Array.isArray(result?.content))
        return result;
    return {
        ...result,
        content: result.content.map((entry) => entry?.type === "text" ? { ...entry, text: capToolText(entry.text) } : entry),
    };
}
...
function makeToolSchemaStrict(schema, seen = new Set()) {
    ...
    for (const value of Object.values(schema)) {
        if (Array.isArray(value))
            value.forEach((entry) => makeToolSchemaStrict(entry, seen));
        else
            makeToolSchemaStrict(value, seen);
    }
    return schema;
}
function withAgentFirstSchemas(tool) {
    if (!tool?.name?.startsWith?.("focusa_"))
        return tool;
    return {
        ...tool,
...
function withToolResultEnvelope(tool) {
    if (!tool?.name?.startsWith?.("focusa_") || typeof tool.execute !== "function")
        return tool;
...
function formatPushDeltaFailure(reason) {
    switch (reason) {
        case "offline":
...
function pushDeltaFailureRecovery(reason, apiReason) {
    switch (reason) {
        case "offline":
            return {
                failure_class: "daemon_unavailable",
                retry_posture: "safe_retry",
                recovery_hint: "Run focusa_tool_doctor; if resource mode is emergency, use focusa_resource_mode before retrying.",
                next_tools: ["focusa_tool_doctor", "focusa_resource_mode"],
        ...
        case "frame_unavailable":
            return {
                ...
                retry_posture: "safe_retry",
                recovery_hint: "Use scratchpad for this note; checkpoint/resume a project-bound Workpoint before retrying Focus State writes.",
                next_tools: [
                    "focusa_project_identity",
                    "focusa_workpoint_checkpoint",
                    "focusa_workpoint_resume",
        ...
        case "scope_mismatch":
            return {
                ...
                recovery_hint: "Refresh project_root+continuity_id via focusa_project_verify and focusa_workpoint_resume; do not retry with stale project context.",
                next_tools: [
                    "focusa_project_verify",
                    "focusa_workpoint_resume",
                    "focusa_workpoint_checkpoint",
        ...
        case "read_model_lag":
            return {
                ...
                retry_posture: "safe_retry",
                recovery_hint: "Read model may lag a just-created frame or Workpoint; resume/check current packet before retrying once.",
                next_tools: ["focusa_workpoint_resume", "focusa_tool_doctor"],
                api_reason: apiReason,
            };
        case "validation_rejected":
        ...
        default:
            return {
                ...
                recovery_hint: "Run focusa_tool_doctor and inspect response details before retrying to avoid duplicate or cross-scope writes.",
                next_tools: ["focusa_tool_doctor", "focusa_scratch"],
                api_reason: apiReason,
            };
    }
}
function formatNonCriticalWriteFailure(slotLabel, reason, apiReason) {
...
function namedSlotFallback(slotLabel, kind, reason, payload, apiReason) {
...
function conciseObjectiveSuggestion(payload) {
...
function namedSlotValidationFallback(slotLabel, kind, payload, reason) {
...
function projectIdentityVerifiedInPayload(body) {
...
function focusaToolWorkpointScope(packet) {
    ...
    if (!projectRoot || !continuityId || !isProjectRootAuthoritySafe(projectRoot))
        return null;
    if (packet.canonical === false ||
...
async function resolveFocusaToolProjectRoot(explicitProjectRoot) {
    ...
    const cachedIdentity = getLastProjectIdentity() || {};
    const cachedCanonical = normalizeProjectRoot(cachedIdentity.canonical_parent_root || cachedIdentity.project_root);
    const cachedWorking = normalizeProjectRoot(cachedIdentity.active_worktree_root || cachedIdentity.working_context?.active_worktree_root);
    ...
    if (explicit) {
        const identity = await buildFocusaSessionIdentity(explicit, "manual");
    ...
    if (isProjectRootAuthoritySafe(sessionRoot)) {
        const identity = await buildFocusaSessionIdentity(sessionRoot, "manual");
    ...
    if (localScope) {
        if (!getContinuityId())
            getAttachmentRuntime().continuityId = localScope.continuityId;
        return localScope.projectRoot;
    }
    return sessionRoot || normalizeProjectRoot(process.cwd()) || String(process.cwd());
}
function projectRootConfirmationGate(projectRoot, explicitProjectRoot) {
    if (explicitProjectRoot || !projectRootConfirmationRequired(projectRoot))
        return null;
...
function scopeRecoveryContext(body, projectRoot, continuityId, source = "focusa") {
    ...
    const needsRecovery = status === "degraded" ||
        status === "not_found" ||
        canonical === false ||
...
function allowsWorkpointBootstrapFromClarity(body, projectRoot, actionLabel) {
    if (actionLabel !== "workpoint checkpoint")
        return false;
...
export async function pushDelta(delta) {
    ...
    try {
        ...
        const projectRoot = normalizeProjectRoot(resolveFocusWriteProjectRoot(process.cwd(), cachedCwd || liveCwd));
        ...
        if (["no_active_frame", "frame_unavailable", "rejected_scope_mismatch", "scope_mismatch"].includes(String(response?.status || ""))) {
            emitWriteTelemetry("focusa_write_recovery_attempt", {
                targets,
        ...
        const reason = online ? "write_failed" : "offline";
...
export function registerTools(pi) {
    ...
    const agentFirstToolDefinitions = new Map();
    ...
    pi.registerTool({
        ...
        description: "Resolve one explicit project/worktree/continuity/native-session scope against a supplied daemon registry. Never infers a global or foreign daemon.",
    ...
    pi.registerTool({
        ...
        description: "Inspect the current verified Project → HLT → MLG → STG → waypoint → gap → Workpoint → frontier chain before meaningful action. Read-only and fail-closed.",
        promptSnippet: "Use before meaningful work and after session/compaction/model/project/provider/writer transitions; stale authority remains advisory.",
        parameters: Type.Object({
        ...
        async execute(params) {
            ...
            return {
                ...
                details: {
                    ...
                    advisory: true,
                    snapshot,
                    next_tools: snapshot.status === "ready"
                        ? ["focusa_workpoint_resume"]
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            decision: Type.String({
                description: "ONE crystallized architectural choice — what was decided and why (max 160 chars). NOT a task list or debugging note.",
            }),
            rationale: Type.Optional(Type.String({
                description: "Context: why this decision was made (max 200 chars). Summarize from scratchpad notes.",
            })),
        }),
        promptGuidelines: [
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            constraint: Type.String({
                description: "Discovered requirement — hard boundary from environment or architecture (max 200 chars). NOT a task or agent commitment.",
            }),
        ...
        async execute(_id, params) {
            const { constraint, source } = params;
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            failure: Type.String({
                description: "Specific failure: what failed + diagnosis (max 300 chars). Must contain period or colon.",
            }),
    ...
    function focusaRouteTier(path, method = "GET") {
        const route = String(path || "").toLowerCase();
        const verb = String(method || "GET").toUpperCase();
        if (route.includes("/deep") ||
    ...
    function timeoutFailureClassForRoute(path, method) {
        return focusaRouteTier(path, method) === "cold" ? "cold_path_timeout" : "hot_path_timeout";
    }
    function timeoutBudgetForRoute(path, method = "GET") {
    ...
    function compactFallbackPacket(value) {
        if (!value || typeof value !== "object")
            return value;
        return {
    ...
    async function fetchJsonDetailed(url, timeoutMs = 1500) {
    ...
    async function uiaiBrowserHealthCard() {
        ...
        const capacityAvailable = currentCapacity.capacity_available === true ||
        ...
        return {
            ...
            pressure,
            recommended_action: pressure === "high"
    ...
    function scopedResponseFailureClass(response, body) {
        ...
        if (diagnostic.includes("scope"))
            return "scope_mismatch";
        if (response.status === 400 || /schema|invalid|validation/.test(diagnostic))
            return "validation_rejected";
        if (response.status === 403 || diagnostic.includes("permission"))
            return "permission_denied";
        if (response.status === 404 || diagnostic.includes("not_found"))
            return "not_found";
        if (response.status === 0)
            return "daemon_unavailable";
        return "unknown_ambiguous_completion";
    }
    function scopedResponseHuman(body, fallback) {
        return String(body?.human_readable || body?.human?.summary || body?.summary || body?.reason || body?.error || fallback);
    }
    function typedTrajectoryScopeMatches(value, projectRoot, continuityId) {
        const responseRoot = normalizeProjectRoot(value?.project_identity?.project_root ||
            value?.trajectory?.project_root ||
            value?.scope?.project_root ||
            value?.project_root);
        if (!responseRoot || responseRoot !== normalizeProjectRoot(projectRoot))
            return false;
        const responseContinuity = String(value?.trajectory?.continuity_id || value?.scope?.continuity_id || value?.continuity_id || "").trim();
        return !responseContinuity || !continuityId || responseContinuity === continuityId;
    }
    function cachedTrajectoryForScope(projectRoot, continuityId) {
        ...
        if (!cachedRoot || cachedRoot !== normalizeProjectRoot(projectRoot))
            return null;
        if (cachedContinuity && continuityId && cachedContinuity !== continuityId)
            return null;
        return cached;
    }
    async function focusaFetchDetailed(path, opts = {}) {
        const method = String(opts.method || "GET").toUpperCase();
        const timeout = timeoutBudgetForRoute(path, method);
        const bindingDecision = currentProjectBindingDecision();
        const bindingRecoveryRoute = path.startsWith("/project/identity") ||
        ...
        if (!["GET", "HEAD", "OPTIONS"].includes(method) &&
            ...
            const firstMutationSelection = bindingDecision?.state === "QUARANTINED" && !getAttachmentRuntime().vitalInfoPrompted[selectionKey];
        ...
        if (!currentKey)
            throw new Error("attachment_runtime_key_required");
        ...
        const workpointScopeAuthoritative = !!activeWorkpoint &&
        ...
        const verifiedRoot = normalizeProjectRoot((workpointScopeAuthoritative ? activeWorkpoint.project_root : "") ||
            getLastProjectIdentity()?.project_root ||
            getLastProjectVerify()?.project_root ||
            "");
        const verifiedContinuity = String((workpointScopeAuthoritative ? activeWorkpoint.continuity_id : "") || getContinuityId() || "").trim();
        ...
        const verifiedScopeAvailable = isProjectRootAuthoritySafe(verifiedRoot) &&
            verifiedContinuity &&
            verifiedContinuity !== "extension-bootstrap";
        const currentScopeAvailable = isProjectRootAuthoritySafe(currentRoot) &&
            currentContinuity &&
            currentContinuity !== "extension-bootstrap";
        const attachmentKey = verifiedScopeAvailable &&
        ...
        try {
            ...
            if (!r.ok && body === null) {
                body = {
                    ...
                    request_overrides: opts.headers || {},
                };
            }
            if (r.ok && !["GET", "HEAD", "OPTIONS"].includes(method)) {
                const responseRoot = normalizeProjectRoot(body?.scope?.root_scope?.root_path || body?.project_root || body?.project_identity?.project_root);
                ...
                if ((!responseRoot || responseRoot === requestedRoot) &&
                    ...
                    publishScopedStateChange({
                        ...
                        evidence_revision: String(body?.revision || body?.event_id || body?.receipt_id || body?.workpoint_id || "").trim() || undefined,
    ...
    function formatWorkLoopBudgetRemaining(value) {
        if (value == null)
            return "unknown";
        if (typeof value !== "object")
            return String(value);
    ...
    function explainWorkLoopResult(result, fallback) {
        if (result.ok)
            return fallback;
        ...
        if (msg.includes("worktree is not clean"))
            return "blocked: worktree has uncommitted changes";
        if (msg.includes("missing required header"))
            return "blocked: controller identity header missing";
        ...
        if (result.body?.failure_class === "scope_mismatch" ||
        ...
        if (result.status === 0)
            return "blocked: daemon unavailable";
        return `blocked: ${result.body?.error || `request failed (${result.status})`}`;
    }
    function trajectoryTimeoutFallbackResult(action, endpoint, body, response, nextTools, extra = {}) {
        const fallback = {
            ...extra,
    ...
    function replayConsumerSurface(result) {
    ...
    async function enforceTrajectoryClarityPrecondition(projectRoot, actionLabel, opts = {}) {
        ...
        const action = String(clarity.recommended_action ||
            ...
            if (allowsWorkpointBootstrapFromClarity(body, projectRoot, actionLabel)) {
                return {
                    ...
                    details: {
                        ...
                        precondition_warning: "trajectory conflicted because existing Focusa context is for another continuity; checkpointing current operator mission is allowed",
            ...
            if (allowsWorkpointBootstrapFromClarity(body, projectRoot, actionLabel)) {
                return {
                    ...
                    details: {
                        ...
                        failure_class: "validation_rejected",
                        bootstrap_allowed: true,
                        precondition_warning: "trajectory unclear; checkpointing explicit operator mission is allowed to establish Workpoint continuity",
                        scope_recovery_context: recovery?.details || null,
                    },
                };
            }
    ...
    function evidenceClarityFallbackResult(kind, p, projectRoot, clarity) {
    ...
    const workLoopLeases = new Map();
    function workLoopLeaseKey() {
    ...
    function rememberWorkLoopLease(body) {
        if (!body?.writer_id && compatibleWorkLoopStatusState(body) === "unsupported")
            return null;
        ...
        if (!writerId || !Number.isSafeInteger(fencingToken) || fencingToken <= 0 || !expiresAt)
            return null;
        const lease = { writerId, fencingToken, expiresAt };
        workLoopLeases.set(workLoopLeaseKey(), lease);
        return lease;
    }
    async function preferredWriterId() {
        return localWriterId;
    }
    async function currentWorkLoopLease() {
    ...
    function writerLeaseHeaders(writerId, lease) {
        const headers = { "x-focusa-writer-id": writerId };
        if (lease?.writerId === writerId)
            headers["x-focusa-fencing-token"] = String(lease.fencingToken);
        return headers;
    }
    async function requiredWriterLeaseHeaders() {
        const lease = await currentWorkLoopLease();
        if (!lease)
            throw new Error("current scoped Work Loop writer lease is missing, expired, or owned by another writer");
        return writerLeaseHeaders(localWriterId, lease);
    }
    function firstBdReadyIdFromText(text) {
        const t = String(text || "");
        const m = t.match(/\b([a-z0-9]+-[a-z0-9]+(?:\.[0-9]+)?)\b/i);
        return m ? m[1] : null;
    }
    async function inferRootWorkItemId(explicit) {
    ...
    pi.registerTool({
        ...
        async execute() {
            ...
            const enabled = typeof loopStatus?.enabled === "boolean" ? loopStatus.enabled : !!loopStatus?.work_loop?.enabled;
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            ...
            root_work_item_id: Type.Optional(Type.String({
                description: "Optional root provider WorkItem id. If omitted, infer from the active scoped task.",
            })),
            renew_budget: Type.Optional(Type.Boolean({ description: "Explicitly start a fresh budget epoch when action=resume." })),
        ...
        async execute(_id, params) {
            const { action, reason, preset, preflight, root_work_item_id, renew_budget, max_turns, max_wall_clock_ms, max_retries, cooldown_ms, } = params;
            const writerId = await preferredWriterId();
            if (preflight) {
                const route = action === "on"
                    ? "/work-loop/enable"
                    : action === "pause"
                        ? "/work-loop/pause"
                        : action === "resume"
            ...
            if (action === "on") {
                ...
                if (res.ok)
                    rememberWorkLoopLease(res.body);
                return {
                    content: [
                        {
                            type: "text",
                ...
                body: JSON.stringify({
                    ...
                    ...(action === "resume"
                        ? {
                            ...
                            policy_overrides: max_turns !== undefined ||
                                max_wall_clock_ms !== undefined ||
                                max_retries !== undefined ||
                                cooldown_ms !== undefined
            ...
            if (res.ok) {
                if (action === "stop")
                    workLoopLeases.delete(workLoopLeaseKey());
                else
                    rememberWorkLoopLease(res.body);
            }
            return {
    ...
    pi.registerTool({
        ...
        async execute(_id, params) {
            const { parent_work_item_id } = params;
            const writerId = await preferredWriterId();
            ...
            if (!parentWorkItemId) {
                return {
                    ...
                    details: {
                        ...
                        status: 422,
                        response: {
                            error: "parent_work_item_id required when no current_task is active and no bd ready item is available",
                        },
                    },
                };
            }
    ...
    function summarizeWorkpointResponse(body) {
        ...
        const next = String(body?.next_step_hint ||
        ...
        if (mission)
            summary += ` mission="${truncateForSummary(mission, 80)}"`;
        if (action)
            summary += ` action="${truncateForSummary(action, 80)}"`;
        ...
        if (updatedAt) {
            ...
            if (!Number.isNaN(updatedMs)) {
                ...
                if (ageMin > 60)
                    ...
                    freshnessMarker = ` packet_age=${ageMin}min`;
            }
        }
        return `${summary}${freshnessMarker}`;
    }
    function buildStateHygieneReport(stackBody) {
        ...
        for (const slot of slots) {
            ...
            items.forEach((item, index) => {
                ...
                const trimmed = String(value || "").trim();
                if (trimmed)
                    signals.push({ id: `${slot}:${index}`, slot, index, value: trimmed.slice(0, 240) });
            });
        }
        ...
        const stale_candidates = signals
            .filter((signal) => ["next_steps", "open_questions", "current_focus"].includes(signal.slot) &&
        ...
        return {
            ...
            recommended_action: duplicate_groups.length || stale_candidates.length
        ...
        async execute() {
            ...
            return {
                ...
                details: {
                    ...
                    human_readable: result.recommended_action === "no_hygiene_needed"
    ...
    pi.registerTool({
        ...
        description: "Daemon-native Spec133 Silent Session client for status, observation, steering, controls, config, receipts, capabilities, and legacy action compatibility; process-control failures return failure_class=process_control_failed with receipt-backed recovery.",
        promptSnippet: "Use as a thin daemon API client. Supply exact session_id/run_id/generation and durable approval/idempotency fields for mutations; the daemon remains canonical authority.",
        parameters: Type.Object({
            ...
            session_name: Type.Optional(Type.String({ description: "Legacy alias for exact session_id; no legacy name normalization." })),
            ...
            command: Type.Optional(Type.String({ description: "Legacy alias for text; never executed as a shell command." })),
            ...
            approved: Type.Optional(Type.Boolean({ description: "Legacy compatibility hint only; never grants authority." })),
            force: Type.Optional(Type.Boolean({ description: "Legacy compatibility hint only; daemon policy decides force." })),
        }),
        async execute(_id, params) {
            const p = params;
            ...
            const requireSession = () => {
                if (!sessionId)
                    throw new Error("exact session_id is required");
            ...
            if (action === "list") {
                ...
                if (!p.run_id || !p.generation)
                    throw new Error("run_id and generation are required");
                const query = new URLSearchParams({
                ...
                if (p.cursor)
                    query.set("cursor", String(p.cursor));
                ...
                if (!route)
                    throw new Error(`unsupported daemon Silent Session action: ${action}`);
    ...
    pi.registerTool({
        ...
        async execute(_id, params) {
            ...
            const liveContractList = Array.isArray(liveContracts.body?.contracts)
                ...
                : [];
            const liveNames = new Set(liveContractList.map((contract) => String(contract.name || "")).filter(Boolean));
            ...
            const extra_live = liveContractList
                ...
                .filter((name) => name && !staticNames.has(name));
            const stale_live_contracts = scopedContracts
                .filter((contract) => {
                const live = liveContractList.find((item) => item?.name === contract.name);
            ...
            const portableDaemonRestart = "if command -v focusa-daemon >/dev/null 2>&1; then nohup focusa-daemon >/tmp/focusa-daemon.log 2>&1 & elif command -v systemctl >/dev/null 2>&1; then systemctl restart focusa-daemon; else echo 'start focusa-daemon manually from this checkout' >&2; fi";
            const contractDrift = {
                ...
                drift_detected: !liveContracts.ok ||
            ...
            const hookCounts = getAttachmentRuntime().spec92HookTelemetry.reduce((acc, item) => {
            ...
            const latestTokenTurn = String(latestToken?.turn_id || "");
            const currentTurnId = `pi-turn-${getTurnCount()}`;
            const latestTokenIsCurrent = latestTokenTurn === currentTurnId || !latestTokenTurn;
            const latestTokenBudgetClass = String(latestToken?.budget_class || "unknown");
            ...
            const latestTransition = resourceMode.latest_transition ||
            ...
            if (!health.ok)
                recommendations.push("Focusa daemon health is blocked; retry hot status or inspect daemon before state writes.");
            if (!sessionScopeSafe)
                recommendations.push("Session cwd is broad/unsafe; cd to the project folder or pass explicit project_root to project-aware tools.");
            if (projectRootNeedsConfirmation)
                recommendations.push("REQUIRED FIRST: project root confidence is below 90%; use interview/menu to ask the operator which candidate root is correct before Focusa writes.");
            if (sessionScopeSafe && !projectRootNeedsConfirmation)
                recommendations.push("REQUIRED NEXT: run focusa_trajectory_view to confirm current functional state, destination, and waypoints before Workpoint/evidence progress tracking.");
            if (String(resourceMode.mode || "") === "emergency")
                recommendations.push("Resource mode is emergency; avoid cold/full-payload routes and use focusa_resource_mode for recovery posture.");
            ...
            if (!uiaiBrowser.ok)
                recommendations.push("UIAI browser health/metrics unavailable; browser diagnostics may be stale or unreachable.");
            if (uiaiBrowser.pressure === "high")
                recommendations.push("UIAI browser queue pressure is high; narrow browser workload, close stale sessions, or retry after queue drains.");
            ...
            if (missingDocs.length)
                recommendations.push("Some project-aware tool contracts lack docs; run docs maintenance before release proof.");
            if (contractDrift.drift_detected)
                recommendations.push("Tool contract drift detected between Pi static registry and live daemon; rebuild/restart focusa-daemon, then run live contract proof.");
            ...
            const nextActions = !sessionScopeSafe || projectRootNeedsConfirmation
                ? [
                    {
                        action_type: "operator_input_required",
                        prompt: "Confirm the exact existing project_root, choose new-project Genesis, or resume an authorized handoff.",
    ...
    pi.registerTool({
        ...
        execute: async () => {
            ...
            const activeWorktree = normalizeProjectRoot(identity.active_worktree_root || identity.working_context?.active_worktree_root || canonicalParent);
            const workingSubpathId = String(identity.working_context?.working_subpath?.working_subpath_id || "primary");
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            ...
            persisted_project_root: Type.Optional(Type.String({
                description: "Project root retained by the resumed Pi session; advisory candidate only until current worktree/project evidence verifies it.",
            })),
            remote_host: Type.Optional(Type.String({
        ...
        async execute(_id, params) {
            ...
            const authorityProjectRoot = normalizeProjectRoot(markerProjectRoot && requestedProjectRoot === ambientCwd
                ? markerProjectRoot
                : requestedProjectRoot || markerProjectRoot);
            query.set("cwd", requestCwd);
            if (authorityProjectRoot)
                query.set("project_root", authorityProjectRoot);
            if (getSessionFrameKey())
                query.set("pi_session_id", getSessionFrameKey());
            const persistedProjectRoot = normalizeProjectRoot(p.persisted_project_root ||
            ...
            if (p.remote_host)
                query.set("remote_host", p.remote_host);
            if (p.remote_user)
                query.set("remote_user", p.remote_user);
            if (p.remote_port)
                query.set("remote_port", String(p.remote_port));
            if (p.remote_repo_remote)
                query.set("remote_repo_remote", p.remote_repo_remote);
            if (p.remote_workspace_kind)
                query.set("remote_workspace_kind", p.remote_workspace_kind);
            if (p.remote_deploy_root)
                query.set("remote_deploy_root", p.remote_deploy_root);
            ...
            if (!result.ok && body.failure_class === "hot_path_timeout") {
                ...
                return {
                    content: [
                        {
                            type: "text",
                            text: timeoutPreservedText("project identity", cachedIdentity ? "cached identity" : "empty fallback"),
                        },
                    ],
                    details: {
                        ok: false,
            ...
            if (identity && Object.keys(identity).length) {
                ...
                if (verifiedRoot &&
                    ...
                    const priorTrajectory = await focusaFetchDetailed(`/trajectory/view?project_root=${encodeURIComponent(verifiedRoot)}&mode=summary&allow_prior_project_trajectory=true`, { method: "GET" }).catch(() => null);
                    const priorContinuity = String(priorTrajectory?.body?.trajectory?.fallback_source_continuity_id ||
            ...
            const summaryLines = Array.isArray(body.summary_lines)
                ? body.summary_lines.map((line) => String(line)).filter(Boolean)
                : Array.isArray(identity.project_summary?.summary_lines)
                    ? identity.project_summary.summary_lines.map((line) => String(line)).filter(Boolean)
                    : [];
            ...
            const text = result.ok
                ? [
                    ...
                    Array.isArray(body.degraded_reasons) && body.degraded_reasons.length > 0
                        ? `degraded_reasons=${body.degraded_reasons.map((r) => `${r.code}:${r.severity}`).join(", ")}`
                        : null,
    ...
    pi.registerTool({
        ...
        async execute(_id, params) {
            ...
            const requestedProjectRoot = normalizeProjectRoot(p.project_root);
            const cardProjectRoot = normalizeProjectRoot(markerProjectRoot && requestedProjectRoot === ambientCwd
                ? markerProjectRoot
                : requestedProjectRoot || markerProjectRoot);
            if (cardProjectRoot)
                query.set("project_root", cardProjectRoot);
            if (p.current_ask)
                query.set("current_ask", p.current_ask);
            if (p.remote_host)
                query.set("remote_host", p.remote_host);
            if (p.remote_user)
                query.set("remote_user", p.remote_user);
            if (Number.isFinite(p.remote_port))
                query.set("remote_port", String(Math.trunc(Number(p.remote_port))));
            if (p.remote_repo_remote)
                query.set("remote_repo_remote", p.remote_repo_remote);
            if (p.remote_workspace_kind)
                query.set("remote_workspace_kind", p.remote_workspace_kind);
            ...
            const priorContinuityCounts = new Map();
            for (const frame of body.prior_session_context?.recent_frames || []) {
                const candidate = String(frame?.continuity_id || "").trim();
                if (candidate)
                    priorContinuityCounts.set(candidate, (priorContinuityCounts.get(candidate) || 0) + 1);
            }
            const modalPriorContinuity = [...priorContinuityCounts.entries()]
                .sort((a, b) => b[1] - a[1] || a[0].localeCompare(b[0]))[0]?.[0] || "";
            const authoritativeFallbackContinuity = String(body.trajectory_ladder?.fallback_source_continuity_id ||
                ...
                "").trim();
            const inferredFallbackContinuity = String(body.inferred_workpoint_candidate?.source_signals?.prior_session_workpath?.find((entry) => entry?.continuity_id)?.continuity_id || "").trim();
            const recoveredContinuity = authoritativeFallbackContinuity ||
                (!continuityBeforeRecovery || continuityBeforeRecovery === "extension-bootstrap"
                    ? inferredFallbackContinuity
                    : "");
            const recoveredRoot = normalizeProjectRoot(body.project_identity?.canonical_parent_root || body.project_identity?.project_root);
            let continuityRecoveryAdopted = false;
            if (recoveredContinuity &&
                recoveredContinuity !== continuityBeforeRecovery &&
                (continuityRecoveryAdopted = adoptVerifiedContinuityForCurrentSession(recoveredRoot, recoveredContinuity))) {
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            ...
            task_timing: Type.Optional(Type.Any({
                description: "Optional override timing object; Pi auto-populates elapsed task timing when omitted.",
            })),
            token_usage: Type.Optional(Type.Any({
                description: "Optional override token usage object; Pi auto-populates provider/estimated token counts when omitted.",
            })),
        }),
        async execute(_id, params) {
            const p = params;
    ...
    pi.registerTool({
        ...
        description: "Typed save/continue/rollover wrapper for moving long work between Pi sessions without forking or continuity-id fingerprint fallback.",
        promptSnippet: "Use when operator wants to save, continue, or roll over a long Focusa/Pi session with explicit source/target scope.",
        parameters: Type.Object({
            action: Type.String({ description: "save|continue|status|rollover" }),
            rollover_action: Type.Optional(Type.Union([
                Type.Literal("none"),
            ...
            source_scope: Type.Optional(Type.Object({
                ...
                project_root: Type.Optional(Type.String({ description: "Backward-compatible project root field." })),
                canonical_name: Type.Optional(Type.String({ description: "Canonical scope display name." })),
                fingerprint: Type.Optional(Type.String({ description: "Scope fingerprint; never used as continuity id." })),
                continuity_id: Type.Optional(Type.String({ description: "Workstream continuity id under this scope." })),
            })),
            target_scope: Type.Optional(Type.Object({
                scope_kind: Type.Optional(Type.Union([Type.Literal("project"), Type.Literal("host")])),
                scope_id: Type.Optional(Type.String({ description: "Typed scope id from Focusa scope envelope." })),
                root_path: Type.Optional(Type.String({ description: "Verified scope root path." })),
                project_root: Type.Optional(Type.String({ description: "Backward-compatible project root field." })),
            ...
            source_working_subpath_id: Type.Optional(Type.String({ description: "Source WorkingSubpath id; defaults to active context or primary." })),
            target_working_subpath_id: Type.Optional(Type.String({
                description: "Explicit target WorkingSubpath id for auditable cross-worktree transfer.",
            })),
            target_continuity_id: Type.Optional(Type.String({
                description: "Explicit target continuity id when target_scope is same root with rotated continuity.",
            })),
            source_session_id: Type.Optional(Type.String({ description: "Source/native Pi session id." })),
            target_session_id: Type.Optional(Type.String({ description: "Target/native Pi session id after rollover/transfer." })),
            checkpoint_ref: Type.Optional(Type.String({ description: "Pre-created checkpoint ref to bind transfer." })),
            workpoint_packet_ref: Type.Optional(Type.String({ description: "Workpoint/resume packet ref to bind transfer." })),
            compaction_packet_ref: Type.Optional(Type.String({ description: "Spec130 compaction mission packet ref." })),
            project_root: Type.Optional(Type.String({ description: "Deprecated convenience source root; prefer source_scope.root_path." })),
            ...
            continuity_id: Type.Optional(Type.String({ description: "Deprecated source continuity id; prefer source_scope.continuity_id." })),
            write_preload: Type.Optional(Type.Boolean({
                description: "Request preload write guidance; defaults false and never writes implicitly.",
            })),
            preload_target: Type.Optional(Type.Union([
                Type.Literal("cursor"),
                Type.Literal("claude"),
                Type.Literal("codex"),
                Type.Literal("pi"),
            ...
            preload_mode: Type.Optional(Type.Union([
        ...
        async execute(_id, params) {
            ...
            const sourceRootHint = String(p.source_scope?.root_path || p.source_scope?.project_root || p.project_root || getSessionCwd() || "");
            const projectRoot = await resolveFocusaToolProjectRoot(sourceRootHint);
            const sourceContinuityId = String(p.source_scope?.continuity_id || p.continuity_id || getContinuityId() || "").trim();
            ...
            const targetRootHint = String(p.target_scope?.root_path || p.target_scope?.project_root || sourceRootHint);
            const targetProjectRoot = await resolveFocusaToolProjectRoot(targetRootHint);
            const targetContinuityId = String(p.target_scope?.continuity_id || p.target_continuity_id || sourceContinuityId).trim();
            const sourceWorkingSubpathId = String(p.source_working_subpath_id || process.env.FOCUSA_WORKING_SUBPATH_ID || "primary").trim();
            const targetWorkingSubpathId = String(p.target_working_subpath_id || sourceWorkingSubpathId).trim();
            const sourceScope = buildProjectWorkstreamKey(projectRoot, sourceContinuityId, p.source_scope?.canonical_name);
            const targetScope = buildProjectWorkstreamKey(targetProjectRoot, targetContinuityId, p.target_scope?.canonical_name || p.source_scope?.canonical_name);
            const sourceSessionId = String(p.source_session_id || getSessionFrameKey() || getAttachmentRuntime().sessionFrameKey || "");
            ...
            const currentAsk = p.current_ask ||
            ...
            const inferred = apiBody.transfer?.inferred_workpoint_candidate ||
            ...
            let resume = null;
            let trajectory = null;
            let transitionVerification = null;
            if (action === "save" || rolloverAction === "checkpoint") {
                ...
                checkpoint = await focusaFetchDetailed("/workpoint/checkpoint", {
                    ...
                    body: JSON.stringify({
                        ...
                        canonical: true,
                        checkpoint_reason: rolloverAction === "checkpoint"
            ...
            if (action === "rollover" &&
                ...
                const targetNextAction = p.next_action ||
                    hint.next_action ||
                    inferred.next_action ||
            ...
            if (action === "rollover" && targetCheckpoint?.ok && resume?.ok && resume.body?.canonical === true) {
                const targetWorkpointId = resume.body?.workpoint_id ||
            ...
            const rolloverVerified = action !== "rollover" ||
                targetContinuityId === sourceContinuityId ||
                (targetCheckpoint?.ok &&
                    resume?.ok &&
    ...
    pi.registerTool({
        ...
        async execute(_id, params) {
            ...
            const payload = {
                ...
                persisted_project_root: p.persisted_project_root ||
                    getActiveWorkpointPacket()?.scope?.project_root ||
                    getActiveWorkpointPacket()?.project_root ||
                    getLastProjectRootResolution()?.projectRoot ||
                    getLastProjectIdentity()?.project_root,
            ...
            if (!result.ok && body.failure_class === "hot_path_timeout") {
                ...
                return {
                    content: [
                        {
                            type: "text",
                            text: timeoutPreservedText("project verify", cachedIdentity ? "cached identity" : "empty fallback"),
                        },
                    ],
                    details: {
                        ok: false,
            ...
            const bindingCandidates = Array.isArray(body.binding_candidates) ? body.binding_candidates : [];
            const selectedCandidate = bindingCandidates.find((candidate) => normalizeProjectRoot(candidate?.project_root) === verifiedRoot);
    ...
    pi.registerTool({
        ...
        description: "Preview, apply, inspect, or repair the idempotent local project-discipline baseline before Project Genesis.",
        promptSnippet: "Use for new/existing project onboarding; preview first, require confirm=true for apply/rollback, and never infer a remote, stack, deployment, domain, or secret.",
        parameters: Type.Object({
            action: Type.Optional(Type.Union([Type.Literal("preview"), Type.Literal("apply"), Type.Literal("status"), Type.Literal("repair")], { description: "Bootstrap operation; defaults to status." })),
        ...
        async execute(_toolCallId, params) {
            ...
            let result;
            if (action === "status") {
                result = await focusaFetchDetailed(`/project/bootstrap/status?project_root=${encodeURIComponent(projectRoot)}`);
            }
            else {
    ...
    pi.registerTool({
        ...
        description: "Start, resume, inspect, or atomically commit the Project Genesis chain from verified identity and HLT through the first Workpoint.",
        promptSnippet: "Use after project verification when onboarding/readiness is incomplete; HLT Impasse asks at most one concise intent question, and commit requires confirm=true.",
        parameters: Type.Object({
            action: Type.Optional(Type.Union([Type.Literal("start"), Type.Literal("resume"), Type.Literal("status"), Type.Literal("commit")], { description: "Genesis operation; defaults to status." })),
        ...
        async execute(_toolCallId, params) {
            const action = String(params.action || "status");
            const projectRoot = normalizeProjectRoot(params.project_root || getLastProjectIdentity()?.project_root || getSessionCwd());
            if (!projectRoot || !isProjectRootAuthoritySafe(projectRoot)) {
                return {
            ...
            let result;
            if (action === "status") {
                result = await focusaFetchDetailed(`/project/genesis/status?project_root=${encodeURIComponent(projectRoot)}`);
            }
            else {
                result = await focusaFetchDetailed(`/project/genesis/${encodeURIComponent(action)}`, {
                    ...
                    body: JSON.stringify({
                        ...
                        project_root: projectRoot,
                        continuity_id: continuityId,
                        idempotency_key: params.idempotency_key || `genesis:${continuityId}:${params.specification_ref || "project"}`,
                    }),
                });
            }
            const body = result.body || {};
    ...
    pi.registerTool({
        ...
        async execute(_id, params) {
            ...
            const query = new URLSearchParams();
            if (p.family)
                query.set("family", String(p.family));
            if (p.query)
                query.set("query", String(p.query));
            query.set("limit", String(Math.max(1, Math.min(50, Number(p.limit || 20)))));
            if (p.include_payload === true)
                query.set("include_payload", "true");
            const result = await focusaFetchDetailed(`/reflex/primitives?${query.toString()}`);
            const body = result.body || {};
            if (!result.ok)
                return blockedToolResponse("focusa_reflex_primitives", "reflex", `reflex primitives blocked → ${explainWorkLoopResult(result, "reflex registry unavailable")}`, body.failure_class || "daemon_unavailable", body, ["focusa_traverse", "focusa_tool_doctor"]);
            const items = Array.isArray(body.items) ? body.items : [];
            const families = Array.from(new Set(items.map((item) => String(item.family || "unknown"))))
                .slice(0, 6)
                .join(",");
            const toolResult = body.details?.tool_result_v1 ||
                focusaToolResult({
    ...
    pi.registerTool({
        ...
        description: "Read, commit, revise, observe, forecast, or preflight project-scoped temporal claims without fabricating deadlines or urgency.",
        promptSnippet: "Use status first; external commitments require explicit confirmation/evidence, forecasts remain non-canonical ranges, and scope mismatch fails closed.",
        parameters: Type.Object({
            action: Type.Optional(Type.Union([
                Type.Literal("status"),
                Type.Literal("commit"),
            ...
            forecast_authority: Type.Optional(Type.Object({
        ...
        async execute(_toolCallId, params) {
            ...
            if (action === "status") {
                ...
                const actionPath = action === "high-consequence-preflight"
                ...
                const actionBody = action === "high-consequence-preflight"
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            ...
            continuity_id: Type.Optional(Type.String({
                description: "Optional logical continuity id; defaults to Pi continuity id and is part of authority boundary.",
            })),
            mode: Type.Optional(Type.Union([Type.Literal("summary"), Type.Literal("full")], {
                description: "View mode; summary is hot-path bounded.",
            })),
            allow_prior_project_trajectory: Type.Optional(Type.Boolean({
                description: "If true, use the prior same-project trajectory as advisory reload fallback when continuity_id changed.",
            })),
        }),
        async execute(_id, params) {
            const p = params;
            ...
            if (requestedContinuity)
                query.set("continuity_id", requestedContinuity);
            const viewMode = String(p.mode || "summary");
            query.set("mode", viewMode);
            if (p.allow_prior_project_trajectory === true)
                ...
                return {
                    ...
                    details: {
                        ...
                        response_scope: {
                            ...
                            continuity_id: trajectory.continuity_id || body.scope?.continuity_id || body.continuity_id || null,
                        },
            ...
            const responseContinuity = String(trajectory.continuity_id || body.scope?.continuity_id || body.continuity_id || requestedContinuity).trim();
            ...
            if (trajectory.long_term_goal ||
                ...
                setLastTrajectoryClarity({
                    ...
                    refreshed_at: Date.now(),
                    status: String(body.intelligence_view?.clarity_gate?.status ||
                        trajectory.definition_status ||
                        body.status ||
                        "unknown"),
                    recommended_action: String(body.intelligence_view?.clarity_gate?.recommended_action ||
                        body.intelligence_view?.context_sufficiency?.recommended_action ||
                        "unknown"),
                    canonical: body.canonical === true,
                    degraded: body.degraded === true,
                    ...
                    session_id: String(p.session_id || getAttachmentRuntime().sessionFrameKey || body.session_id || "") || null,
                    project_identity_status: String(project.status || "unknown"),
                    trajectory_id: trajectory.trajectory_id || null,
                    scope_verification: trajectory.scope_verification || body.scope_verification || body.trajectory_scope_verification || null,
            ...
            const trajectoryUnset = body.status === "not_found" &&
            ...
            const recovery = trajectoryUnset || trajectoryBootstrapDefault
                ? null
                : scopeRecoveryContext(body, projectRoot, String(p.continuity_id || getContinuityId() || ""), "trajectory_view");
    ...
    pi.registerTool({
        ...
        label: "HLT History",
        description: "Read append-only HLT ledger entries with session filters, fallback candidates, and generic HLT tracking. Spec 125 §7.2-7.6.",
        promptSnippet: "Use when reconstructing trajectory wording, checking previous-valid fallback, or verifying exact HLT history across sessions.",
        parameters: Type.Object({
        ...
        async execute(_id, params) {
            ...
            if (p.continuity_id)
                query.set("continuity_id", String(p.continuity_id));
            if (p.session_id)
                query.set("session_id", String(p.session_id));
            if (p.include_cross_session_fallbacks)
                query.set("include_cross_session_fallbacks", "true");
            if (p.include_generic)
                query.set("include_generic", "true");
            ...
            const body = result.body || {};
            if (!result.ok) {
                return blockedToolResponse("focusa_hlt_history", "trajectory", `hlt history blocked → ${explainWorkLoopResult(result, "hlt history unavailable")}`, body.failure_class || "daemon_unavailable", body, ["focusa_trajectory_view", "focusa_project_verify", "focusa_tool_doctor"]);
            }
            const toolResult = body.details?.tool_result_v1 ||
                focusaToolResult({
                    ok: true,
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            ...
            current_ask: Type.Optional(Type.String({
                description: "Explicit current operator intent; satisfies verified state gate (§169-175). Auto-populated from Pi session if omitted.",
            })),
            goal_source: Type.Optional(Type.String({
            ...
            supersession_evidence_refs: Type.Optional(Type.Array(Type.String(), {
                description: "Durable evidence refs allowing root goal supersession without direct operator prompt.",
            })),
            ...
            required_checks: Type.Optional(Type.Array(Type.String(), {
    ...
    pi.registerTool({
        ...
        async execute(_id, params) {
            ...
            const b = result.body || {};
            if (!result.ok && b.failure_class === "hot_path_timeout")
                return trajectoryTimeoutFallbackResult("assess", "/v1/trajectory/assess", body, b, [
                    "focusa_tool_doctor",
                    "focusa_resource_mode",
            ...
            const text = result.ok
                ? `trajectory assess → gaps=${Array.isArray(b.gaps) ? b.gaps.length : 0} action=${String(b.recommended_action || "unknown")} canonical=${b.canonical === true}${Array.isArray(b.gaps) && b.gaps.length > 0
                    ? "\n" +
                        b.gaps
                            .slice(0, 3)
                            .map((g, i) => `  gap[${i}]: ${String(g.gap_ref || "unknown")} severity=${String(g.severity || "unknown")} → ${String(g.recommended_action || "none")}`)
                            .join("\n")
                    : ""}`
    ...
    pi.registerTool({
        ...
        async execute(_id, params) {
            ...
            const b = result.body || {};
            if (!result.ok && b.failure_class === "hot_path_timeout")
                return trajectoryTimeoutFallbackResult("propose_workpoint", "/v1/trajectory/propose-workpoint", body, b, [
                    "focusa_tool_doctor",
                    "focusa_resource_mode",
                    "focusa_trajectory_propose_workpoint",
                    "focusa_workpoint_checkpoint",
    ...
    pi.registerTool({
        ...
        async execute(_id, params) {
            ...
            const b = result.body || {};
            if (!result.ok && b.failure_class === "hot_path_timeout")
                return trajectoryTimeoutFallbackResult("checkpoint", "/v1/trajectory/checkpoint", body, b, [
                    "focusa_tool_doctor",
                    "focusa_resource_mode",
    ...
    pi.registerTool({
        ...
        async execute(_id, params) {
            ...
            const b = result.body || {};
            if (!result.ok && b.failure_class === "hot_path_timeout")
                return trajectoryTimeoutFallbackResult("resume", "/v1/trajectory/resume", body, b, [
                    "focusa_tool_doctor",
                    "focusa_resource_mode",
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            ...
            project_root: Type.Optional(Type.String({
                description: "Explicit safe project folder/root; use after compaction if Pi cwd is broad like /root.",
            })),
        ...
        async execute(_id, params) {
            ...
            const recovery = res.ok
                ? null
                : scopeRecoveryContext(res.body || {}, projectRoot, p.continuity_id || getContinuityId() || "", "evidence_capture");
            const text = res.ok
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            ...
            diagnostics_ref: Type.Optional(Type.String({
                description: "Stable file/artifact/URL handle for diagnostics JSON; local files are read best-effort.",
            })),
            target_ref: Type.Optional(Type.String({
                description: "Object/page/endpoint proven by these diagnostics; inferred from diagnostics when omitted.",
            })),
            ...
            attach_to_workpoint: Type.Optional(Type.Boolean({
            ...
            create_metacog: Type.Optional(Type.Boolean({
                description: "Defaults false; capture only when this diagnostics pattern should become reusable learning.",
            })),
        }),
        async execute(_id, params) {
            const p = params;
            const readJsonArtifact = (ref) => {
            ...
            const diagnostics = p.diagnostics && typeof p.diagnostics === "object"
                ? p.diagnostics
                : readJsonArtifact(p.diagnostics_ref) || {};
            const focusaScope = diagnostics.focusa_scope ||
            ...
            const dig = (obj, keys) => keys.reduce((cur, key) => (cur && typeof cur === "object" ? cur[key] : undefined), obj);
            const consoleItems = [
            ...
            const classifyDiagnosticsSeverity = () => {
                ...
                const benignAsset = failedItems.length > 0 &&
                    failedItems.every((item) => /\.(png|jpe?g|gif|webp|svg|ico|css|woff2?|ttf)(\?|$)|favicon|analytics|pixel|beacon|tracking/.test(String(item.url || item.request_url || item.name || item).toLowerCase()));
                if (exceptionItems.length > 0 ||
                    /blank page|page crashed|navigation failed|main frame|document failed|hydration failed/.test(allText))
                    return {
                        severity: "page_breaking",
                        alarm: "repair_required",
                        recommended_action: "capture diagnostics evidence, inspect page-breaking exception/navigation failure, then repair before retry",
                    };
                if (/selector_not_found|timeout|click failed|form failed|wait failed|429|403|cors|api failed|workflow blocked/.test(allText))
                    return {
                        severity: "workflow_blocking",
                        alarm: "action_blocked",
                        recommended_action: "use snapshot/read/diagnostics to choose a different selector or API recovery path",
                    };
                if (benignAsset)
                    return {
                        severity: "benign_asset",
                        alarm: "calm",
                        recommended_action: "record bounded evidence only if relevant; continue workflow without alarming repair loop",
                    };
            ...
            const url = String(diagnostics.url ||
                ...
                "unknown-url");
            const action = String(diagnostics.action || diagnostics.operation || diagnostics.selector
            ...
            const evidenceRef = String(p.diagnostics_ref ||
            ...
            const resultSummary = String(p.result ||
                `${errorClass}: severity=${severityClassification.severity} alarm=${severityClassification.alarm} console=${consoleItems.length} exceptions=${exceptionItems.length} failed_requests=${failedItems.length}${diagSummary ? `; ${diagSummary}` : ""}`).slice(0, 500);
            const activeObjectHints = Array.from(new Set([targetRef, url, action, diagnostics.selector, diagnostics.request_url, diagnostics.endpoint]
                .filter(Boolean)
            ...
            if (p.attach_to_workpoint !== false) {
                ...
                if (evidenceResult.ok)
                    sideEffects.push("workpoint_evidence_link");
            }
            ...
            if (p.create_prediction !== false) {
                predictionResult = await focusaFetchDetailed("/predictions", {
                    ...
                    body: JSON.stringify({
                        ...
                        predicted_outcome: severityClassification.severity === "benign_asset"
                        ...
                        confidence: severityClassification.severity === "benign_asset"
                            ? 0.82
                            : failedItems.length || exceptionItems.length || consoleItems.length
                                ? 0.78
                                : 0.62,
                ...
                if (predictionResult.ok)
                    sideEffects.push("prediction_store");
            }
            let metacogResult = null;
            const diagnosticSignalCount = consoleItems.length + exceptionItems.length + failedItems.length;
            const recurringOrSignificant = diagnostics.recurring === true ||
            ...
            if (p.create_metacog === true && recurringOrSignificant) {
                ...
                if (metacogResult.ok)
                    sideEffects.push("metacog_capture");
            }
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            ...
            checkpoint_reason: Type.Optional(Type.String({
                description: "manual|operator_checkpoint|before_compact|after_compact|context_overflow|session_resume|model_switch|fork",
            })),
            mission: Type.String({ description: "Current mission/objective to preserve across compaction." }),
            target_objects: Type.Optional(Type.Array(Type.String(), {
            ...
            verified_evidence: Type.Optional(Type.Array(Type.String(), {
        ...
        async execute(_id, params) {
            ...
            const clarity = p.canonical === false
            ...
            const payload = {
                mission: p.mission,
                next_slice: [p.next_action, ...doNotDrift.map((d) => `DO_NOT_DRIFT: ${d}`)]
                    .filter(Boolean)
                    .join("\n"),
            ...
            const checkpointSummary = String(res.body?.rendered_summary || res.body?.checkpoint_summary?.one_line || "");
            if (res.ok && res.body?.canonical === true && res.body?.workpoint) {
                const adoptedRoot = adoptWorkpointScopeForFrameRecovery(res.body.workpoint, "workpoint_checkpoint_tool");
                if (adoptedRoot) {
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            ...
            current_ask: Type.Optional(Type.String({
                description: "Optional latest operator ask used to compute current-action authority; defaults to Pi current ask.",
            })),
        }),
        promptGuidelines: [
        ...
        async execute(_id, params) {
            ...
            const actionAuthority = res.body?.action_authority_for_current_ask !== false &&
            ...
            const baseToolResult = res.body?.details?.tool_result_v1 ||
                ...
                failure_class: res.body?.failure_class || (rejected ? "scope_mismatch" : canonical ? null : "frame_unavailable"),
                retry: {
            ...
            return {
                ...
                details: {
                    ...
                    next_tools: (toolResult.next_tools ||
                        ...
                        res.body?.next_tools || ["focusa_workpoint_resume", "focusa_trajectory_view", "focusa_traverse"]).slice(0, 4),
                    request: compactApiEcho(payload),
                    response: compactApiEcho(res.body),
                },
            };
    ...
    function spec80ErrorCode(result) {
        if (result.ok)
            return "OK";
    ...
    function metacogQualityGate(input) {
        ...
        if (contentWords >= 8)
            score += 0.35;
        if (String(input.rationale || "").trim().length >= 20)
            score += 0.25;
        if ((input.confidence ?? 0) >= 0.5)
            score += 0.15;
    ...
    function spec80Result(tool, endpoint, request, result, successText, fallbackText, template) {
        let text;
        if (result.ok && result.body) {
            text = template
                ? piToolText({
                    ...
                    tool,
                    summary: successText,
                    ids: (template.ids || []).map((i) => ({ label: i.label, value: i.value })),
    ...
    function spec80CompositeResult(tool, endpoint, request, ok, status, response, successText, fallbackText, template) {
    ...
    async function callSpec80Tool(tool, endpoint, request, opts = {}) {
        ...
        const identity = getLastProjectIdentity() || {};
        const requestProjectRoot = normalizeProjectRoot(resolveCanonicalMarkerProjectRoot(process.cwd()) ||
            identity.canonical_parent_root ||
            identity.project_root ||
            getSessionCwd());
        const requestContinuityId = String(getContinuityId() || "").trim();
        const requestScopeHeaders = isProjectRootAuthoritySafe(requestProjectRoot) && requestContinuityId
            ? {
    ...
    function spec80ValidationResult(tool, endpoint, request, fallbackText, error, code = "SCHEMA_INVALID", expectedSchema) {
        const body = { code, error };
        if (expectedSchema) {
            body.expected_schema = expectedSchema;
            const rejectedField = expectedSchema.required_fields.find((field) => !request || request[field] === undefined || error.toLowerCase().includes(field.toLowerCase())) || expectedSchema.required_fields[0];
        ...
        const result = spec80Result(tool, endpoint, request, { ok: false, status: 422, body }, `${fallbackText}: ok`, fallbackText);
        ...
        if (result?.details?.tool_result_v1) {
            ...
            if (!tr.failure_class)
                tr.failure_class = "schema_invalid";
            if (!tr.retry)
                tr.retry = { posture: "do_not_retry_unchanged", safe: false };
        }
        return result;
    }
    function validateRequiredString(name, value, maxLength, opts = {}) {
        const text = String(value ?? "").trim();
        if (!text)
            return { ok: false, error: `${name} required` };
        if (text.length > maxLength)
            return { ok: false, error: `${name} too long (max ${maxLength})` };
        if (opts.pattern && !opts.pattern.test(text))
            return { ok: false, error: `${name} has invalid format` };
        return { ok: true, value: text };
    }
    function validateOptionalString(name, value, maxLength, opts = {}) {
    ...
    function validateStringArray(name, value, opts) {
        if (value === undefined || value === null)
        ...
        for (const raw of value) {
            if (typeof raw !== "string")
                return { ok: false, error: `${name} items must be strings` };
            const item = raw.trim();
            if (!item)
                return { ok: false, error: `${name} items must not be blank` };
    ...
    function validateNoExtraKeys(tool, params, allowedKeys) {
    ...
    function summarizeArray(values, limit = 3) {
        if (!Array.isArray(values) || values.length === 0)
            return "none";
        return values.slice(0, limit).map(summarizeValue).filter(Boolean).join("; ") || "none";
    }
    function summarizeTraverseItems(items, limit = 6) {
    ...
    pi.registerTool({
        ...
        async execute(_id, params) {
            ...
            if (!keyCheck.ok) {
                return spec80ValidationResult("focusa_tree_head", "/v1/lineage/head", params, "tree head", keyCheck.error);
            }
            const sessionIdCheck = validateOptionalString("session_id", keyCheck.value.session_id, SPEC81_LIMITS.sessionId, { pattern: SPEC81_ID_RE });
            if (!sessionIdCheck.ok) {
                return spec80ValidationResult("focusa_tree_head", "/v1/lineage/head", params, "tree head", sessionIdCheck.error);
            }
            ...
            if (requestedSessionId && activeSessionId && requestedSessionId !== activeSessionId) {
                return spec80ValidationResult("focusa_tree_head", "/v1/lineage/head", params, "tree head", "session_id must match the active native Pi session; foreign lineage is quarantined", "SCOPE_MISMATCH");
            }
            const session_id = requestedSessionId || activeSessionId;
            if (!session_id) {
                return spec80ValidationResult("focusa_tree_head", "/v1/lineage/head", params, "tree head", "active Pi session_id is required; global lineage fallback is prohibited");
            }
            ...
            if (res.ok && returnedSession !== session_id) {
                return spec80ValidationResult("focusa_tree_head", "/v1/lineage/head", req, "tree head", "lineage response session scope mismatch");
            }
            ...
            return spec80Result("focusa_tree_head", "/v1/lineage/head", req, res, `tree head: ${head}\nbranch=${branch} session=${session}\nnext_tools=focusa_tree_path,focusa_tree_snapshot_state`, "tree head");
        },
    });
    pi.registerTool({
        ...
        async execute(_id, params) {
            ...
            if (!keyCheck.ok) {
                return spec80ValidationResult("focusa_tree_path", "/v1/lineage/path/{clt_node_id}", params, "tree path", keyCheck.error);
            }
            const nodeIdCheck = validateRequiredString("clt_node_id", keyCheck.value.clt_node_id, SPEC81_LIMITS.id, { pattern: SPEC81_ID_RE });
            if (!nodeIdCheck.ok) {
                return spec80ValidationResult("focusa_tree_path", "/v1/lineage/path/{clt_node_id}", params, "tree path", nodeIdCheck.error);
            }
            ...
            return spec80Result("focusa_tree_path", "/v1/lineage/path/{clt_node_id}", { clt_node_id: nodeId }, res, `tree path: depth=${depth} nodes=${pathItems.length}\npath=${summarizeArray(pathItems, 5)}\nnext_tools=focusa_tree_snapshot_state,focusa_tree_diff_context`, "tree path");
        },
    });
    pi.registerTool({
        ...
        parameters: strictObject({
            clt_node_id: Type.Optional(Type.String({
                maxLength: SPEC81_LIMITS.id,
                pattern: SPEC81_ID_PATTERN,
        ...
        async execute(_id, params) {
            ...
            if (!keyCheck.ok) {
                return spec80ValidationResult("focusa_tree_snapshot_state", "/v1/focus/snapshots", params, "tree snapshot", keyCheck.error);
            }
            const raw = keyCheck.value;
            const nodeCheck = validateOptionalString("clt_node_id", raw.clt_node_id, SPEC81_LIMITS.id, {
                pattern: SPEC81_ID_RE,
            });
            if (!nodeCheck.ok) {
                return spec80ValidationResult("focusa_tree_snapshot_state", "/v1/focus/snapshots", raw, "tree snapshot", nodeCheck.error);
            }
            const reasonCheck = validateOptionalString("snapshot_reason", raw.snapshot_reason, SPEC81_LIMITS.snapshotReason);
            if (!reasonCheck.ok) {
                return spec80ValidationResult("focusa_tree_snapshot_state", "/v1/focus/snapshots", raw, "tree snapshot", reasonCheck.error);
            }
            ...
            return spec80Result("focusa_tree_snapshot_state", "/v1/focus/snapshots", { ...req, writer_id: res.writerId || null }, res, `tree snapshot: ${String(res.body?.snapshot_id || "created")}\nclt_node=${String(res.body?.clt_node_id || req.clt_node_id || "current")} created_at=${String(res.body?.created_at || "unknown")}\nnext_tools=focusa_tree_diff_context,focusa_tree_restore_state`, "tree snapshot");
        },
    });
    pi.registerTool({
        ...
        parameters: strictObject({
            ...
            restore_mode: Type.Optional(Type.Union([Type.Literal("exact"), Type.Literal("merge")], {
        ...
        async execute(_id, params) {
            ...
            if (!keyCheck.ok) {
                return spec80ValidationResult("focusa_tree_restore_state", "/v1/focus/snapshots/restore", params, "tree restore", keyCheck.error);
            }
            const raw = keyCheck.value;
            ...
            if (!sidCheck.ok) {
                return spec80ValidationResult("focusa_tree_restore_state", "/v1/focus/snapshots/restore", raw, "tree restore", sidCheck.error);
            }
            const mode = String(raw.restore_mode || "exact")
                .trim()
                .toLowerCase();
            if (mode !== "exact" && mode !== "merge") {
                return spec80ValidationResult("focusa_tree_restore_state", "/v1/focus/snapshots/restore", { snapshot_id: sidCheck.value, restore_mode: mode }, "tree restore", "restore_mode must be exact|merge", "INVALID_REQUEST");
            }
            ...
            return spec80Result("focusa_tree_restore_state", "/v1/focus/snapshots/restore", { ...req, writer_id: res.writerId || null }, res, `tree restore: status=${String(res.body?.status || "ok")} snapshot=${String(res.body?.snapshot_id || req.snapshot_id)}\nmode=${mode} conflicts=${conflicts}\nnext_tools=focusa_tree_head,focusa_tree_path`, "tree restore");
        },
    });
    pi.registerTool({
        ...
        async execute(_id, params) {
            ...
            if (!keyCheck.ok) {
                return spec80ValidationResult("focusa_tree_diff_context", "/v1/focus/snapshots/diff", params, "tree diff", keyCheck.error);
            }
            const raw = keyCheck.value;
            ...
            if (!fromCheck.ok) {
                return spec80ValidationResult("focusa_tree_diff_context", "/v1/focus/snapshots/diff", raw, "tree diff", fromCheck.error);
            }
            const toCheck = validateRequiredString("to_snapshot_id", raw.to_snapshot_id, SPEC81_LIMITS.id, {
                pattern: SPEC81_ID_RE,
            });
            if (!toCheck.ok) {
                return spec80ValidationResult("focusa_tree_diff_context", "/v1/focus/snapshots/diff", raw, "tree diff", toCheck.error);
            }
            ...
            return spec80Result("focusa_tree_diff_context", "/v1/focus/snapshots/diff", req, res, `tree diff: changed=${boolLabel(res.body?.checksum_changed)} version_delta=${String(res.body?.version_delta ?? "unknown")}\nclt_changed=${boolLabel(res.body?.clt_node_changed)} decisions_changed=${boolLabel(res.body?.decisions_delta?.changed)}\nnext_tools=focusa_tree_restore_state,focusa_tree_path`, "tree diff");
        },
    });
    pi.registerTool({
        ...
        async execute(_id, params) {
            ...
            if (!keyCheck.ok) {
                return spec80ValidationResult("focusa_metacog_capture", "/v1/metacognition/capture", params, "metacog capture", keyCheck.error);
            }
            const raw = keyCheck.value;
            const kindCheck = validateRequiredString("kind", raw.kind, SPEC81_LIMITS.kind);
            if (!kindCheck.ok) {
                return spec80ValidationResult("focusa_metacog_capture", "/v1/metacognition/capture", raw, "metacog capture", kindCheck.error);
            }
            const contentCheck = validateRequiredString("content", raw.content, SPEC81_LIMITS.longText);
            if (!contentCheck.ok) {
                return spec80ValidationResult("focusa_metacog_capture", "/v1/metacognition/capture", raw, "metacog capture", contentCheck.error);
            }
            const rationaleCheck = validateOptionalString("rationale", raw.rationale, SPEC81_LIMITS.rationale);
            if (!rationaleCheck.ok) {
                return spec80ValidationResult("focusa_metacog_capture", "/v1/metacognition/capture", raw, "metacog capture", rationaleCheck.error);
            }
            const strategyCheck = validateOptionalString("strategy_class", raw.strategy_class, SPEC81_LIMITS.strategyClass);
            if (!strategyCheck.ok) {
                return spec80ValidationResult("focusa_metacog_capture", "/v1/metacognition/capture", raw, "metacog capture", strategyCheck.error);
            }
            if (raw.confidence !== undefined &&
                (!Number.isFinite(raw.confidence) || raw.confidence < 0 || raw.confidence > 1)) {
                return spec80ValidationResult("focusa_metacog_capture", "/v1/metacognition/capture", raw, "metacog capture", "confidence must be between 0 and 1");
            }
            const req = {
            ...
            const relevanceReason = compactText(req.rationale ||
                (req.evidence_refs.length ? `evidence=${req.evidence_refs[0]}` : "captured for future retrieval"), "captured for future retrieval", 100);
            return spec80Result("focusa_metacog_capture", "/v1/metacognition/capture", {
                ...req,
                writer_id: res.writerId || null,
                compact_lesson_line: { lesson: lessonLine, why_relevant: relevanceReason, rehydrate_id: captureId },
            }, res, `metacog capture: id=${captureId} lesson="${lessonLine}" why="${relevanceReason}" rehydrate_id=${captureId}`, "metacog capture", {
                kind: "ok",
                ids: [
    ...
    pi.registerTool({
        ...
        parameters: strictObject({
            ...
            scope_tags: Type.Optional(Type.Array(Type.String({ maxLength: SPEC81_LIMITS.tagText, description: "Optional scope tag." }), {
        ...
        async execute(_id, params) {
            ...
            if (!keyCheck.ok) {
                return spec80ValidationResult("focusa_metacog_retrieve", "/v1/metacognition/retrieve", params, "metacog retrieve", keyCheck.error);
            }
            const raw = keyCheck.value;
            const askCheck = validateRequiredString("current_ask", raw.current_ask, SPEC81_LIMITS.currentAsk);
            if (!askCheck.ok) {
                return spec80ValidationResult("focusa_metacog_retrieve", "/v1/metacognition/retrieve", raw, "metacog retrieve", askCheck.error);
            }
            ...
            if (!tagsCheck.ok) {
                return spec80ValidationResult("focusa_metacog_retrieve", "/v1/metacognition/retrieve", raw, "metacog retrieve", tagsCheck.error);
            }
            let normalizedK = Math.trunc(Number(raw.k ?? 5));
            if (!Number.isFinite(normalizedK))
                normalizedK = 5;
            ...
            const rehydrateHint = lessonRequiresRehydrate
                ...
                : null;
            const topWhy = compactText(top?.rationale ||
                top?.why_relevant ||
                (top?.score !== undefined
                    ? `retrieval_score=${String(top.score)}`
                    : `matched current_ask=${req.current_ask}`), "matched current ask", 100);
            return spec80Result("focusa_metacog_retrieve", "/v1/metacognition/retrieve", {
                ...req,
                compact_top_lesson: total > 0
                    ? {
                        lesson: topLesson,
                        why_relevant: topWhy,
                ...
                : `metacog retrieve: candidates=0 lesson="none" why="no prior signals matched" rehydrate_id=none`, "metacog retrieve", {
    ...
    pi.registerTool({
        ...
        parameters: strictObject({
            ...
            failure_classes: Type.Optional(Type.Array(Type.String({ maxLength: SPEC81_LIMITS.tagText, description: "Failure class tag." }), {
        ...
        async execute(_id, params) {
            ...
            if (!keyCheck.ok) {
                return spec80ValidationResult("focusa_metacog_reflect", "/v1/metacognition/reflect", params, "metacog reflect", keyCheck.error);
            }
            const raw = keyCheck.value;
            ...
            if (!turnRangeCheck.ok) {
                return spec80ValidationResult("focusa_metacog_reflect", "/v1/metacognition/reflect", raw, "metacog reflect", turnRangeCheck.error);
            }
            ...
            if (!failureCheck.ok) {
                return spec80ValidationResult("focusa_metacog_reflect", "/v1/metacognition/reflect", raw, "metacog reflect", failureCheck.error);
            }
            ...
            return spec80Result("focusa_metacog_reflect", "/v1/metacognition/reflect", { ...req, writer_id: res.writerId || null }, res, `metacog reflect: ${String(res.body?.reflection_id || "ok")} hypotheses=${Array.isArray(res.body?.hypotheses) ? res.body.hypotheses.length : 0}\nstrategy_updates=${summarizeArray(updates, 4)}\nnext_tools=focusa_metacog_plan_adjust,focusa_metacog_doctor`, "metacog reflect", {
                kind: "ok",
    ...
    pi.registerTool({
        ...
        parameters: strictObject({
            ...
            selected_updates: Type.Optional(Type.Array(Type.String({ maxLength: SPEC81_LIMITS.updateText, description: "Selected update." }), {
        ...
        async execute(_id, params) {
            ...
            if (!keyCheck.ok) {
                return spec80ValidationResult("focusa_metacog_plan_adjust", "/v1/metacognition/adjust", params, "metacog adjust", keyCheck.error);
            }
            const raw = keyCheck.value;
            ...
            if (!reflectionCheck.ok) {
                return spec80ValidationResult("focusa_metacog_plan_adjust", "/v1/metacognition/adjust", raw, "metacog adjust", reflectionCheck.error);
            }
            ...
            if (!updatesCheck.ok) {
                return spec80ValidationResult("focusa_metacog_plan_adjust", "/v1/metacognition/adjust", raw, "metacog adjust", updatesCheck.error);
            }
            ...
            return spec80Result("focusa_metacog_plan_adjust", "/v1/metacognition/adjust", { ...req, writer_id: res.writerId || null }, res, `metacog adjust: ${String(res.body?.adjustment_id || "ok")} updates=${updatesCheck.value.length}\nnext_step_policy=${summarizeArray(res.body?.next_step_policy || updatesCheck.value, 4)}\nnext_tools=focusa_metacog_evaluate_outcome,focusa_metacog_doctor`, "metacog adjust", {
                kind: "ok",
                ids: [
    ...
    pi.registerTool({
        ...
        parameters: strictObject({
            ...
            observed_metrics: Type.Optional(Type.Array(Type.String({ maxLength: SPEC81_LIMITS.metricText, description: "Observed metric id." }), {
        ...
        async execute(_id, params) {
            ...
            if (!keyCheck.ok) {
                return spec80ValidationResult("focusa_metacog_evaluate_outcome", "/v1/metacognition/evaluate", params, "metacog evaluate", keyCheck.error);
            }
            const raw = keyCheck.value;
            ...
            if (!adjustmentCheck.ok) {
                return spec80ValidationResult("focusa_metacog_evaluate_outcome", "/v1/metacognition/evaluate", raw, "metacog evaluate", adjustmentCheck.error);
            }
            ...
            if (!metricsCheck.ok) {
                return spec80ValidationResult("focusa_metacog_evaluate_outcome", "/v1/metacognition/evaluate", raw, "metacog evaluate", metricsCheck.error);
            }
            ...
            const observed = Array.isArray(res.body?.delta_scorecard?.metrics_observed)
                ...
                : metricsCheck.value;
            return spec80Result("focusa_metacog_evaluate_outcome", "/v1/metacognition/evaluate", { ...req, writer_id: res.writerId || null }, res, `metacog evaluate: decision=${String(res.body?.result || "unknown")} promote=${boolLabel(res.body?.promote_learning)}\nobserved_metrics=${summarizeArray(observed, 4)}\nnext_tools=focusa_metacog_doctor,focusa_metacog_recent_adjustments`, "metacog evaluate", {
                kind: "ok",
                ids: [
    ...
    pi.registerTool({
        ...
        execute: async (_toolCallId, params) => {
            ...
            if (!keyCheck.ok) {
                return spec80ValidationResult("focusa_bloatgaurd_domain", "/v1/bloatgaurd/domain/{name}", params, "bloatgaurd domain", keyCheck.error);
            }
    ...
    pi.registerTool({
        ...
        execute: async (_toolCallId, params) => {
            ...
            if (!keyCheck.ok) {
                return spec80ValidationResult("focusa_bloatgaurd_tokenbloat_domain", "/v1/bloatgaurd/tokenbloat/domain/{name}", params, "bloatgaurd tokenbloat domain", keyCheck.error);
            }
            const name = encodeURIComponent(String(params.name || ""));
    ...
    pi.registerTool({
        ...
        execute: async (_toolCallId, params) => {
            ...
            if (!keyCheck.ok) {
                return spec80ValidationResult("focusa_bloatgaurd_gate_mode", "/v1/bloatgaurd/gate-modes/mode/{name}", params, "bloatgaurd gate mode", keyCheck.error);
            }
            const name = encodeURIComponent(String(params.name || ""));
    ...
    pi.registerTool({
        ...
        execute: async (_toolCallId, params) => {
            ...
            if (!keyCheck.ok)
                return spec80ValidationResult("focusa_bloatgaurd_profile", "/v1/bloatgaurd/profiles/profile/{name}", params, "bloatgaurd profile", keyCheck.error);
    ...
    pi.registerTool({
        ...
        execute: async (_toolCallId, params) => {
            ...
            if (!keyCheck.ok)
                return spec80ValidationResult("focusa_bloatgaurd_routine", "/v1/bloatgaurd/routines/routine/{name}", params, "bloatgaurd routine", keyCheck.error);
    ...
    pi.registerTool({
        ...
        execute: async (_toolCallId, params) => {
            ...
            if (!keyCheck.ok)
                return spec80ValidationResult("focusa_dxux_requirement", "/v1/dxux/requirement/{id}", params, "dxux requirement", keyCheck.error, "SCHEMA_INVALID", { required_fields: ["id"], field_types: { id: "string" }, example: { id: "DXUX-004" } });
            const idCheck = validateRequiredString("id", keyCheck.value.id, 40, {
                pattern: /^DXUX[-_]\d{3}$/i,
            });
            if (!idCheck.ok)
                return spec80ValidationResult("focusa_dxux_requirement", "/v1/dxux/requirement/{id}", params, "dxux requirement", idCheck.error, "SCHEMA_INVALID", { required_fields: ["id"], field_types: { id: "string" }, example: { id: "DXUX-004" } });
    ...
    pi.registerTool({
        ...
        execute: async (_toolCallId, params) => {
            ...
            if (!keyCheck.ok)
                return spec80ValidationResult("focusa_dxux_explain", "/v1/dxux/explain/{failure}", params, "dxux explain", keyCheck.error, "SCHEMA_INVALID", {
            ...
            if (!failureCheck.ok)
                return spec80ValidationResult("focusa_dxux_explain", "/v1/dxux/explain/{failure}", params, "dxux explain", failureCheck.error, "SCHEMA_INVALID", {
                    required_fields: ["failure"],
                    field_types: { failure: "string" },
                    example: { failure: "daemon unavailable" },
                });
    ...
    pi.registerTool({
        ...
        async execute(_id, params) {
            ...
            if (!keyCheck.ok) {
                return spec80ValidationResult("focusa_context_cognition", "/v1/context-cognition", params, "context cognition", keyCheck.error);
            }
            const projectRoot = await resolveFocusaToolProjectRoot(keyCheck.value.project_root);
            const projectRootGate = projectRootConfirmationGate(projectRoot, keyCheck.value.project_root);
            if (projectRootGate)
                return projectRootGate;
            const query = new URLSearchParams();
            query.set("project_root", String(projectRoot));
            ...
            if (workingSubpathId)
                query.set("working_subpath_id", workingSubpathId);
            const cid = keyCheck.value.continuity_id;
            if (typeof cid === "string" && cid.trim() !== "")
                query.set("continuity_id", cid.trim());
            const sid = keyCheck.value.session_id;
            if (typeof sid === "string" && sid.trim() !== "")
                query.set("session_id", sid.trim());
            const include = keyCheck.value.include_rehydrate_refs;
            if (typeof include === "boolean")
                query.set("include_rehydrate_refs", String(include));
            const res = await focusaFetchDetailed(`/context-cognition?${query.toString()}`);
            const body = res.body || {};
            if (!res.ok) {
                return blockedToolResponse("focusa_context_cognition", "trajectory", `context cognition blocked → ${explainWorkLoopResult(res, "context cognition unavailable")}`, body.failure_class || "daemon_unavailable", body, ["focusa_project_verify", "focusa_workpoint_resume", "focusa_tool_doctor"]);
            }
    ...
    pi.registerTool({
        ...
        async execute(_id, params) {
            ...
            if (!keyCheck.ok) {
                return spec80ValidationResult("focusa_context_cognition_render", "/v1/context-cognition/render", params, "context cognition render", keyCheck.error);
            }
            const projectRoot = await resolveFocusaToolProjectRoot(keyCheck.value.project_root);
            const projectRootGate = projectRootConfirmationGate(projectRoot, keyCheck.value.project_root);
            if (projectRootGate)
            ...
            const body = res.body || {};
            if (!res.ok) {
                return blockedToolResponse("focusa_context_cognition_render", "trajectory", `context cognition render blocked → ${explainWorkLoopResult(res, "render unavailable")}`, body.failure_class || "daemon_unavailable", body, ["focusa_context_cognition", "focusa_project_verify", "focusa_tool_doctor"]);
            }
    ...
    pi.registerTool({
        ...
        async execute(_id, params) {
            ...
            if (!keyCheck.ok) {
                return spec80ValidationResult("focusa_context_cognition_proof", "/v1/context-cognition/proof", params, "context cognition proof", keyCheck.error);
            }
            const projectRoot = await resolveFocusaToolProjectRoot(keyCheck.value.project_root);
            const projectRootGate = projectRootConfirmationGate(projectRoot, keyCheck.value.project_root);
            if (projectRootGate)
            ...
            const body = res.body || {};
            if (!res.ok) {
                return blockedToolResponse("focusa_context_cognition_proof", "trajectory", `context cognition proof blocked → ${explainWorkLoopResult(res, "proof unavailable")}`, body.failure_class || "daemon_unavailable", body, ["focusa_context_cognition", "focusa_project_verify", "focusa_tool_doctor"]);
            }
    ...
    pi.registerTool({
        ...
        parameters: strictObject({
            ...
            target: Type.Optional(Type.String({
                description: "Curator target string (workpoint next_slice, mission, query). Defaults to the active workpoint's next_slice/mission.",
            })),
            ...
            candidates: Type.Optional(Type.Array(Type.Object({
                ...
                evidence_ref: Type.Optional(Type.String({
                    description: "Optional evidence ref id; curator boosts candidates matching the supplied evidence_refs.",
                })),
        ...
        async execute(_id, params) {
            ...
            if (!keyCheck.ok) {
                return spec80ValidationResult("focusa_context_cognition_curate", "/v1/context-cognition/curate", params, "context cognition curate", keyCheck.error);
            }
            const projectRoot = await resolveFocusaToolProjectRoot(keyCheck.value.project_root);
            const projectRootGate = projectRootConfirmationGate(projectRoot, keyCheck.value.project_root);
            if (projectRootGate)
            ...
            const body = {
                project_root: String(projectRoot),
                continuity_id: keyCheck.value.continuity_id ?? null,
                target: keyCheck.value.target ?? null,
                token_budget: keyCheck.value.token_budget ?? 2000,
                candidates: Array.isArray(keyCheck.value.candidates)
                    ? keyCheck.value.candidates
                    : [],
                evidence_refs: Array.isArray(keyCheck.value.evidence_refs)
                    ? keyCheck.value.evidence_refs
                    : [],
            };
            ...
            const resp = res.body || {};
            if (!res.ok) {
                return blockedToolResponse("focusa_context_cognition_curate", "trajectory", `context cognition curate blocked → ${explainWorkLoopResult(res, "curate unavailable")}`, resp.failure_class || "daemon_unavailable", resp, ["focusa_context_cognition", "focusa_project_verify", "focusa_tool_doctor"]);
            }
            ...
            const toolResult = resp.details?.tool_result_v1 ||
                focusaToolResult({
                    ok: true,
                    status: "completed",
    ...
    pi.registerTool({
        ...
        parameters: strictObject({
            device_name: Type.Optional(Type.String({
                maxLength: 256,
                description: "Human-readable device name (e.g. 'operator-macbook-pro'). Defaults to 'operator-device'.",
            })),
            platform: Type.Optional(Type.String({ description: "Platform string. Default: 'macos'." })),
            daemon_base_url: Type.Optional(Type.String({
        ...
        async execute(_id, params) {
            ...
            const resp = res.body || {};
            if (!res.ok) {
                return blockedToolResponse("focusa_device_pair_start", "session_transfer", `device pair start blocked → ${explainWorkLoopResult(res, "pair start unavailable")}`, resp.failure_class || "daemon_unavailable", resp, ["focusa_project_verify", "focusa_tool_doctor"]);
            }
    ...
    pi.registerTool({
        ...
        async execute(_id, params) {
            ...
            const resp = res.body || {};
            if (!res.ok) {
                return blockedToolResponse("focusa_device_pair_complete", "session_transfer", `device pair complete blocked → ${explainWorkLoopResult(res, "pair complete unavailable")}`, resp.failure_class || "daemon_unavailable", resp, ["focusa_device_pair_status", "focusa_device_pair_list"]);
            }
    ...
    pi.registerTool({
        ...
        async execute(_id, params) {
            if (!params.code && !params.device_id) {
                return spec80ValidationResult("focusa_device_pair_status", "/v1/device/pair/status", params, "device pair status", "--code or --device-id required");
            }
            const q = new URLSearchParams();
            if (params.code)
                q.set("code", String(params.code).trim().toUpperCase());
            if (params.device_id)
                q.set("device_id", String(params.device_id));
            const res = await focusaFetchDetailed(`/device/pair/status?${q.toString()}`);
            const body = res.body || {};
            if (!res.ok) {
                return blockedToolResponse("focusa_device_pair_status", "session_transfer", `device pair status blocked → ${explainWorkLoopResult(res, "pair status unavailable")}`, body.failure_class || "daemon_unavailable", body, ["focusa_device_pair_start", "focusa_device_pair_list"]);
            }
            const status = String(body.status || "unknown");
    ...
    pi.registerTool({
        ...
        async execute(_id, params) {
            ...
            const body = res.body || {};
            if (!res.ok) {
                return blockedToolResponse("focusa_device_pair_list", "session_transfer", `device pair list blocked → ${explainWorkLoopResult(res, "pair list unavailable")}`, body.failure_class || "daemon_unavailable", body, ["focusa_tool_doctor"]);
            }
            ...
            return {
                content: [
                    ...
                    },
                    {
                        ...
                        text: devices
                            .slice(0, 5)
                            .map((d) => `  - ${d.device_id || "?"} name=${d.name || "?"} revoked=${d.revoked === true ? "yes" : "no"}`)
                            .join("\n") || "(no devices)",
                    },
                ],
                details: {
    ...
    pi.registerTool({
        ...
        async execute(_id, params) {
            if (!params.device_id) {
                return spec80ValidationResult("focusa_device_pair_revoke", "/v1/device/pair/revoke", params, "device pair revoke", "--device-id required");
            }
            const body = {
            ...
            const resp = res.body || {};
            if (!res.ok) {
                return blockedToolResponse("focusa_device_pair_revoke", "session_transfer", `device pair revoke blocked → ${explainWorkLoopResult(res, "pair revoke unavailable")}`, resp.failure_class || "daemon_unavailable", resp, ["focusa_device_pair_list", "focusa_tool_doctor"]);
            }
    ...
    pi.registerTool({
        ...
        parameters: strictObject({
            ...
            expected_selected_paths: Type.Optional(Type.Array(Type.String(), {
        ...
        async execute(_id, params) {
            ...
            if (!keyCheck.ok) {
                return spec80ValidationResult("focusa_context_cognition_curate_eval", "/v1/context-cognition/curate/eval", params, "context cognition curate eval", keyCheck.error);
            }
            const projectRoot = await resolveFocusaToolProjectRoot(keyCheck.value.project_root);
            const projectRootGate = projectRootConfirmationGate(projectRoot, keyCheck.value.project_root);
            if (projectRootGate)
            ...
            const body = {
                project_root: String(projectRoot),
                continuity_id: keyCheck.value.continuity_id ?? null,
                case_id: keyCheck.value.case_id ?? null,
                target: keyCheck.value.target ?? null,
                token_budget: keyCheck.value.token_budget ?? 2000,
                candidates: Array.isArray(keyCheck.value.candidates)
                    ? keyCheck.value.candidates
                    : [],
                expected_selected_paths: Array.isArray(keyCheck.value.expected_selected_paths)
                    ? keyCheck.value.expected_selected_paths
                    : [],
                score_threshold: keyCheck.value.score_threshold ?? 0.5,
                baseline_f1: keyCheck.value.baseline_f1 ?? 0.0,
                evidence_refs: Array.isArray(keyCheck.value.evidence_refs)
                    ? keyCheck.value.evidence_refs
                    : [],
            };
            ...
            const resp = res.body || {};
            if (!res.ok) {
                return blockedToolResponse("focusa_context_cognition_curate_eval", "trajectory", `context cognition curate eval blocked → ${explainWorkLoopResult(res, "curate eval unavailable")}`, resp.failure_class || "daemon_unavailable", resp, ["focusa_context_cognition", "focusa_project_verify", "focusa_tool_doctor"]);
            }
    ...
    pi.registerTool({
        ...
        async execute(_id, params) {
            ...
            if (!keyCheck.ok) {
                return spec80ValidationResult("focusa_context_cognition_curate_optimize", "/v1/context-cognition/curate/optimize", params, "context cognition curate optimize", keyCheck.error);
            }
            const projectRoot = await resolveFocusaToolProjectRoot(keyCheck.value.project_root);
            const projectRootGate = projectRootConfirmationGate(projectRoot, keyCheck.value.project_root);
            if (projectRootGate)
            ...
            const body = {
                project_root: String(projectRoot),
                continuity_id: keyCheck.value.continuity_id ?? null,
                module_name: keyCheck.value.module_name ?? "curator",
                prompt_artifact_ref: keyCheck.value.prompt_artifact_ref,
                eval_score: keyCheck.value.eval_score,
                baseline_score: keyCheck.value.baseline_score ?? 0.0,
                score_threshold: keyCheck.value.score_threshold ?? 0.5,
                eval_run_id: keyCheck.value.eval_run_id ?? null,
                rollback: Boolean(keyCheck.value.rollback),
            };
            ...
            const resp = res.body || {};
            if (!res.ok) {
                return blockedToolResponse("focusa_context_cognition_curate_optimize", "trajectory", `context cognition curate optimize blocked → ${explainWorkLoopResult(res, "curate optimize unavailable")}`, resp.failure_class || "daemon_unavailable", resp, ["focusa_context_cognition_curate_eval", "focusa_project_verify", "focusa_tool_doctor"]);
            }
    ...
    pi.registerTool({
        ...
        async execute(_id, params) {
            ...
            if (!keyCheck.ok) {
                return spec80ValidationResult("focusa_context_cognition_optimizer_artifacts", "/v1/context-cognition/optimizer/artifacts", params, "context cognition optimizer artifacts", keyCheck.error);
            }
            const projectRoot = await resolveFocusaToolProjectRoot(keyCheck.value.project_root);
            const projectRootGate = projectRootConfirmationGate(projectRoot, keyCheck.value.project_root);
            if (projectRootGate)
            ...
            const query = new URLSearchParams();
            query.set("project_root", String(projectRoot));
            const moduleName = keyCheck.value.module_name ?? "curator";
            query.set("module_name", String(moduleName));
            query.set("limit", String(keyCheck.value.limit ?? 10));
            const res = await focusaFetchDetailed(`/context-cognition/optimizer/artifacts?${query.toString()}`);
            const body = res.body || {};
            if (!res.ok) {
                return blockedToolResponse("focusa_context_cognition_optimizer_artifacts", "trajectory", `context cognition optimizer artifacts blocked → ${explainWorkLoopResult(res, "optimizer artifacts unavailable")}`, body.failure_class || "daemon_unavailable", body, ["focusa_project_verify", "focusa_tool_doctor"]);
            }
    ...
    pi.registerTool({
        ...
        parameters: strictObject({
            ...
            entry_surface: Type.Optional(Type.Union([Type.Literal("pi_tool"), Type.Literal("cli_command"), Type.Literal("http_route")], {
        ...
        async execute(_id, params) {
            ...
            if (!keyCheck.ok) {
                return spec80ValidationResult("focusa_call_stack_design", "/v1/call-stack/design", params, "call stack design", keyCheck.error);
            }
            const projectRoot = await resolveFocusaToolProjectRoot(keyCheck.value.project_root);
            const projectRootGate = projectRootConfirmationGate(projectRoot, keyCheck.value.project_root);
            if (projectRootGate)
            ...
            const body = res.body || {};
            if (!res.ok) {
                return blockedToolResponse("focusa_call_stack_design", "workpoint", `call stack design blocked → ${explainWorkLoopResult(res, "design unavailable")}`, body.failure_class || "daemon_unavailable", body, ["focusa_project_verify", "focusa_workpoint_resume", "focusa_tool_doctor"]);
            }
    ...
    pi.registerTool({
        ...
        async execute(_id, params) {
            ...
            if (!keyCheck.ok) {
                return spec80ValidationResult("focusa_call_stack_verify", "/v1/call-stack/verify", params, "call stack verify", keyCheck.error);
            }
            const projectRoot = await resolveFocusaToolProjectRoot(keyCheck.value.project_root);
            const projectRootGate = projectRootConfirmationGate(projectRoot, keyCheck.value.project_root);
            if (projectRootGate)
            ...
            const body = res.body || {};
            if (!res.ok) {
                return blockedToolResponse("focusa_call_stack_verify", "workpoint", `call stack verify blocked → ${explainWorkLoopResult(res, "verify unavailable")}`, body.failure_class || "daemon_unavailable", body, ["focusa_call_stack_design", "focusa_project_verify", "focusa_tool_doctor"]);
            }
    ...
    pi.registerTool({
        ...
        async execute(_id, params) {
            ...
            if (!keyCheck.ok) {
                return spec80ValidationResult("focusa_tree_recent_snapshots", "/v1/focus/snapshots/recent", params, "tree recent snapshots", keyCheck.error);
            }
            let limit = Math.trunc(Number(keyCheck.value.limit ?? 5));
            ...
            const ids = items.map((item) => item?.snapshot_id).filter(Boolean);
            return spec80Result("focusa_tree_recent_snapshots", "/v1/focus/snapshots/recent", { limit }, res, items.length > 0
                ...
                : `tree recent snapshots: total=0\nno prior snapshots available\nnext_tools=focusa_tree_snapshot_state`, "tree recent snapshots");
        },
    });
    pi.registerTool({
        ...
        parameters: strictObject({
            snapshot_reason: Type.Optional(Type.String({
                maxLength: SPEC81_LIMITS.snapshotReason,
                description: "Reason label for the new snapshot.",
            })),
            baseline_snapshot_id: Type.Optional(Type.String({
                maxLength: SPEC81_LIMITS.id,
                pattern: SPEC81_ID_PATTERN,
        ...
        async execute(_id, params) {
            ...
            if (!keyCheck.ok) {
                return spec80ValidationResult("focusa_tree_snapshot_compare_latest", "/v1/focus/snapshots/recent+create+diff", params, "tree snapshot compare latest", keyCheck.error);
            }
            const raw = keyCheck.value;
            const reasonCheck = validateOptionalString("snapshot_reason", raw.snapshot_reason, SPEC81_LIMITS.snapshotReason);
            if (!reasonCheck.ok) {
                return spec80ValidationResult("focusa_tree_snapshot_compare_latest", "/v1/focus/snapshots/recent+create+diff", raw, "tree snapshot compare latest", reasonCheck.error);
            }
            ...
            if (!baselineCheck.ok) {
                return spec80ValidationResult("focusa_tree_snapshot_compare_latest", "/v1/focus/snapshots/recent+create+diff", raw, "tree snapshot compare latest", baselineCheck.error);
            }
            ...
            if (!createRes.ok || !createRes.body?.snapshot_id) {
                return spec80CompositeResult("focusa_tree_snapshot_compare_latest", "/v1/focus/snapshots/recent+create+diff", { ...createReq, baseline_snapshot_id: baselineSnapshotId || null }, false, createRes.status, createRes.body, "tree snapshot compare latest: ok", "tree snapshot compare latest");
            }
            const newSnapshotId = String(createRes.body.snapshot_id);
            if (!baselineSnapshotId) {
                return spec80CompositeResult("focusa_tree_snapshot_compare_latest", "/v1/focus/snapshots/recent+create+diff", { ...createReq, baseline_snapshot_id: null, writer_id: createRes.writerId || null }, true, createRes.status, { snapshot_id: newSnapshotId, baseline_snapshot_id: null, diff: null }, `tree snapshot compare latest: created=${newSnapshotId}\nno prior snapshot to compare\nnext_tools=focusa_tree_recent_snapshots,focusa_tree_snapshot_state`, "tree snapshot compare latest");
            }
            ...
            return spec80CompositeResult("focusa_tree_snapshot_compare_latest", "/v1/focus/snapshots/recent+create+diff", { ...createReq, baseline_snapshot_id: baselineSnapshotId, writer_id: createRes.writerId || null }, diffRes.ok, diffRes.status, {
                snapshot_id: newSnapshotId,
                baseline_snapshot_id: baselineSnapshotId,
                diff: diffRes.body,
            }, `tree snapshot compare latest: new=${newSnapshotId} baseline=${baselineSnapshotId}\nchanged=${boolLabel(diffRes.body?.checksum_changed)} version_delta=${String(diffRes.body?.version_delta ?? "unknown")}\nnext_tools=focusa_tree_restore_state,focusa_tree_path`, "tree snapshot compare latest");
        },
    });
    pi.registerTool({
        ...
        async execute(_id, params) {
            ...
            if (!keyCheck.ok) {
                return spec80ValidationResult("focusa_metacog_recent_reflections", "/v1/metacognition/reflections/recent", params, "metacog recent reflections", keyCheck.error);
            }
            let limit = Math.trunc(Number(keyCheck.value.limit ?? 5));
            if (!Number.isFinite(limit))
                limit = 5;
            ...
            const ids = items.map((item) => item?.reflection_id).filter(Boolean);
            return spec80Result("focusa_metacog_recent_reflections", "/v1/metacognition/reflections/recent", { limit }, res, items.length > 0
    ...
    pi.registerTool({
        ...
        async execute(_id, params) {
            ...
            if (!keyCheck.ok) {
                return spec80ValidationResult("focusa_metacog_recent_adjustments", "/v1/metacognition/adjustments/recent", params, "metacog recent adjustments", keyCheck.error);
            }
            let limit = Math.trunc(Number(keyCheck.value.limit ?? 5));
            if (!Number.isFinite(limit))
                limit = 5;
            ...
            const ids = items.map((item) => item?.adjustment_id).filter(Boolean);
            return spec80Result("focusa_metacog_recent_adjustments", "/v1/metacognition/adjustments/recent", { limit }, res, items.length > 0
                ...
                : `metacog recent adjustments: total=0\nno prior adjustments available\nnext_tools=focusa_metacog_plan_adjust`, "metacog recent adjustments", {
                kind: items.length > 0 ? "ok" : "advisory",
                ids: items.length > 0
                    ? ids.slice(0, 4).map((id, idx) => ({
    ...
    pi.registerTool({
        ...
        parameters: strictObject({
            ...
            failure_classes: Type.Optional(Type.Array(Type.String({ maxLength: SPEC81_LIMITS.tagText }), {
                maxItems: SPEC81_LIMITS.failureClasses,
            })),
            selected_updates: Type.Optional(Type.Array(Type.String({ maxLength: SPEC81_LIMITS.updateText }), {
                maxItems: SPEC81_LIMITS.selectedUpdates,
            })),
            observed_metrics: Type.Optional(Type.Array(Type.String({ maxLength: SPEC81_LIMITS.metricText }), {
                maxItems: SPEC81_LIMITS.observedMetrics,
            })),
        }),
        async execute(_id, params) {
            ...
            if (!keyCheck.ok) {
                return spec80ValidationResult("focusa_metacog_loop_run", "/v1/metacognition/loop-run", params, "metacog loop run", keyCheck.error);
            }
            const raw = keyCheck.value;
            const askCheck = validateRequiredString("current_ask", raw.current_ask, SPEC81_LIMITS.currentAsk);
            if (!askCheck.ok)
                return spec80ValidationResult("focusa_metacog_loop_run", "/v1/metacognition/loop-run", raw, "metacog loop run", askCheck.error);
            const turnCheck = validateRequiredString("turn_range", raw.turn_range, SPEC81_LIMITS.turnRange, {
                pattern: SPEC81_TURN_RANGE_RE,
            });
            if (!turnCheck.ok)
                return spec80ValidationResult("focusa_metacog_loop_run", "/v1/metacognition/loop-run", raw, "metacog loop run", turnCheck.error);
            const kindCheck = validateOptionalString("kind", raw.kind, SPEC81_LIMITS.kind);
            if (!kindCheck.ok)
                return spec80ValidationResult("focusa_metacog_loop_run", "/v1/metacognition/loop-run", raw, "metacog loop run", kindCheck.error);
            const contentCheck = validateOptionalString("content", raw.content, SPEC81_LIMITS.longText);
            if (!contentCheck.ok)
                return spec80ValidationResult("focusa_metacog_loop_run", "/v1/metacognition/loop-run", raw, "metacog loop run", contentCheck.error);
            const rationaleCheck = validateOptionalString("rationale", raw.rationale, SPEC81_LIMITS.rationale);
            if (!rationaleCheck.ok)
                return spec80ValidationResult("focusa_metacog_loop_run", "/v1/metacognition/loop-run", raw, "metacog loop run", rationaleCheck.error);
            const strategyCheck = validateOptionalString("strategy_class", raw.strategy_class, SPEC81_LIMITS.strategyClass);
            if (!strategyCheck.ok)
                return spec80ValidationResult("focusa_metacog_loop_run", "/v1/metacognition/loop-run", raw, "metacog loop run", strategyCheck.error);
            if (raw.confidence !== undefined &&
                (!Number.isFinite(raw.confidence) || raw.confidence < 0 || raw.confidence > 1)) {
            ...
            if (!tagsCheck.ok)
                return spec80ValidationResult("focusa_metacog_loop_run", "/v1/metacognition/loop-run", raw, "metacog loop run", tagsCheck.error);
            const failuresCheck = validateStringArray("failure_classes", raw.failure_classes, {
                maxItems: SPEC81_LIMITS.failureClasses,
                itemMaxLength: SPEC81_LIMITS.tagText,
            });
            if (!failuresCheck.ok)
                return spec80ValidationResult("focusa_metacog_loop_run", "/v1/metacognition/loop-run", raw, "metacog loop run", failuresCheck.error);
            const selectedCheck = validateStringArray("selected_updates", raw.selected_updates, {
                maxItems: SPEC81_LIMITS.selectedUpdates,
                itemMaxLength: SPEC81_LIMITS.updateText,
            });
            if (!selectedCheck.ok)
                return spec80ValidationResult("focusa_metacog_loop_run", "/v1/metacognition/loop-run", raw, "metacog loop run", selectedCheck.error);
            const metricsCheck = validateStringArray("observed_metrics", raw.observed_metrics, {
                maxItems: SPEC81_LIMITS.observedMetrics,
                itemMaxLength: SPEC81_LIMITS.metricText,
            });
            if (!metricsCheck.ok)
                return spec80ValidationResult("focusa_metacog_loop_run", "/v1/metacognition/loop-run", raw, "metacog loop run", metricsCheck.error);
            let normalizedK = Math.trunc(Number(raw.k ?? 5));
            if (!Number.isFinite(normalizedK))
                normalizedK = 5;
            normalizedK = Math.max(1, Math.min(50, normalizedK));
            ...
            const updates = selectedCheck.value.length > 0
                ...
                : Array.isArray(reflectRes.body?.strategy_updates)
                    ? reflectRes.body.strategy_updates.map((x) => String(x))
                    : [];
    ...
    pi.registerTool({
        ...
        async execute(_id, params) {
            ...
            if (!keyCheck.ok) {
                return spec80ValidationResult("focusa_metacog_doctor", "/v1/metacognition/doctor", params, "metacog doctor", keyCheck.error);
            }
            const raw = keyCheck.value;
            const askCheck = validateRequiredString("current_ask", raw.current_ask, SPEC81_LIMITS.currentAsk);
            if (!askCheck.ok)
                return spec80ValidationResult("focusa_metacog_doctor", "/v1/metacognition/doctor", raw, "metacog doctor", askCheck.error);
            const tagsCheck = validateStringArray("scope_tags", raw.scope_tags, {
                maxItems: SPEC81_LIMITS.scopeTags,
                itemMaxLength: SPEC81_LIMITS.tagText,
            });
            if (!tagsCheck.ok)
                return spec80ValidationResult("focusa_metacog_doctor", "/v1/metacognition/doctor", raw, "metacog doctor", tagsCheck.error);
            let normalizedK = Math.trunc(Number(raw.k ?? 5));
            if (!Number.isFinite(normalizedK))
                normalizedK = 5;
            normalizedK = Math.max(1, Math.min(50, normalizedK));
            ...
            const candidates = Array.isArray(res.body?.candidates) ? res.body.candidates : [];
            const withConfidence = candidates.filter((item) => item?.confidence !== null && item?.confidence !== undefined).length;
            const top = candidates[0];
            return spec80Result("focusa_metacog_doctor", "/v1/metacognition/doctor", { current_ask: askCheck.value, scope_tags: tagsCheck.value, k: normalizedK }, {
                ok: res.ok,
                status: res.status,
                body: {
                    ...(res.body || {}),
                ...
                : `metacog doctor: candidates=0\nno usable prior signals found\nnext_tools=focusa_metacog_capture,focusa_metacog_reflect`, "metacog doctor", {
                kind: candidates.length > 0 ? "ok" : "advisory",
                ids: candidates.length > 0
    ...
    pi.registerTool({
        ...
        parameters: strictObject({
            ...
            selector: Type.Optional(Type.String({
                maxLength: 80,
                description: "Selector: window|head|path|children|neighborhood|summaries|search|recent|tags_verify.",
            })),
            ...
            fields: Type.Optional(Type.Array(Type.String({ maxLength: 80 }), {
                maxItems: 16,
            ...
            tag_mode: Type.Optional(Type.Union([
                Type.Literal("item"),
                Type.Literal("range"),
                Type.Literal("window"),
            ...
            include_full_payload: Type.Optional(Type.Boolean({
        ...
        async execute(_id, params) {
            ...
            if (!keyCheck.ok) {
                return spec80ValidationResult("focusa_traverse", "/v1/traverse", params, "traverse", keyCheck.error);
            }
            const raw = keyCheck.value;
            const surfaceCheck = validateRequiredString("surface", raw.surface, 80);
            if (!surfaceCheck.ok)
                return spec80ValidationResult("focusa_traverse", "/v1/traverse", raw, "traverse", surfaceCheck.error);
            const selectorCheck = validateOptionalString("selector", raw.selector, 80);
            if (!selectorCheck.ok)
                return spec80ValidationResult("focusa_traverse", "/v1/traverse", raw, "traverse", selectorCheck.error);
            const anchorCheck = validateOptionalString("anchor", raw.anchor, SPEC81_LIMITS.id);
            if (!anchorCheck.ok)
                return spec80ValidationResult("focusa_traverse", "/v1/traverse", raw, "traverse", anchorCheck.error);
            const queryCheck = validateOptionalString("query", raw.query, SPEC81_LIMITS.currentAsk);
            if (!queryCheck.ok)
                return spec80ValidationResult("focusa_traverse", "/v1/traverse", raw, "traverse", queryCheck.error);
            const cursorCheck = validateOptionalString("cursor", raw.cursor, 80);
            if (!cursorCheck.ok)
                return spec80ValidationResult("focusa_traverse", "/v1/traverse", raw, "traverse", cursorCheck.error);
            const fieldsCheck = validateStringArray("fields", raw.fields, { maxItems: 16, itemMaxLength: 80 });
            if (!fieldsCheck.ok)
                return spec80ValidationResult("focusa_traverse", "/v1/traverse", raw, "traverse", fieldsCheck.error);
            const tags = Array.isArray(raw.tags)
                ? raw.tags.slice(0, 32).map((tag) => {
                    if (typeof tag === "string")
                        return tag.slice(0, 240);
            ...
            if (raw.tags !== undefined && !Array.isArray(raw.tags))
                return spec80ValidationResult("focusa_traverse", "/v1/traverse", raw, "traverse", "tags must be an array of strings or TraverseTagRef objects");
            ...
            const req = {
                ...
                budget_tokens: raw.budget_tokens !== undefined
            ...
            const projectionNote = projection.fallback_to_defaults
                ...
                : omittedFields.length
                    ...
                    : "";
            return spec80Result("focusa_traverse", endpoint === "/traverse" ? "/v1/traverse" : "/v1/traverse/verify-tags", req, res, `traverse: surface=${req.surface} selector=${selector} returned=${items.length}/${String(traversal.total ?? items.length)} truncated=${Boolean(traversal.truncated)} more_available=${Boolean(traversal.more_available ?? res.body?.more_available)}${projectionNote}
...
next_tools=focusa_traverse,focusa_trajectory_view,focusa_workpoint_resume`, "traverse");
        },
    });
    ...
    pi.registerTool({
        ...
        description: "Render a surface-aware AwarenessPacket with DVS-scored visible lines, suppressed lines, metadata, next_tools, and recovery_tools, including Spec 111 preload status surfaces.",
        parameters: strictObject({
        ...
        async execute(_id, params) {
            ...
            const resultOk = Boolean(result && typeof result === "object" && result.ok === true);
            const raw = result && typeof result === "object" ? result.value : null;
            ...
            if (Array.isArray(packet.visibleLines)) {
                ...
                for (const line of lines) {
                    if (!line)
                        continue; // guard against null entries in array
                    textLines.push(`  ${line.layer || "?"} | ${line.category || "?"} | ${String(line.text || "").slice(0, 80)}`);
                }
                if (packet.visibleLines.length > 5)
                    textLines.push(`  ... +${packet.visibleLines.length - 5} more`);
            }
    ...
    pi.registerTool({
        ...
        async execute(_id, params) {
            const { session_id, max_nodes, include_full_payload } = params;
            ...
            if (include_full_payload === true)
                queryParts.push(`include_full_payload=true`);
    ...
    pi.registerTool({
        ...
        async execute(_id, params) {
            const { max_candidates, session_id } = params;
            const effectiveSessionId = String(session_id || getAttachmentRuntime().sessionFrameKey || "").trim();
            if (!effectiveSessionId) {
                return {
            ...
            const summaryNodes = nodes.filter((n) => String(n?.node_type || "").toLowerCase() === "summary").length;
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            prediction_type: Type.String({
                description: "Prediction type, e.g. next_action_success|tool_choice|release_failure|stale_state|context_relevance|token_risk|cache_hit|drift_risk|workpoint_resume_success|compaction_recovery",
            }),
            ...
            ontology_context: Type.Optional(Type.Any({
                description: "Bounded ontology refs: object_refs, action_refs, tool_refs, evidence_refs, relation_refs.",
            })),
            project_root: Type.Optional(Type.String({
                description: "Optional project root to bind prediction trajectory scope; auto-filled when omitted.",
            })),
            continuity_id: Type.Optional(Type.String({
                description: "Optional continuity id to bind prediction trajectory scope; auto-filled when omitted.",
            })),
        }),
        async execute(_id, params) {
            const p = params;
            ...
            if (!continuityId)
                return blockedToolResponse("focusa_predict_record", "prediction", "prediction record blocked → typed continuity scope required", "scope_mismatch", {}, ["focusa_workpoint_resume", "focusa_project_identity"]);
            ...
            if (!res.ok || body.authority?.status === "blocked") {
                ...
                return blockedToolResponse("focusa_predict_record", "prediction", `prediction record blocked → ${scopedResponseHuman(body, explainWorkLoopResult(res, "prediction write unavailable"))}`, failureClass, body, failureClass === "scope_mismatch"
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            ...
            continuity_id: Type.Optional(Type.String({ description: "Explicit or current workstream continuity id." })),
        }),
        async execute(_id, params) {
            const p = params;
            const projectRoot = await resolveFocusaToolProjectRoot(p.project_root);
            ...
            const continuityId = String(p.continuity_id || getContinuityId() || "").trim();
            if (!continuityId)
                return blockedToolResponse("focusa_predict_recent", "prediction", "predictions recent blocked → typed continuity scope required", "scope_mismatch", {}, ["focusa_workpoint_resume"]);
            const scope = buildProjectWorkstreamKey(projectRoot, continuityId);
            ...
            if (!res.ok || body.authority?.status === "blocked") {
                const failureClass = scopedResponseFailureClass(res, body);
                return blockedToolResponse("focusa_predict_recent", "prediction", `predictions recent blocked → ${scopedResponseHuman(body, "scoped read unavailable")}`, failureClass, body, failureClass === "scope_mismatch"
                    ? ["focusa_project_identity", "focusa_workpoint_resume"]
                    : ["focusa_tool_doctor"]);
            }
            if (!isWorkstreamKey(body.scope) || !sameWorkstream(body.scope, scope)) {
                return blockedToolResponse("focusa_predict_recent", "prediction", "predictions recent blocked → response scope differs from requested project/workstream", "scope_mismatch", body, ["focusa_project_identity", "focusa_workpoint_resume"]);
            }
    ...
    pi.registerTool({
        ...
        parameters: Type.Object({
            ...
            learning_signal_ref: Type.Optional(Type.String({ description: "Optional scoped learning signal ref." })),
            project_root: Type.Optional(Type.String({ description: "Explicit or current verified project root." })),
            continuity_id: Type.Optional(Type.String({ description: "Explicit or current workstream continuity id." })),
        }),
        async execute(_id, params) {
            ...
            const continuityId = String(p.continuity_id || getContinuityId() || "").trim();
            if (!continuityId)
                return blockedToolResponse("focusa_predict_evaluate", "prediction", "prediction evaluate blocked → typed continuity scope required", "scope_mismatch", {}, ["focusa_workpoint_resume"]);
            const scope = buildProjectWorkstreamKey(projectRoot, continuityId);
            ...
            if (!res.ok || body.authority?.status === "blocked") {
                const failureClass = scopedResponseFailureClass(res, body);
                return blockedToolResponse("focusa_predict_evaluate", "prediction", `prediction evaluate blocked → ${scopedResponseHuman(body, "scoped evaluation unavailable")}`, failureClass, body, failureClass === "scope_mismatch"
    ...
    pi.registerTool({
        ...
        async execute() {
            ...
            const profileList = profiles
                .map((profile) => `${String(profile.id || "unknown")} (${String(profile.label || "unlabeled")})`)
                .join(", ");
            const defaultProfile = String(res.body?.default_profile || "unknown");
            return spec80Result("focusa_preload_profiles", "/v1/preload/profiles", {}, res, `preload profiles → ${profiles.length} available: ${profileList || "none"}; default=${defaultProfile}`, "preload profiles unavailable", {
    ...
    const preloadProfile = Type.Optional(Type.Union([
    ...
    for (const [name, label, action, nextTools] of preloadReadTools) {
        pi.registerTool({
            ...
            async execute(_id, params) {
                ...
                const failureMessage = String(res.body?.human_readable || res.body?.error?.message || `${action} preload unavailable`);
                return spec80Result(name, `/v1/preload/${action}`, request, functionalResult, `${action} preload → ${bodyStatus}${res.body?.human_readable ? ` | ${res.body.human_readable}` : ""}`, `${action} preload failed: ${failureMessage}`, {
    ...
    pi.registerTool({
        ...
        async execute(_id, params) {
            ...
            return spec80Result("focusa_preload_write", "/v1/preload/write", request, res, `preload write → ${res.body?.target_path || p.target}`, "preload write blocked", {
                kind: res.ok ? "ok" : "blocked",
    ...
    pi.registerTool({
        ...
        async execute(_id, params) {
            ...
            return spec80Result("focusa_preload_receipt_preview", "/v1/preload/receipt-preview", request, res, `preload receipt preview → ${res.body?.status || "completed"}`, "preload receipt preview unavailable", {
                kind: res.ok ? "ok" : "blocked",
    ...
    pi.registerTool({
        ...
        async execute(_id, params) {
            ...
            return spec80Result("focusa_preload_receipt_commit", "/v1/preload/receipt-commit", request, res, `preload receipt commit → ${res.body?.status || "completed"}`, "preload receipt commit blocked", { kind: res.ok ? "ok" : "blocked", nextTools: ["focusa_receipt_verify", "focusa_preload_doctor"] });
        },
    });
    pi.registerTool({
        ...
        async execute(_id, params) {
            ...
            const continuityId = String(p.continuity_id || getContinuityId() || "").trim();
            if (!continuityId)
                return blockedToolResponse("focusa_predict_stats", "prediction", "prediction stats blocked → typed continuity scope required", "scope_mismatch", {}, ["focusa_workpoint_resume"]);
            const scope = buildProjectWorkstreamKey(projectRoot, continuityId);
            const query = scopedQueryParams(scope);
            ...
            if (!res.ok || body.authority?.status === "blocked") {
                const failureClass = scopedResponseFailureClass(res, body);
                return blockedToolResponse("focusa_predict_stats", "prediction", `prediction stats blocked → ${scopedResponseHuman(body, "scoped stats unavailable")}`, failureClass, body, failureClass === "scope_mismatch"
                    ? ["focusa_predict_recent", "focusa_workpoint_resume"]
                    : ["focusa_predict_recent", "focusa_tool_doctor"]);
            }
            return {
    ...
    pi.registerTool({
        ...
        description: "Append or project immutable Spec 138 prediction/outcome/learning/transfer authority in one typed project/workstream scope.",
        parameters: Type.Object({
        ...
        async execute(_id, params) {
            ...
            const scope = buildProjectWorkstreamKey(projectRoot, continuityId);
            const endpoint = p.action === "append" ? "/prediction-authority/events" : "/prediction-authority/projection";
    ...
    pi.registerTool({
        ...
        description: "Search the bounded Focusa capability catalog before loading full schemas. Returns ranked metadata, scope, side-effect, skill, documentation, and discovery refs so agents can select the narrowest tool under token budget.",
        parameters: Type.Object({
        ...
        async execute(_id, params) {
            ...
            const results = FOCUSA_TOOL_CONTRACTS.filter((contract) => !p.family || contract.family === p.family)
                ...
                const haystack = [
                    ...
                    .join(" ")
                    .toLowerCase();
                const score = terms.reduce((sum, term) => sum + (contract.name.includes(term) ? 5 : haystack.includes(term) ? 1 : 0), query === contract.name.toLowerCase() ? 20 : 0);
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: modelVisibleDiscoveryPayload(`tool search → ${results.length} ranked result(s)`, payload),
                    },
                ],
                details: payload,
            };
    ...
    pi.registerTool({
        ...
        description: "Cold-load one complete runtime Focusa tool definition after search. Returns strict input/output schemas, operational guidance, authority, side effects, failures, recovery, dependencies, skills, docs, and protocol bindings without loading unrelated tools.",
        parameters: Type.Object({
        ...
        async execute(_id, params) {
            ...
            if (!contract || !affordance || !definition) {
                return blockedToolResponse("focusa_tool_describe", "traversal", `tool describe blocked → unknown capability ${name}`, "not_found", { name }, ["focusa_tool_search"]);
            }
            const descriptor = {
                name: contract.name,
                family: contract.family,
    ...
    pi.registerTool({
        ...
        description: "Traverse the bounded capability dependency and likely-next graph from one tool or family. Use it to plan a valid workflow sequence without loading the complete registry or inventing dependencies.",
        parameters: Type.Object({
        ...
        async execute(_id, params) {
            ...
            let frontier = FOCUSA_TOOL_CONTRACTS.filter((item) => item.name === p.anchor || item.family === p.anchor).map((item) => item.name);
            if (!frontier.length) {
                const alternatives = [...new Set(FOCUSA_TOOL_CONTRACTS.map((item) => item.family))]
                    .sort()
                    .slice(0, 12);
                return blockedToolResponse("focusa_tool_graph", "traversal", `tool graph blocked → unknown tool or family ${p.anchor}`, "not_found", { anchor: p.anchor, valid_family_examples: alternatives }, ["focusa_tool_search", "focusa_tool_bundle"]);
            }
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: modelVisibleDiscoveryPayload(`tool graph → anchor=${p.anchor} nodes=${seen.size} edges=${edges.length}`, payload),
                    },
                ],
                details: payload,
            };
    ...
    pi.registerTool({
        ...
        description: "Load a bounded family bundle of capability metadata and optionally strict schemas. Use after search or graph traversal when one workflow needs several related tools; avoid broad all-tool prompt injection.",
        parameters: Type.Object({
        ...
        async execute(_id, params) {
            ...
            if (!items.length) {
                const alternatives = [...new Set(FOCUSA_TOOL_CONTRACTS.map((item) => item.family))]
                    .sort()
                    .slice(0, 12);
                return blockedToolResponse("focusa_tool_bundle", "traversal", `tool bundle blocked → unknown family ${p.family}`, "not_found", { family: p.family, valid_family_examples: alternatives }, ["focusa_tool_search"]);
            }
            const payload = {
            ...
            return {
                content: [
                    {
                        type: "text",
                        text: modelVisibleDiscoveryPayload(`tool bundle → family=${p.family} count=${items.length}`, payload),
                    },
                ],
                details: payload,
            };
    ...
    pi.registerTool({
        ...
        description: "Read a compact, versioned Focusa Agent Card for cross-harness discovery. Returns interfaces, auth methods, progressive-discovery entry points, capability families, registry digest guidance, and extended-card routes without loading full schemas.",
        parameters: Type.Object({
        ...
        async execute(_id, params) {
            ...
            const registryDigest = createHash("sha256")
                .update(JSON.stringify(FOCUSA_TOOL_CONTRACTS.map((item) => ({
                name: item.name,
                family: item.family,
            ...
            const card = {
                ...
                description: "Agent-first cognitive infrastructure with scoped Workpoints, Trajectory, evidence, recovery, browser interoperability, and cross-harness contracts.",
    ...
    pi.registerTool({
        ...
        description: "Validate and govern a UIAI or WebMCP browser capability manifest. Binds page tools to one session and origin, treats page safety annotations as untrusted, requires confirmation/evidence for mutation, and returns Focusa browser capability descriptors without executing them.",
        parameters: Type.Object({
        ...
        async execute(_id, params) {
            ...
            const body = res.body || {};
            if (!res.ok) {
                return blockedToolResponse("focusa_browser_capabilities_intake", "browser_interop", `browser capability intake blocked → ${scopedResponseHuman(body, "manifest rejected")}`, (body.failure_class || "validation_rejected"), body, ["focusa_browser_diagnostics_intake", "focusa_tool_doctor"]);
            }
            return {
                content: [
                    {
    ...
    pi.registerTool({
        ...
        description: "Dry-run, apply, inspect, or roll back granular legacy ontology scope migration. Apply/rollback require explicit confirmation and per-record evidence; ownership is never inferred.",
        parameters: Type.Object({
            action: Type.Union([
            ...
            selections: Type.Optional(Type.Array(Type.Object({
        ...
        async execute(_id, params) {
            ...
            if (mutation && params.confirm !== true) {
                return blockedToolResponse("focusa_ontology_scope_migration", "ontology", `ontology scope migration ${params.action} blocked → explicit confirm=true required`, "approval_required", { action: params.action, canonical: false, mutation: true }, ["focusa_ontology_scope_migration", "focusa_project_verify"]);
            }
            ...
            const body = res.body || {};
            if (!res.ok) {
                return blockedToolResponse("focusa_ontology_scope_migration", "ontology", `ontology scope migration blocked → ${scopedResponseHuman(body, "request rejected")}`, (body.failure_class || "validation_rejected"), body, ["focusa_project_verify", "focusa_tool_doctor"]);
            }
            return {
                content: [
                    {
    ...
    pi.registerTool({
        ...
        description: "Build the governed UIAI/WebMCP sequence for one browser operation before action. Returns health, read/source, diagnostics, snapshot refs, mutation confirmation, bound execution, evidence intake, Workpoint linkage, and session cleanup steps.",
        parameters: Type.Object({
        ...
        async execute(_id, params) {
            ...
            const body = res.body || {};
            if (!res.ok) {
                return blockedToolResponse("focusa_browser_workflow_plan", "browser_interop", `browser workflow plan blocked → ${scopedResponseHuman(body, "plan rejected")}`, (body.failure_class || "validation_rejected"), body, ["focusa_browser_diagnostics_intake", "focusa_tool_doctor"]);
            }
            return {
                content: [
                    {
