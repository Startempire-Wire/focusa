...
function operationPolicyForContract(contract) {
    ...
    const read = /^(read|advisory|local_note|read_only|none)/.test(profile) ||
    ...
    const operation_class = recovery
        ? "recovery"
        : read
            ? "read"
            : "value_mutation";
    const capability_family = recovery
        ? "account_recovery"
        : read
            ? "read_projection"
            : lowerName.includes("silent_sessions")
                ? "automation"
                : "base_focusa";
    const commercial_treatment = capability_family === "account_recovery"
        ? "always_available"
...
const AGENT_RUNTIME_TOOL_CONTRACTS = [
    [
...
export const FOCUSA_TOOL_CONTRACTS = [
    ...
    },
    {
        ...
        purpose: "Dry-run, apply, inspect, and roll back granular evidence-backed migration of quarantined legacy ontology records into one verified workstream.",
    ...
    },
    {
        ...
        purpose: "Rank explicit, active-worktree, canonical-parent, marker/Beads, persisted-session, and bounded parent-directory project candidates; fail closed on ambiguity before trusting project-bound context.",
        family: "project_identity",
        ...
        live_check: "contract_static plus /v1/project/identity binding_candidates, worktree-parent relation, ambiguity, and quorum status",
    ...
    },
    {
        ...
        purpose: "Save, continue, or Spec130-roll over a long Focusa/Pi work session with explicit source_scope/target_scope or target_continuity_id, source/target session ids, checkpoint/packet refs, and rollover action.",
        family: "workpoint",
        ...
        parity_status: "full",
        exemptions: [],
        live_check: "contract_static plus Pi tool typecheck and Spec104 static check that session_transfer carries source_scope,target_scope,target_continuity_id,session ids,packet refs,rollover_action and does not call ensureContinuityId",
    ...
    },
    {
        ...
        purpose: "Verify expected project identity fields against ranked worktree/session candidates and surface ambiguity or project/continuity mismatches without mutating Focusa state.",
        family: "project_identity",
    ...
    },
    {
        ...
        purpose: "Preview, apply, inspect, or repair an idempotent local project-discipline baseline with explicit Git/task choices, receipts, rollback, and Project Genesis handoff.",
        family: "project_identity",
    ...
    },
    {
        ...
        purpose: "Stage, resume, inspect, or atomically commit the verified project journey from HLT and specification through tasks, first Workpoint, coordination, and readiness receipt.",
        family: "project_identity",
    ...
    },
    {
        ...
        purpose: "Read, commit, revise, observe, forecast, or preflight scoped temporal claims with evidence, confidence, uncertainty, freshness, and no fabricated urgency.",
        family: "trajectory",
    ...
    },
    {
        ...
        purpose: "Record a crystallized architectural decision in Focus State. Use focusa_scratch for working notes first. Decisions are ONE sentence (<=160 chars) \u2014 architectural choices only, not task lists.",
        family: "focus_state",
    ...
    },
    {
        ...
        purpose: "Thin daemon-native Spec133 API client for exact session/run status, bounded observation, steering, controls, config, receipts, capabilities, and legacy action compatibility; process-control failures return failure_class=process_control_failed with receipt-backed recovery.",
        family: "work_loop",
        ...
        core_surface: "Daemon API facade; canonical authority, lifecycle, model, health, persistence and recovery remain server-side",
        doc_path: "docs/focusa-tools/tools/focusa_silent_sessions.md",
        result_envelope: "tool_result_v1",
        side_effect_profile: "daemon_api_control",
        parity_status: "full",
        exemptions: [],
        live_check: "contract_static plus daemon route conformance; mutations require exact target, durable approval and idempotency",
        scope_requirement: { kind: "read", route_family: "auto" },
...
export function findFocusaToolContract(name) {
    return FOCUSA_TOOL_CONTRACTS.find((contract) => contract.name === name);
}
const FAMILY_NEXT_TOOLS = {
...
function invocationFor(contract) {
    if (contract.cli_commands?.length)
        return contract.cli_commands[0];
    return `${contract.name} { ... }`;
}
function mutabilityFor(sideEffectProfile) {
    ...
    if (lower.includes("advisory"))
        return ["advisory projection only; does not choose or execute work"];
    return ["read-only or summary-first path; still inspect canonical/degraded status"];
}
export function buildFocusaToolAffordanceCatalog(contracts = FOCUSA_TOOL_CONTRACTS) {
