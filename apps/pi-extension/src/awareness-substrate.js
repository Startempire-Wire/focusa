...
import { getAttachmentRuntime, getScopedWorkpointPacket, isProjectRootAuthoritySafe, normalizeProjectRoot, focusaFetch, getLastProjectIdentity, } from "./state.js";
// ─── Helpers ─────────────────────────────────────────────────────────────────
function contextTierFromString(tier) {
    if (tier === "hard")
        return "critical";
    if (tier === "auto")
        return "high";
...
export async function gatherAwarenessInput(surface) {
    const scopedPacket = getScopedWorkpointPacket();
    const projectRoot = normalizeProjectRoot(scopedPacket?.project_root || getAttachmentRuntime().sessionCwd || process.cwd());
            ...
            if (tv && tv.ok) {
                trajectoryView = {
                    ...
                    waypoints: Array.isArray(tv.waypoints)
                        ? tv.waypoints
                            .map((item) => typeof item === "string" ? item : String(item?.title || item?.desired_state_delta || ""))
                            .filter(Boolean)
                        : [],
                    clarityGate: (tv.clarity_gate || tv.clarityGate || "provisional"),
    ...
    const activeWorktreeRoot = normalizeProjectRoot(cachedIdentity.active_worktree_root || cachedIdentity.working_context?.active_worktree_root || projectRoot);
    const workingSubpathId = String(cachedIdentity.working_context?.working_subpath?.working_subpath_id || "primary");
    return {
        projectIdentity: {
            projectRoot,
            activeWorktreeRoot,
...
// ─── Tool graph gatherer ─────────────────────────────────────────────────────
async function gatherToolGraph() {
    const families = {};
    const sideEffectProfiles = {};
    const nextToolsByFamily = {};
    const allNextTools = [];
    const allRecoveryTools = [];
    try {
            ...
            for (const contract of tc.contracts) {
                ...
                if (Array.isArray(contract.next_tools)) {
                    ...
                    for (const nt of contract.next_tools) {
                        if (!allNextTools.includes(nt))
                            allNextTools.push(nt);
                    }
                }
                if (contract.parity_status === "full" || contract.parity_status === "domain") {
                    if (!allRecoveryTools.includes(name))
                        allRecoveryTools.push(name);
                }
            }
        }
    }
...
function selectMode(input) {
    ...
    if (!projectRootSafety.safe)
        return "standard";
    if (workpointResume?.actionAuthority === false)
        return "standard";
    if (surface === "post_compaction")
        return "standard";
    if (surface === "warning")
        return "standard";
    if (currentAsk.match && currentAsk.match(/architecture|design|explain|how/))
        return "rich";
    if (contextPressure.tier === "critical" || contextPressure.tier === "high")
        return "minimal";
    if (workpointResume?.canonical === true)
        return "minimal";
    if (surface === "uiai_bridge")
        return "standard";
    return "standard";
}
...
function makeLine(partial) {
    ...
    const dvs = authorityValue * DVS_WEIGHTS.authorityValue +
...
// ─── Line scorer ─────────────────────────────────────────────────────────────
function scoreLines(lines, mode, surface) {
    ...
    return lines
        ...
        if (line.authorityValue >= 8)
            return true;
        return line.dvs >= cutoff;
    });
}
// ─── Tool-family selector ────────────────────────────────────────────────────
export function selectTopTools(input, count = 3) {
    const candidates = [];
...
function sideEffectToRisk(profile) {
    if (profile.includes("write_state") || profile.includes("control_state"))
        return "risky";
    if (profile.includes("write_"))
        return "moderate";
    return "safe";
}
// ─── Context pressure dedupe ─────────────────────────────────────────────────
export function shouldShowPressureWarning(state, input) {
    const now = Date.now();
...
export function updateCadenceState(state, input, showed) {
    const now = Date.now();
    if (!state) {
        return {
...
export async function buildAwarenessPacket(surface) {
    ...
    if (input.workpointResume?.canonical)
        authorityScore += 30;
    if (input.projectRootSafety.safe)
        authorityScore += 20;
    ...
    if (input.workpointResume?.canonical)
        freshnessScore += 15;
    if (input.trajectoryView?.canonical)
        freshnessScore += 10;
    ...
    return {
        ...
        mode: input.mode,
        surface,
        status: input.projectRootSafety.safe && input.workpointResume?.canonical
            ? "fresh"
...
export function renderAwarenessPacketText(packet) {
    const authority = packet.visibleLines.find((line) => line.category === "workpoint" || line.category === "authority");
