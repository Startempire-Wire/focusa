...
import { DEFAULT_COMPACTION_SETTINGS, estimateTokens, findCutPoint, } from "@earendil-works/pi-coding-agent";
import { providerCompactionCapabilities, } from "./provider-compaction-capabilities.js";
import { contextPressureTelemetry } from "./context-pressure-telemetry.js";
import { applyCompactionPolicyQuarantine, } from "./compaction-policy-selector.js";
import { observeFrozenCompactionOutcome, prewarmCompactionPolicy, selectFrozenCompactionPolicy, } from "./compaction-policy-adapter.js";
import { evaluateCompactionOutcome, } from "./compaction-outcome-evaluator.js";
import { currentAttachmentKey, focusaFetch, focusaPost, runWithAttachmentRuntime } from "./state.js";
import { emptyCompactionAuthorityProjection, reduceCompactionAuthorityEvents, } from "./compaction-authority-projection.js";
...
export const DEFAULT_PROACTIVE_COMPACTION_POLICY = {
    enabled: true,
...
const INSTRUCTIONS = "Preserve the current user ask, project_root + continuity_id authority, Workpoint and Trajectory authority, verified evidence handles, blockers, exact next action, and do-not-drift boundaries. Keep stable instructions verbatim where practical; summarize only older conversation detail.";
function selectionOwner(route) {
    if (route === "no_op")
        return "none";
    if (route === "rollover")
        return "operator";
...
function applyOperatorOverride(selection, override) {
    if (!override)
        return selection;
    return {
        ...selection,
...
function policyOverride(value) {
    const candidate = value?.policy?.operator_override ?? value?.operator_override;
    return candidate &&
        typeof candidate.receipt_id === "string" &&
        ["no_op", "curate_context", "checkpoint", "summarize", "native_compact", "rollover"].includes(candidate.route)
        ? candidate
        : undefined;
}
function piSupportsToolBoundaryCompaction() {
    return Boolean(globalThis[PI_TOOL_BOUNDARY_COMPACTION_SYMBOL]);
}
function processCompactionLease() {
...
export function requestCoordinatedCompaction(ctx, request) {
    return processCompactionLease().request?.(ctx, request) ?? "coordinator_unavailable";
}
export function proactiveCompactionPolicy(config) {
    if (!config)
        return { ...DEFAULT_PROACTIVE_COMPACTION_POLICY };
    return {
...
export function proactiveCompactionDecision(usage, policy = DEFAULT_PROACTIVE_COMPACTION_POLICY) {
    const tokens = usage?.tokens ?? null;
    const contextWindow = Math.max(0, usage?.contextWindow ?? 0);
    const reserveTokens = contextWindow > 0
        ? Math.min(Math.floor(contextWindow / 2), Math.max(policy.reserveTokens, Math.ceil(contextWindow * (policy.reservePercent / 100))))
        : 0;
    const absoluteCap = policy.tokenCap > 0 ? policy.tokenCap : Number.MAX_SAFE_INTEGER;
    const triggerAtTokens = contextWindow > 0
        ? Math.max(1, Math.min(contextWindow - reserveTokens, Math.ceil(contextWindow * (policy.triggerPercent / 100)), absoluteCap))
        : 0;
    const percent = tokens !== null && contextWindow > 0 ? Math.round((tokens / contextWindow) * 10_000) / 100 : null;
    if (!policy.enabled) {
        return {
            trigger: false,
            tokens,
 ...
 */
export function evaluateProactiveCompactionEligibility(entries, contextWindow) {
    if (entries.length === 0)
        return ineligible("empty_session", true);
    if (entries.at(-1)?.type === "compaction")
        return ineligible("already_compacted", true);
    ...
    const cutPoint = findCutPoint([...entries], boundaryStart, entries.length, DEFAULT_COMPACTION_SETTINGS.keepRecentTokens);
    ...
    const maxSummaryTokens = Math.ceil(DEFAULT_COMPACTION_SETTINGS.reserveTokens * (cutPoint.isSplitTurn ? 1.3 : 0.8));
    return evaluateEstimatedRoi(compactableTokens, contextWindow, maxSummaryTokens);
}
function evaluateExactPreparation(messagesToSummarize, turnPrefixMessages, contextWindow, reserveTokens) {
    const compactableTokens = [...messagesToSummarize, ...turnPrefixMessages].reduce((total, message) => total + estimateTokens(message), 0);
    const maxSummaryTokens = Math.ceil(reserveTokens * (turnPrefixMessages.length > 0 ? 1.3 : 0.8));
    return evaluateEstimatedRoi(compactableTokens, contextWindow, maxSummaryTokens);
}
function evaluateEstimatedRoi(compactableTokens, contextWindow, maxSummaryTokens) {
...
function ineligible(reason, terminal, compactableTokens = 0, estimatedOverheadTokens = 0) {
    return {
        eligible: false,
        terminal,
        reason,
...
function estimateEntryRange(entries, start, end) {
    let total = 0;
    for (let index = Math.max(0, start); index < Math.max(start, end); index += 1) {
        ...
        if (entry?.type === "custom_message")
            total += Math.ceil(JSON.stringify(entry.content).length / 4);
...
export function registerAutoCompaction(pi, getPolicy = () => DEFAULT_PROACTIVE_COMPACTION_POLICY, getConfig = () => undefined) {
    ...
    if (processLease.owner) {
        if (processLease.owner.moduleLoadId === MODULE_LOAD_ID) {
            if (!processLease.duplicateDiagnosticEmitted) {
                ...
                console.warn(`[focusa] duplicate extension suppressed; active compaction owner=${processLease.owner.registrationId} build=${processLease.owner.extensionBuild} source=${processLease.owner.registrationSource}. Remove the duplicate Focusa installation and reload Pi.`);
            }
            return false;
        }
    ...
    const ownsRegistrationLease = () => processLease.owner?.registrationId === registrationId;
    let inFlight = false;
    let lastAttemptAt;
    ...
    let activeEpoch;
    let activeRequest;
    let terminalNoopContextKey;
    let lastNoticeKey;
    const setActiveEpoch = (epoch) => {
        activeEpoch = epoch;
        if (ownsRegistrationLease())
            processLease.activeEpoch = epoch;
    };
    const createEpoch = (ctx, triggerClass, contextWindow, attempt = 1, retryOfEpochId) => {
    ...
    const persist = (kind, details = {}, epoch = activeEpoch) => {
        try {
            const event = {
    ...
    const recordOutcome = (ctx, epoch, outcome) => {
        if (!epoch.outcomeBaseline)
            return;
        ...
        if (evaluation.rollbackRequired) {
            persist("policy_rollback_required", {
    ...
    pi.registerCommand("focusa-compaction-policy", {
        ...
        handler: async (args, ctx) => {
            ...
            if (ctx.hasUI)
                ctx.ui.notify(text, response ? "info" : "warning");
        },
    });
    const notifyOnce = (ctx, key, message, level) => {
        if (lastNoticeKey === key)
            return;
        lastNoticeKey = key;
        if (ctx.hasUI)
            ctx.ui.notify(message, level);
    };
    const stopCompactionHeartbeat = (ctx) => {
        if (heartbeatTimer)
    ...
    const startCompactionHeartbeat = (ctx, epoch, contextPercent) => {
        ...
        const render = () => {
            ...
            if (ctx.hasUI) {
                ctx.ui.setStatus("focusa-auto-compaction", `⏳ compacting · ${elapsedSeconds}s · ${contextPercent ?? "?"}% context · attempt ${epoch.attempt}`);
                if (elapsedSeconds >= nextVisibleNoticeSeconds) {
                    ctx.ui.notify(`⏳ Focusa compaction still running · ${elapsedSeconds}s · attempt ${epoch.attempt}`, "info");
    ...
    const releaseProcessAttempt = (epochId) => {
        if (!epochId || processLease.inFlightEpochId !== epochId)
            return;
        processLease.inFlightEpochId = undefined;
        processLease.attemptOwnerId = undefined;
    };
    const clearProcessRetry = () => {
        if (processLease.retryOwnerId === registrationId)
            processLease.retryOwnerId = undefined;
    };
    ...
    const attemptCompaction = (ctx, usageBefore) => {
        if (!activeEpoch)
            return;
        if (!ownsRegistrationLease()) {
        ...
        const withinAttachment = (operation) => attachmentKey ? runWithAttachmentRuntime(attachmentKey, operation) : operation();
        const bindAttachmentCallback = (callback) => (...args) => withinAttachment(() => callback(...args));
        ctx.compact({
            ...
            onComplete: bindAttachmentCallback((result) => {
                if (invokedEpoch.settlement) {
                    persist("secondary_duplicate_settlement", { first_settlement: invokedEpoch.settlement, duplicate_settlement: "complete" }, invokedEpoch);
                    return;
                }
                ...
                persist("attempt_completed", {
                    tokens_before: result.tokensBefore,
                    tokens_after: tokensAfter,
                    saved_tokens: savedTokens,
                    net_positive: savedTokens === undefined
                        ? undefined
                ...
                notifyOnce(ctx, `complete:${result.firstKeptEntryId}`, "Focusa compacted context proactively.", "info");
            ...
            onError: bindAttachmentCallback((error) => {
                ...
                if (invokedEpoch.settlement) {
                    persist("secondary_duplicate_settlement", {
                ...
                const exactRejection = failedEpoch.exactEligibility?.eligible === false ? failedEpoch.exactEligibility : undefined;
                ...
                stopCompactionHeartbeat(ctx);
                persist(exactRejection ? "eligibility_rejected" : "attempt_failed", {
                ...
                if (exactRejection || isTerminalNoopError(message)) {
                    ...
                    consecutiveTransientFailures = 0;
                    setActiveEpoch(undefined);
                    notifyOnce(ctx, `noop:${terminalNoopContextKey}:${exactRejection?.reason ?? "nothing_to_compact"}`, `Focusa deferred proactive compaction: ${exactRejection?.reason ?? "nothing_to_compact"}.`, "warning");
                ...
                if (retryableFailure && consecutiveTransientFailures < maxTransientRetries) {
                    ...
                    persist("retry_scheduled", {
                        primary_error: message,
                        retry_delay_ms: retryDelay,
                        next_attempt: consecutiveTransientFailures + 1,
                    }, failedEpoch);
                    notifyOnce(ctx, `retry:${failedEpoch.epochId}:${consecutiveTransientFailures + 1}`, `Focusa compaction attempt ${failedEpoch.attempt} failed: ${message.slice(0, 240)}. Retrying in ${Math.ceil(retryDelay / 1000)}s.`, "warning");
                    setActiveEpoch(createEpoch(ctx, failedEpoch.triggerClass, failedEpoch.contextWindow, consecutiveTransientFailures + 1, priorEpochId));
                ...
                if (retryableFailure) {
                    ...
                    persist("native_recovery_deferred_to_pi", {
                ...
                const recoveryInstruction = retryableFailure
                    ...
                    : ".";
                setActiveEpoch(undefined);
                notifyOnce(ctx, `failed:${failedEpoch.contextKey}:${message}`, `Focusa proactive compaction failed after ${failedAttempts} attempt(s): ${message}${recoveryInstruction}`, retryableFailure ? "warning" : "error");
    ...
    const maybeCompact = (ctx, request = {
        ...
        const selectedPolicy = applyOperatorOverride(applyCompactionPolicyQuarantine(candidatePolicy, processLease.projection.quarantinedPolicyKeys, processLease.projection.rollbackRoute), processLease.operatorOverride);
        const policySelection = decision.trigger &&
        ...
        const successCooldownRemaining = processLease.lastSuccessfulCompactionAt
            ...
            : 0;
        if (request.triggerClass !== "hard_pressure" &&
        ...
        if (policySelection.route === "rollover") {
            notifyOnce(ctx, `rollover:${policySelection.deterministicKey}`, "Focusa cannot safely compact with the current provider capability; checkpoint and rollover are required.", "warning");
            return "ineligible";
        }
        if (!usage)
            return "suppressed";
        ...
        const contextKey = contextEpochKey(ctx);
        if (terminalNoopContextKey === contextKey)
            return "ineligible";
        if (lastAttemptAt !== undefined && Date.now() - lastAttemptAt < policy.cooldownMs) {
            notifyOnce(ctx, `cooldown:${contextKey}`, `Focusa context pressure is ${(decision.percent ?? 0).toFixed(1)}%; proactive compaction is cooling down.`, "warning");
            return "suppressed";
        }
        const eligibility = evaluateProactiveCompactionEligibility(ctx.sessionManager.getBranch(), usage.contextWindow);
        if (!eligibility.eligible) {
            ...
            setActiveEpoch(undefined);
            notifyOnce(ctx, `preflight:${contextKey}:${eligibility.reason}`, `Focusa deferred proactive compaction: ${eligibility.reason}.`, "warning");
            return "ineligible";
        }
        ...
        persist("native_compaction_requested", {
    ...
    pi.on("session_before_compact", async (event, ctx) => {
        ...
        if (externalNativeInvocation) {
            const usage = ctx.getContextUsage();
            const nativeReason = event.reason;
            const reason = String(nativeReason || "").toLowerCase();
            const triggerClass = reason === "manual"
                ? "manual"
                : reason.includes("overflow")
                    ? "provider_overflow"
                    : "predicted_pressure";
            const observedEpoch = createEpoch(ctx, triggerClass, usage?.contextWindow ?? event.preparation.tokensBefore);
        ...
        if (["manual", "provider_overflow"].includes(activeEpoch.triggerClass))
            return;
        const exactEligibility = evaluateExactPreparation(event.preparation.messagesToSummarize, event.preparation.turnPrefixMessages, activeEpoch.contextWindow, event.preparation.settings.reserveTokens);
    ...
    pi.on("session_shutdown", async () => {
        ...
        if (!processLease.inFlightEpochId) {
            ...
            if (processLease.owner)
                processLease.owner.nativeSession = undefined;
            processLease.duplicateDiagnosticEmitted = false;
        }
    });
    return true;
}
function contextEpochKey(ctx) {
...
function isTerminalNoopError(message) {
    return /nothing to compact|already compacted/i.test(message);
}
function compactionFailureClass(message, exactRejection) {
    if (exactRejection)
        return "eligibility_rejection";
    if (isTerminalNoopError(message))
    ...
    if (isTransientCompactionError(message))
        return "primary_transport";
    return "primary";
}
function isTransientCompactionError(message) {
    return /websocket|network|socket|timeout|timed out|connection|temporar|rate.?limit|429|502|503|504/i.test(message);
}
function isRetryableCompactionError(message) {
