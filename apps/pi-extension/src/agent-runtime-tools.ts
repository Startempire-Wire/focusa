...
async function post(path: string, body: Payload) {
  return toolResult(
    await focusaFetch(path, {
      method: "POST",
...
function projectQuery(projectRoot: string, maxSourceBytes?: number) {
...
export function registerAgentRuntimeTools(pi: ExtensionAPI) {
  pi.registerTool({
    ...
    async execute(_id, params) {
      const p = params as { project_root: string; max_source_bytes?: number };
  ...
  pi.registerTool({
    ...
    async execute(_id, params) {
      const p = params as { project_root: string; claim_id: string; max_source_bytes?: number };
      const response = await focusaFetch(
        `/agent-runtime/instructions/claims?${projectQuery(p.project_root, p.max_source_bytes)}`
      );
      const claim = response?.claims?.find((candidate: any) => candidate?.claim_id === p.claim_id);
      return toolResult(
        claim
  ...
  pi.registerTool({
    ...
    async execute(_id, params) {
      return post("/agent-runtime/instructions/simulate", params as Payload);
    },
  });
  ...
  pi.registerTool({
    ...
    async execute(_id, params) {
      const p = params as { constitution_id: string; request: Payload };
  ...
  pi.registerTool({
    ...
    async execute(_id, params) {
      return post("/agent-runtime/compile/system-prompt", (params as { request: Payload }).request);
    },
  });
  ...
  pi.registerTool({
    ...
    async execute(_id, params) {
      const p = params as { baseline: any; candidate: any };
      return toolResult({
  ...
  pi.registerTool({
    ...
    async execute(_id, params) {
      return post("/agent-runtime/delivery/preview", (params as { request: Payload }).request);
    },
  });
  ...
  pi.registerTool({
    ...
    async execute(_id, params) {
      const p = params as { request: Payload; confirmed: boolean };
  ...
  pi.registerTool({
    ...
    async execute(_id, params) {
      return post("/agent-runtime/delivery/verify", (params as { request: Payload }).request);
    },
  });
