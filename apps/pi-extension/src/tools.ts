...
export function registerTools(pi: ExtensionAPI) {
  ...
  pi.registerTool({
    ...
    async execute(_id, params) {
      ...
      const text = result.ok ? `trajectory assess → gaps=${Array.isArray(b.gaps) ? b.gaps.length : 0} action=${String(b.recommended_action || "unknown")} canonical=${b.canonical === true}${Array.isArray(b.gaps) && b.gaps.length > 0 ? "\n" + b.gaps.slice(0, 3).map((g: any, i: number) => `  gap[${i}]: ${String(g.gap_ref || "unknown")} severity=${String(g.severity || "unknown")} → ${String(g.recommended_action || "none")}`).join("\n") : ""}` : `trajectory assess blocked → ${explainWorkLoopResult(result, "assess failed")}`;
