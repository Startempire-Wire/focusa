...
const root = path.resolve(here, "../.."); // the deployed extension dir
const extensionSource = fs.readFileSync(path.join(repoRoot, "apps/pi-extension/src/compaction.ts"), "utf8");
const resumeProjectionSource = fs.readFileSync(
  path.join(repoRoot, "apps/pi-extension/src/compaction-resume-projection.ts"),
  "utf8"
);
const stateSource = fs.readFileSync(path.join(repoRoot, "apps/pi-extension/src/state.ts"), "utf8");
const turnsSource = fs.readFileSync(path.join(repoRoot, "apps/pi-extension/src/turns.ts"), "utf8");
const apiSource = fs.readFileSync(path.join(repoRoot, "crates/focusa-api/src/routes/compaction.rs"), "utf8");
const fallbackDoc = fs.readFileSync(path.join(repoRoot, "docs/current/COMPACTION_FALLBACKS.md"), "utf8");
const spec142 = fs.readFileSync(
  path.join(repoRoot, "docs/142-focusa-seamless-pi-continuation-and-workflow-dependency-onboarding-spec.md"),
  "utf8"
);
const spec130a = fs.readFileSync(
  path.join(repoRoot, "docs/130a-zero-waste-compaction-performance-addendum.md"),
  "utf8"
);
const piPatchSource = fs.readFileSync(
  path.join(repoRoot, "apps/pi-extension/scripts/patch-pi-shrinkwrap-dependencies.mjs"),
  "utf8"
);
