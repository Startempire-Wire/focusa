...
const manifest=JSON.parse(readFileSync(`${root}/docs/contracts/48-49-focusa-pi-menu-inventory.json`,"utf8"));
const digest=createHash("sha256").update(commands+session).digest("hex");
...
for(const item of settings){
  assert.ok(item.owner&&item.user_job&&item.placement&&item.rationale&&item.help&&item.scope&&item.risk&&item.interaction_test);
...
assert.equal(manifest.items.filter(x=>["select","confirm","input","custom"].includes(x.kind)).length,3);
assert.ok(manifest.after.simple_settings<manifest.baseline.simple_settings);
assert.equal(manifest.after.simple_settings,8);
assert.deepEqual(Object.keys(manifest.migration).sort(),["otaProfile","vitalInfoPromptSurfaces","workLoopStatusHeartbeatMs"].sort());
...
console.log(`menu audit gate: PASS (${manifest.items.length} entries; ${settings.length} setting placements)`);
