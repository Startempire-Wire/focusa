...
for(const marker of ["focusaFetchDetailed","/silent-sessions","run_id","generation","approval_id","idempotency_key",'authority: "daemon"','parity: "full"']) assert(facade.includes(marker),marker);
for(const forbidden of ["defaultSilentSessionCommand","silentSessionExec","tmux","execFileSync","/tmp/"]) assert(!facade.includes(forbidden),forbidden);
