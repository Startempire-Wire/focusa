...
```text
focusa_project_card -> focusa_project_card_outcome
focusa_project_card_outcome -> focusa_predict_record
focusa_predict_record -> focusa_workpoint_checkpoint
```

## Minimal path

1. Call `focusa_project_card` with only required bounded inputs.
2. Call `focusa_project_card_outcome` with only required bounded inputs.
3. Call `focusa_predict_record` with only required bounded inputs.
4. Call `focusa_workpoint_checkpoint` with only required bounded inputs.
