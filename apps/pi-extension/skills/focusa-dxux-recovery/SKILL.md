---
name: focusa-dxux-recovery
description: "Use for dxux recovery authority, routing, evidence, and recovery."
---

# Focusa Dxux Recovery

Use for dxux recovery authority, routing, evidence, and recovery.

## Progressive disclosure

1. Load this core file only when its trigger matches.
2. Read `references/01-focusa-dxux-recovery-runbook.md` only for the selected workflow.
...
- dxux recovery authority, routing, evidence, and recovery
...
1. `focusa_dxux_report`
2. `focusa_dxux_requirement`
3. `focusa_dxux_explain`
4. `focusa_dxux_digest`
5. `focusa_tool_doctor`
...
- workflow: `focusa-project-scope` → `focusa-dxux-recovery` → `focusa-workpoint` → `focusa-evidence-outcomes`
