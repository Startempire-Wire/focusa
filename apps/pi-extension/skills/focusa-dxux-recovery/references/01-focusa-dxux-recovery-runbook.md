...
```text
focusa_dxux_report -> focusa_dxux_requirement
focusa_dxux_requirement -> focusa_dxux_explain
focusa_dxux_explain -> focusa_dxux_digest
focusa_dxux_digest -> focusa_tool_doctor
```

## Minimal path

1. Call `focusa_dxux_report` with only required bounded inputs.
2. Call `focusa_dxux_requirement` with only required bounded inputs.
3. Call `focusa_dxux_explain` with only required bounded inputs.
4. Call `focusa_dxux_digest` with only required bounded inputs.
5. Call `focusa_tool_doctor` with only required bounded inputs.
