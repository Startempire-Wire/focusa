---
name: focusa-roadmap-to-workpoint
description: "Bounded reusable workflow for pre-implementation reasoning: scope, authority, decisions, blockers, graph, Trajectory, and Workpoint derivation."
---

# Focusa Roadmap To Workpoint

Bounded reusable workflow for pre-implementation reasoning: scope, authority, decisions, blockers, graph, Trajectory, and Workpoint derivation.

## Progressive disclosure

1. Load this core file only when its trigger matches.
2. Read `references/01-focusa-roadmap-to-workpoint-runbook.md` only for the selected workflow.
...
- before a code change in an unfamiliar scope
- engineering lifecycle stage selection
...
- workflow: `focusa-project-scope` → `focusa-roadmap-to-workpoint` → `focusa-workpoint` → `focusa-evidence-outcomes`
...
Skill file present with runbook; admitted via config/agent-skills-v2.json registry v2.
