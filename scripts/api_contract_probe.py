...
def main():
    ...
    add_check(
        "health_contract",
        ok,
        {"status": status, "body": body if body else raw, "parse_error": err},
    )
    ...
    status, headers, raw = req(
        args.base_url, "/v1/prompt/assemble", method="POST", body=payload
    )
    body, err = parse_json(raw)
    required = [
        "assembled",
    ...
    status, headers, raw = req(
        args.base_url, "/v1/prompt/assemble", method="POST", body={"turn_id": "bad"}
    )
    body, err = parse_json(raw)
    ...
    fixture_calls = [
        ...
        (
            "/v1/ecs/store",
            {"kind": "text", "label": "probe", "content_b64": "cHJvYmU="},
        ),
    ...
    miss = (
        []
        if b is None
        else check_fields(b, ["enabled", "scheduler", "guardrails", "telemetry"])
    )
    guardrail_miss = (
        []
        if b is None
            ...
            b.get("guardrails", {}),
            ["low_confidence_threshold", "no_delta_min_event_delta"],
        )
    )
    telemetry_miss = (
        []
    ...
    add_check(
        ...
        ok,
        {
            "status": s,
            "missing": miss,
            "guardrail_missing": guardrail_miss,
            "telemetry_missing": telemetry_miss,
            "body": b if b else r,
            "parse_error": e,
        },
    )
    ...
    hs, _, hr = req(
        args.base_url, "/v1/reflect/history?limit=20&stop_reason=low_confidence"
    )
    hb, he = parse_json(hr)
    ...
    ts, _, tr = req(
        args.base_url,
        "/v1/reflect/history?limit=20&since=2999-01-01T00:00:00Z&until=2999-01-02T00:00:00Z",
    )
    tb, te = parse_json(tr)
    ...
    us, _, ur = req(
        ...
        method="POST",
        body={"enabled": True, "cooldown_seconds": 3600},
    )
    ub, ue = parse_json(ur)
    ...
    add_check(
        ...
        ok,
        {
            ...
            "tick2_status_value": None
            if not isinstance(t2b, dict)
            else t2b.get("status"),
    ...
    s, _, r = req(
        ...
        "/v1/trust/metrics",
        method="PATCH",
        body={"event": "test_correction", "detail": "probe test"},
    )
    b, e = parse_json(r)
    ok = (
        s in (200, 403)
    ...
    add_check(
        ...
        ok,
        {"status": s, "body": b if b else r, "parse_error": e},
    )
    ...
    s, _, r = req(
        ...
        "/v1/ecs/store",
        method="POST",
        body={"kind": "text", "label": label, "content_b64": "cHJvYmU="},
    )
    b, e = parse_json(r)
    ...
    add_check(
        ...
        ok and found,
        {
            ...
            "handle_id": handle_id,
            "found_in_list": found,
            "store_body": b if b else r,
            "store_error": e,
            "handles_status": hs,
            "handles_body": hb if hb else hr,
            "handles_error": he,
        },
    )
