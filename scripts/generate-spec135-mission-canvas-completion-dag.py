...
STATE_FINDINGS = [
    ...
    {
        "id": "STATE-003",
        "classification": "pi_native_renderer_implemented",
        "finding": "ResolvedWorkspaceProjection is implemented in Core/API and the authoritative renderer mounts through ctx.ui.custom in the current Pi TypeScript TUI.",
        "evidence": ["repository search across apps/, crates/, and packages/"],
    },
    {
        "id": "STATE-004",
        "classification": "authoritative_pi_native_foundation",
        "finding": "The Pi extension has durable mode precedence, current-session lifecycle, adaptive activity/profile composition, actual surfaces, same-session transcript/editor, accessibility, and deterministic visual evidence.",
    ...
    },
    {
        ...
        "finding": "Core owns contribution resolution, eligibility, deterministic layout, persistence, profile memory, and omission diagnostics; Pi renders the bounded projection responsively.",
    ...
    },
    {
        ...
        "finding": "A2UI/Lit and Focusa elements remain portable renderer foundations while Pi-native C.R.I.S.T. activity cards bind the current canonical generated actions.",
...
phase(
    "P04",
    "Pi-native Mission Canvas lifecycle",
    "Mount and restore the authoritative Mission Canvas inside the current Pi TypeScript TUI without launching another host.",
    [
        ("architecture", [
            task("Write Pi-native Canvas lifecycle call-stack design and ADR", ["docs/contracts/spec135-pi-native-canvas-call-stack.v1.json", "docs/decisions/spec135-pi-native-canvas-host.md"], "The design proves ctx.ui.custom owns presentation only and preserves current Pi process/session authority.", "design"),
            task("Define current-Pi lifecycle and transcript events", ["apps/pi-extension/src/mission-canvas-tool.ts", "docs/contracts/spec135/mission-canvas-v1/operation-registry.yaml"], "Canvas lifecycle and Pi transcript/tool events use stable IDs without a remote-host precondition.", "contract"),
            task("Define Pi root mount/restore adapter", ["apps/pi-extension/src/mission-canvas-shell.ts"], "Mount, dispose, focus restoration, and stock-root restoration are explicit and idempotent.", "contract"),
            task("Prohibit browser, webview, sidecar, and remote host launch", ["docs/contracts/spec135-mission-canvas-host-renderer-contract.v1.yaml", "apps/pi-extension/src/mission-canvas-tool.ts"], "Canvas control cannot launch or transfer authority to another process or window.", "security"),
            task("Map current Pi SessionManager and ExtensionContext invariants", ["apps/pi-extension/src/mission-canvas-shell.ts", "docs/contracts/spec135-pi-native-canvas-call-stack.v1.json"], "The shell preserves current model, transcript, tools, attachments, and editor target.", "design"),
            task("Add no-sidecar lifecycle security gate", ["apps/pi-extension/tests/mission-canvas-pi-surface.test.mjs"], "Tests fail if Canvas imports or launches RichHostLifecycleManager.", "security"),
        ]),
        ("lifecycle", [
            task("Mount Canvas through ctx.ui.custom in current Pi", ["apps/pi-extension/src/commands.ts", "apps/pi-extension/src/mission-canvas-shell.ts"], "Canvas ON replaces only the visible root while current ExtensionContext and SessionManager remain active.", "integration"),
            task("Restore stock Pi through deterministic OFF", ["apps/pi-extension/src/mission-canvas-shell.ts", "apps/pi-extension/src/commands.ts"], "Canvas OFF restores the same Pi root, editor focus, transcript, tools, and session.", "integration"),
            task("Implement idempotent ON/OFF/toggle/status tool control", ["apps/pi-extension/src/mission-canvas-tool.ts"], "Repeated lifecycle actions never duplicate shells or create another host.", "integration"),
            task("Stream current Pi turns, messages, and tools into Canvas", ["apps/pi-extension/src/mission-canvas-tool.ts", "apps/pi-extension/src/mission-canvas-shell.ts"], "The Canvas transcript is the current Pi session rather than a copied or remote stream.", "integration"),
            task("Preserve unsent editor input across Canvas toggles", ["apps/pi-extension/src/mission-canvas-shell.ts"], "OFF restores the stock editor without losing pending operator text.", "integration"),
            task("Reject duplicate shell mounts and stale disposals", ["apps/pi-extension/src/mission-canvas-shell.ts"], "One active shell exists per current Pi session and disposal is idempotent.", "integration"),
        ]),
        ("state", [
            task("Persist interaction mode and workspace profile preferences", ["apps/pi-extension/src/config.ts"], "Project/user/session precedence is deterministic and migration-safe.", "persistence"),
            task("Keep shell state attachment-scoped and non-canonical", ["apps/pi-extension/src/mission-canvas-shell.ts", "apps/pi-extension/src/mission-canvas-view.ts"], "Renderer state cannot fork canonical project/session authority.", "persistence"),
            task("Migrate prior rich-host preferences to Pi-native Canvas", ["apps/pi-extension/src/config.ts", "docs/contracts/spec135-interaction-mode-toggle.v1.json"], "Legacy host preferences preserve profile and mode without launching old hosts.", "migration"),
            task("Bind Canvas state to exact Pi attachment identity", ["apps/pi-extension/src/state.ts", "apps/pi-extension/src/mission-canvas-tool.ts"], "Cross-session state cannot leak into the current Canvas.", "persistence"),
            task("Record restart and restoration receipts", ["apps/pi-extension/src/mission-canvas-tool.ts", "docs/contracts/spec135-mission-canvas-agent-first-gui-proof.v1.json"], "Lifecycle continuity is evidence-backed rather than inferred from mode state.", "recovery"),
        ]),
        ("compatibility", [
            task("Define Pi TUI capability handshake", ["apps/pi-extension/src/mission-canvas-accessibility.ts", "apps/pi-extension/src/mission-canvas-shell.ts"], "Renderer activation validates current Pi TUI APIs and viewport support.", "contract"),
            task("Enforce Pi extension/API compatibility", ["apps/pi-extension/package.json", "apps/pi-extension/src/index.ts"], "Supported Pi versions and extension contracts are explicit.", "contract"),
            task("Implement installed-extension reload workflow", ["apps/pi-extension/README.md", "docs/contracts/spec135-mission-canvas-agent-first-gui-proof.v1.json"], "Updated Canvas source reloads in the current Pi session without changing project authority.", "release"),
            task("Reconnect Canvas projection after daemon interruption", ["apps/pi-extension/src/commands.ts", "apps/pi-extension/src/workspace-invalidation.ts"], "Canvas preserves current Pi interaction and refreshes canonical projection after reconnect.", "recovery"),
            task("Verify Pi 0.83 custom-component compatibility", ["apps/pi-extension/tests/mission-canvas-pi-surface.test.mjs", "apps/pi-extension/package.json"], "The Canvas uses supported Pi TUI component and lifecycle APIs.", "integration"),
            task("Keep stock Pi as local degraded recovery root", ["apps/pi-extension/src/commands.ts", "apps/pi-extension/src/mission-canvas-shell.ts"], "Renderer failure restores stock Pi locally without launching a substitute host.", "recovery"),
        ]),
        ("distribution", [
            task("Package Canvas entirely in the Pi extension", ["apps/pi-extension/package.json", "apps/pi-extension/src"], "No Tauri binary, webview bundle, or browser runtime is required for Canvas ON.", "build"),
            task("Install and update Pi-native Canvas safely", ["apps/pi-extension/README.md", "scripts"], "Installation is atomic, backup-capable, and leaves stock Pi recoverable.", "release"),
            task("Verify current macOS Pi terminal lifecycle", ["apps/pi-extension/tests/mission-canvas-pi-surface.test.mjs"], "The current Mac proves ON/OFF, same session, same prompt editor, and no external host.", "integration"),
            task("Verify portable terminal behavior", ["apps/pi-extension/tests/mission-canvas-reference-design.test.mjs", "tests/spec135_mission_canvas_portability_test.py"], "Shared TypeScript/TUI behavior remains portable across supported terminals.", "integration"),
            task("Back up and roll back installed extension sources", ["apps/pi-extension/README.md", "docs/contracts/spec135-mission-canvas-agent-first-gui-proof.v1.json"], "Local installation is recoverable without touching the main worktree.", "release"),
            task("Run installed-extension package smoke test", ["apps/pi-extension/tests/mission-canvas-pi-surface.test.mjs"], "The installed extension resolves all Pi runtime imports and mounts the Canvas.", "release"),
        ]),
    ],
    ["SPEC135-F1", "SPEC135-F2", "SPEC135-F3", "SPEC135-F4", "SPEC135-F5", "SPEC135-F6", "SPEC135-F7", "SPEC135-F8", "SPEC135-F9", "SPEC135-F10", "SPEC135-F11", "SPEC135-F12", "SPEC135-P1", "SPEC135-P2", "SPEC135-P3", "SPEC135-P4", "SPEC135-P5"],
)

phase(
    "P05",
    "Authoritative adaptive Pi-native shell",
...
phase(
    ...
    [
        ...
        ("security", [
            task("Threat-model current Pi TUI, generated UI, extension events, and local API", ["docs/contracts/spec135-rich-host-threat-model.v1.md"], "Assets, origins, tokens, IPC, CSP, permissions, attachments, file access, clipboard, and generated actions have mitigations and tests.", "security", ["SPEC135-Q2"]),
            task("Enforce no-sidecar, terminal-escape, and trusted-origin policy", ["apps/pi-extension/rich-host/", "apps/pi-extension/ui/"], "No arbitrary remote content, eval, inline unsafe scripts, or untrusted navigation enters the rich host.", "security"),
        ...
        ("hardening-proof", [
            ...
            task("Revalidate Q1 through Q6 against Pi-native Canvas", ["docs/contracts/spec135-proof-matrix.v1.yaml"], "Security, privacy, performance, recovery, licensing/SBOM, and release gates are no longer inherited from terminal-only proof.", "integration", ["SPEC135-Q1", "SPEC135-Q2", "SPEC135-Q3", "SPEC135-Q4", "SPEC135-Q5", "SPEC135-Q6"]),
        ]),
    ],
...
phase(
    ...
    [
        ...
        ("integration", [
            ...
            task("Run macOS Pi-native Canvas release candidate", [".github/workflows/"], "Canonical pipeline builds, signs where configured, packages, installs, and proves the extension on macOS.", "ci"),
        ...
        ("acceptance", [
            task("Regenerate interaction-mode and Pi-native reference proof", ["docs/contracts/spec135-interaction-mode-toggle.v1.json", "docs/contracts/spec135-mission-canvas-agent-first-gui-proof.v1.json"], "Artifacts are generated from runtime traces and mark rich host accepted only when all lifecycle invariants pass.", "evidence", ["SPEC135-K1", "SPEC135-M1"]),
