...
```text
focusa_trajectory_view -> focusa_hlt_history
focusa_hlt_history -> focusa_trajectory_define_goal
focusa_trajectory_define_goal -> focusa_trajectory_assess
focusa_trajectory_assess -> focusa_trajectory_propose_workpoint
focusa_trajectory_propose_workpoint -> focusa_trajectory_checkpoint
focusa_trajectory_checkpoint -> focusa_trajectory_resume
```

## Minimal path

1. Call `focusa_trajectory_view` with only required bounded inputs.
2. Call `focusa_hlt_history` with only required bounded inputs.
3. Call `focusa_trajectory_define_goal` with only required bounded inputs.
4. Call `focusa_trajectory_assess` with only required bounded inputs.
5. Call `focusa_trajectory_propose_workpoint` with only required bounded inputs.
6. Call `focusa_trajectory_checkpoint` with only required bounded inputs.
7. Call `focusa_trajectory_resume` with only required bounded inputs.
