...
```text
focusa_predict_evaluate -> focusa_predict_recent
focusa_predict_recent -> focusa_predict_record
focusa_predict_record -> focusa_predict_stats
focusa_predict_stats -> focusa_prediction_authority
```

## Minimal path

1. Call `focusa_predict_evaluate` with only required bounded inputs.
2. Call `focusa_predict_recent` with only required bounded inputs.
3. Call `focusa_predict_record` with only required bounded inputs.
4. Call `focusa_predict_stats` with only required bounded inputs.
5. Call `focusa_prediction_authority` with only required bounded inputs.
