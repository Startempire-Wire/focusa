# Focusa Temporal Authority Runbook
...
focusa_hlt_history -> focusa_predict_record
focusa_predict_record -> focusa_predict_evaluate
focusa_predict_evaluate -> focusa_evidence_capture
```
...
3. Call `focusa_predict_record` with only required bounded inputs.
4. Call `focusa_predict_evaluate` with only required bounded inputs.
5. Call `focusa_evidence_capture` with only required bounded inputs.
...
- Done: Decision cites current temporal evidence, freshness, confidence, and bounded forecast/recovery.
