...
impl ConnectorAuthLifecycle {
    pub fn new(config: ConnectorOAuthConfig) -> Result<Self, ConnectorCredentialStatus> {
            ...
            || config.keyring_account.trim().is_empty()
        {
            return Err(status(
                &config.connector_id,
                "invalid_config",
                "repair_connector_configuration",
            ));
        }
    ...
    pub fn begin_authorization(
        &self,
    ) -> Result<ConnectorAuthorizationRequest, ConnectorCredentialStatus> {
        let client = BasicClient::new(ClientId::new(self.config.client_id.clone()))
            .set_auth_uri(
                AuthUrl::new(self.config.authorization_url.clone()).map_err(|_| self.invalid())?,
            )
            .set_token_uri(
                TokenUrl::new(self.config.token_url.clone()).map_err(|_| self.invalid())?,
            )
            .set_redirect_uri(
                RedirectUrl::new(self.config.redirect_url.clone()).map_err(|_| self.invalid())?,
            );
        ...
        self.entry("pkce")?
            .set_password(verifier.secret())
            .map_err(|_| self.keyring_failure())?;
        self.entry("csrf")?
            .set_password(csrf.secret())
            .map_err(|_| self.keyring_failure())?;
    ...
    pub fn store_access_token(
        &self,
        access_token: &str,
    ) -> Result<ConnectorCredentialStatus, ConnectorCredentialStatus> {
    ...
    pub fn access_token(&self) -> Result<String, ConnectorCredentialStatus> {
        self.entry("access")?.get_password().map_err(|_| {
            status(
                &self.config.connector_id,
