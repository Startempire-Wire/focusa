  ...
  "confidence": 0.0-1.0
    ...
    async fn run_gate_pipeline(&mut self) {
        ...
        if changed {
            ...
            // The pipeline already computed a bounded Focus Gate projection.
            // Applying that small projection directly avoids cloning/reducing the
            // complete large state a second time on the Tokio worker. Replay uses
            // the matching reducer event and reaches the same state/version.
            self.state.focus_gate = gate_after;
