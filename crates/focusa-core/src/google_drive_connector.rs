...
impl GoogleDriveConnector {
    ...
    pub fn new(
            ...
            || attachment_id.trim().is_empty()
        {
            return Err(error(
                &connector_id,
                "scope_missing",
                "Google Drive connector requires exact Focusa scope",
            ));
        }
    ...
    pub async fn health(&self) -> ConnectorHealth {
        ...
        let (status, message) = match status {
            Ok(response) if response.status().is_success() => {
                (ConnectorHealthStatus::Ready, "Google Drive connector ready")
            }
            Ok(response)
                if response.status().as_u16() == 401 || response.status().as_u16() == 403 =>
            {
                (
                    ...
                    "Google Drive authorization expired or revoked",
                )
            }
            Ok(_) => (
                ConnectorHealthStatus::Degraded,
                "Google Drive health returned a non-success status",
            ),
            Err(_) => (
                ConnectorHealthStatus::Offline,
                "Google Drive health request failed",
            ),
        };
        ConnectorHealth {
            status,
    ...
    pub async fn discover(
        ...
        let response = request.send().await.map_err(|_| {
            error(
                &self.connector_id,
                "transport",
                "Google Drive discovery failed",
            )
        })?;
        if !response.status().is_success() {
            return Err(status_error(&self.connector_id, response.status().as_u16()));
        }
        let payload = response
            ...
            .map_err(|_| {
                error(
                    &self.connector_id,
                    "invalid_response",
                    "Google Drive discovery response was invalid",
                )
            })?;
    ...
    pub async fn import_candidate(
        &self,
        file_id: &str,
    ) -> Result<GoogleDriveImportCandidate, ConnectorErrorEnvelope> {
        ...
        let response = self
            ...
            .get(&url)
            .bearer_auth(token)
            .query(&[("fields", "id,name,mimeType,modifiedTime,size,webViewLink")])
            .send()
            .await
            .map_err(|_| {
                error(
                    &self.connector_id,
                    "transport",
                    "Google Drive import lookup failed",
                )
            })?;
        if !response.status().is_success() {
            return Err(status_error(&self.connector_id, response.status().as_u16()));
        }
        let file = response.json::<GoogleDriveFile>().await.map_err(|_| {
            error(
                &self.connector_id,
                "invalid_response",
                "Google Drive file metadata was invalid",
            )
        })?;
    ...
    fn token(&self) -> Result<String, ConnectorErrorEnvelope> {
        self.auth
            .access_token()
            .map_err(|status| error(&self.connector_id, &status.status, &status.recovery_action))
    }
}

fn status_error(connector_id: &str, code: u16) -> ConnectorErrorEnvelope {
    let class = if code == 401 || code == 403 {
    ...
    error(
        connector_id,
        class,
        "Google Drive returned a non-success status",
    )
}
...
fn error(connector_id: &str, class: &str, message: &str) -> ConnectorErrorEnvelope {
    ConnectorErrorEnvelope {
        ...
        retry_after_ms: if class == "rate_limited" {
            Some(1_000)
        } else {
            None
        },
