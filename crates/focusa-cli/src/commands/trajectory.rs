...
fn print_summary(label: &str, resp: &Value) {
    ...
    let trajectory = resp.get("trajectory").or_else(|| resp.pointer("/resume_packet/trajectory")).unwrap_or(&null_value);
    let definition = trajectory.get("definition_status").and_then(Value::as_str).unwrap_or("unknown");
    let project = resp.pointer("/project_identity/status").or_else(|| resp.pointer("/resume_packet/project_identity/status")).and_then(Value::as_str).unwrap_or("unknown");
    println!("trajectory {label}: status={status} canonical={canonical} project={project} definition={definition}");
    if let Some(next) = resp.get("recommended_action").and_then(Value::as_str)
        .or_else(|| resp.pointer("/intelligence_view/context_sufficiency/recommended_action").and_then(Value::as_str)) {
        println!("  action: {next}");
    }
...
pub async fn run(cmd: TrajectoryCmd, json_output: bool) -> anyhow::Result<()> {
    ...
    let (label, resp) = match cmd {
        ...
        TrajectoryCmd::Resume(scope) => ("resume", api.post("/v1/trajectory/resume", &json!({
            "project_root": scope.project_root,
            "session_id": scope.session_id,
            "continuity_id": scope.continuity_id,
            "mode": scope.mode,
        })).await?),
        TrajectoryCmd::DefineGoal { long_term_goal, desired_end_state, short_term_goal, current_state, goal_source, supersedes_trajectory_id, operator_confirmed, supersession_evidence_refs, project_root, session_id, continuity_id, idempotency_key } => ("define-goal", api.post("/v1/trajectory/define-goal", &json!({
        ...
        TrajectoryCmd::Assess { observed_state, evidence_refs, project_root, session_id, continuity_id } => ("assess", api.post("/v1/trajectory/assess", &json!({
        ...
        TrajectoryCmd::ProposeWorkpoint { trajectory_id, target_ref, action_type, project_root, session_id, continuity_id } => ("propose-workpoint", api.post("/v1/trajectory/propose-workpoint", &json!({
        ...
        TrajectoryCmd::Checkpoint { summary, project_root, session_id, continuity_id, idempotency_key } => ("checkpoint", api.post("/v1/trajectory/checkpoint", &json!({
            "summary": summary,
            "project_root": project_root,
            "session_id": session_id,
            "continuity_id": continuity_id,
