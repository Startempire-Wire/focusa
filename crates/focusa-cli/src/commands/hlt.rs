...
pub async fn run(cmd: HltCmd, _json_output: bool) -> anyhow::Result<()> {
    ...
    match cmd {
        HltCmd::Ls { project_root, continuity_id, mode, json } => {
            run_ls(&api, &cwd, project_root.as_deref(), continuity_id.as_deref(), &mode, json).await
        }
        HltCmd::Set { project_root, hlt, desired_end_state, continuity_id, mlg, stg, waypoints, reason, evidence_refs, confirm } => {
            run_set(&api, &cwd, project_root.as_deref(), &hlt, &desired_end_state, continuity_id.as_deref(), mlg.as_deref(), stg.as_deref(), waypoints, reason.as_deref(), evidence_refs, confirm).await
        }
        HltCmd::Reset { project_root, continuity_id, steps, dry_run } => {
            run_reset(&api, &cwd, project_root.as_deref(), continuity_id.as_deref(), &steps, dry_run).await
        }
        HltCmd::History { project_root, continuity_id, limit, json } => {
            run_history(&api, &cwd, project_root.as_deref(), continuity_id.as_deref(), &limit, json).await
        }
        HltCmd::Diff { project_root, continuity_id, from, to } => {
            run_diff(&api, &cwd, project_root.as_deref(), continuity_id.as_deref(), &from, &to).await
        }
        HltCmd::Mlg { project_root, continuity_id, set, list } => {
            run_mlg(&api, &cwd, project_root.as_deref(), continuity_id.as_deref(), set.as_deref(), list).await
        }
        HltCmd::Stg { project_root, continuity_id, set, list } => {
            run_stg(&api, &cwd, project_root.as_deref(), continuity_id.as_deref(), set.as_deref(), list).await
        }
        HltCmd::Waypoint { project_root, continuity_id, add, list, complete } => {
            run_waypoint(&api, &cwd, project_root.as_deref(), continuity_id.as_deref(), add.as_deref(), list, complete.as_deref()).await
        }
        HltCmd::Assess { project_root, continuity_id, current_state, evidence_refs } => {
            run_assess(&api, &cwd, project_root.as_deref(), continuity_id.as_deref(), &current_state, evidence_refs).await
        }
        HltCmd::Checkpoint { project_root, continuity_id, summary } => {
            run_checkpoint(&api, &cwd, project_root.as_deref(), continuity_id.as_deref(), summary.as_deref()).await
        }
        HltCmd::Supersede { project_root, new_hlt, evidence_refs, reason, continuity_id } => {
            run_supersede(&api, &cwd, project_root.as_deref(), &new_hlt, evidence_refs, &reason, continuity_id.as_deref()).await
        }
        HltCmd::Export { project_root, continuity_id, output, format } => {
            run_export(&api, &cwd, project_root.as_deref(), continuity_id.as_deref(), &output, &format).await
        }
        HltCmd::Verify { project_root, continuity_id } => {
            run_verify(&api, &cwd, project_root.as_deref(), continuity_id.as_deref()).await
        }
    }
}

fn get_project_root(project_root: Option<&str>, cwd: &str) -> String {
    project_root.map(String::from).unwrap_or_else(|| cwd.to_string())
}

fn build_query(path: &str, project_root: &str, continuity_id: Option<&str>) -> String {
    let mut q = format!("{}?project_root={}", path, urlencoding::encode(project_root));
...
async fn run_ls(api: &ApiClient, cwd: &str, project_root: Option<&str>, continuity_id: Option<&str>, mode: &str, json_output: bool) -> anyhow::Result<()> {
    ...
    let hlt = response.pointer("/trajectory/long_term_goal").and_then(|v| v.as_str()).unwrap_or("(not set)");
    let mlg = response.pointer("/trajectory/mid_level_goal").and_then(|v| v.as_str());
    let stg = response.pointer("/trajectory/short_term_goal").and_then(|v| v.as_str());
    let desired = response.pointer("/trajectory/desired_end_state").and_then(|v| v.as_str()).unwrap_or("(not set)");
    let waypoints = response.pointer("/trajectory/waypoints").and_then(|v| v.as_array());
...
async fn run_set(api: &ApiClient, cwd: &str, project_root: Option<&str>, hlt: &str, desired_end_state: &str, continuity_id: Option<&str>, mlg: Option<&str>, stg: Option<&str>, waypoints: Vec<String>, _reason: Option<&str>, evidence_refs: Vec<String>, confirm: bool) -> anyhow::Result<()> {
    let project_root = get_project_root(project_root, cwd);
    ...
    let persisted = response.get("persisted").and_then(|v| v.as_bool()).unwrap_or(false);
...
async fn run_reset(api: &ApiClient, cwd: &str, project_root: Option<&str>, continuity_id: Option<&str>, steps: &str, dry_run: bool) -> anyhow::Result<()> {
    let project_root = get_project_root(project_root, cwd);
    ...
    let target_idx = entries.len().saturating_sub(1 + steps.min(entries.len() - 1));
    let target_entry = &entries[target_idx];
    
    let prev_hlt = target_entry.get("old_hlt").and_then(|v| v.as_str()).unwrap_or("(initial)");
    let new_hlt = target_entry.get("new_hlt").and_then(|v| v.as_str()).unwrap_or("(none)");

    println!("Reset HLT for: {}", project_root);
    println!("  Current: {}", wrap_text(new_hlt, 60));
    println!("  Reset to ({} steps back): {}", steps, wrap_text(prev_hlt, 60));
...
async fn run_history(api: &ApiClient, cwd: &str, project_root: Option<&str>, continuity_id: Option<&str>, limit: &str, json_output: bool) -> anyhow::Result<()> {
    let project_root = get_project_root(project_root, cwd);
    ...
    let response: Value = api.get(&url).await?;
    let ledger_file = response.get("ledger_file").and_then(|v| v.as_str()).unwrap_or("unknown");
    ...
    if let Some(entries) = response.get("entries").and_then(|v| v.as_array()) {
        for (i, entry) in entries.iter().enumerate() {
            let old_hlt = entry.get("old_hlt").and_then(|v| v.as_str()).unwrap_or("(initial)");
            let new_hlt = entry.get("new_hlt").and_then(|v| v.as_str()).unwrap_or("unknown");
            let ts = entry.get("timestamp").and_then(|v| v.as_str()).unwrap_or("unknown");
            let source = entry.get("source").and_then(|v| v.as_str()).unwrap_or("unknown");
...
async fn run_diff(api: &ApiClient, cwd: &str, project_root: Option<&str>, continuity_id: Option<&str>, from: &str, to: &str) -> anyhow::Result<()> {
    let project_root = get_project_root(project_root, cwd);
    ...
    let from_entry = entries.get(entries.len().saturating_sub(from))
    ...
    let from_hlt = from_entry.get("new_hlt").and_then(|v| v.as_str()).unwrap_or("(none)");
    let to_hlt = to_entry.get("new_hlt").and_then(|v| v.as_str()).unwrap_or("(none)");
    let from_ts = from_entry.get("timestamp").and_then(|v| v.as_str()).unwrap_or("unknown");
    let to_ts = to_entry.get("timestamp").and_then(|v| v.as_str()).unwrap_or("unknown");
...
async fn run_mlg(api: &ApiClient, cwd: &str, project_root: Option<&str>, continuity_id: Option<&str>, set: Option<&str>, list: bool) -> anyhow::Result<()> {
    let project_root = get_project_root(project_root, cwd);
    ...
    if list {
        ...
        let response: Value = api.get(&url).await?;
        let mlg = response.pointer("/trajectory/mid_level_goal").and_then(|v| v.as_str()).unwrap_or("(not set)");
    ...
    if let Some(set_mlg) = set {
        ...
        let view_url = format!("/v1/trajectory/view?project_root={}", urlencoding::encode(&project_root));
...
async fn run_stg(api: &ApiClient, cwd: &str, project_root: Option<&str>, continuity_id: Option<&str>, set: Option<&str>, list: bool) -> anyhow::Result<()> {
    let project_root = get_project_root(project_root, cwd);
    ...
    if list {
        ...
        let url = format!("{}&mode=summary", path);
        let response: Value = api.get(&url).await?;
        let stg = response.pointer("/trajectory/short_term_goal").and_then(|v| v.as_str()).unwrap_or("(not set)");
...
async fn run_waypoint(api: &ApiClient, cwd: &str, project_root: Option<&str>, continuity_id: Option<&str>, add: Option<&str>, list: bool, _complete: Option<&str>) -> anyhow::Result<()> {
    let project_root = get_project_root(project_root, cwd);

    let path = build_query("/v1/trajectory/view", &project_root, continuity_id);
    let url = format!("{}&mode=summary", path);
    let response: Value = api.get(&url).await?;
    let waypoints = response.get("waypoints").and_then(|v| v.as_array())
...
async fn run_assess(api: &ApiClient, cwd: &str, project_root: Option<&str>, continuity_id: Option<&str>, current_state: &str, evidence_refs: Vec<String>) -> anyhow::Result<()> {
    let project_root = get_project_root(project_root, cwd);
    ...
    if let Some(gap) = response.get("gaps").and_then(|v| v.get(0)).and_then(|v| v.as_str()) {
        println!();
...
async fn run_checkpoint(api: &ApiClient, cwd: &str, project_root: Option<&str>, continuity_id: Option<&str>, summary: Option<&str>) -> anyhow::Result<()> {
    let project_root = get_project_root(project_root, cwd);
    ...
    let traj_id = response.get("trajectory_id").and_then(|v| v.as_str()).unwrap_or("unknown");
    
    println!("✓ Trajectory checkpointed: {}", traj_id);
    Ok(())
}

async fn run_supersede(api: &ApiClient, cwd: &str, project_root: Option<&str>, new_hlt: &str, evidence_refs: Vec<String>, reason: &str, continuity_id: Option<&str>) -> anyhow::Result<()> {
    let project_root = get_project_root(project_root, cwd);
...
async fn run_export(api: &ApiClient, cwd: &str, project_root: Option<&str>, continuity_id: Option<&str>, output: &str, format: &str) -> anyhow::Result<()> {
    let project_root = get_project_root(project_root, cwd);
    ...
    let content = match format {
        ...
        _ => {
            let entries = response.get("entries").and_then(|v| v.as_array());
            entries.map(|e| e.iter().filter_map(|v| serde_json::to_string(v).ok()).collect::<Vec<_>>().join("\n")).unwrap_or_default()
        }
    };
...
async fn run_verify(api: &ApiClient, cwd: &str, project_root: Option<&str>, continuity_id: Option<&str>) -> anyhow::Result<()> {
    let project_root = get_project_root(project_root, cwd);
    ...
    let ledger_file = ledger_response.get("ledger_file").and_then(|v| v.as_str()).unwrap_or("unknown");
    let count = ledger_response.get("count").and_then(|v| v.as_u64()).unwrap_or(0);
    ...
    let current_hlt = traj_response.get("long_term_goal").and_then(|v| v.as_str()).unwrap_or("(not set)");
    ...
    if let Some(entries) = ledger_response.get("entries").and_then(|v| v.as_array()) {
        if let Some(latest) = entries.last() {
            ...
            println!();
            println!("  Ledger ↔ Trajectory: {}", if matches { "✓" } else { "⚠ mismatch" });
        }
    }

    println!();
    println!("✓ Verification complete (scope: {})", if continuity_id.is_some() { "continuity" } else { "project" });
